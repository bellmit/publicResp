...
/*
		...
		* validate-percentage (a valid percentage,can have leading spaces/zeroes,with/without decimal places)
...
function FIC_checkField(c,e) {
    ...
    } else if(c.indexOf(' validate-percentage ') != -1) {
    	    if(isNaN(t)) {
    	    	valid=false;
    	    }
    		if(valid) {
    			var percentage = parseFloat(t);
    			if (percentage < 0 || percentage > 100) {
    				valid=false;
    			}
    		}
    } else if(c.indexOf(' validate-positive-decimal ') != -1 &&
            (isNaN(parseFloat(t)) || !t.match(/^\d*\.?\d+$/) || t < 0.00)) {
            valid=false;
    }
	return valid;
}
