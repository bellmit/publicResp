...
function saveForm() {
	...
	if (max_centers > 1) {
		...
		if (!checked) {
			alert("Please select the supplier applicability for centers");
			return false;
		}
	...
	document.supplier_center_association_form.submit();
	return true;
}
...
function showAllCenters(obj) {
	document.getElementById('d_checkAllCenters').checked = false;
	checkOrUncheckAll('d_center_chkbox', document.getElementById('d_checkAllCenters'))
...
function searchCenters(currentPage) {
	...
	offSet = curPage == 1 ? 0 : ((curPage-1)*10);
	...
	if (!empty(centers)) { 
		for (var i=offSet; i<offSet+10; i++) {
			...
			/*if (document.getElementById('show_all_centers_chkbox').checked == true ) {
				document.getElementsByName('d_center_chkbox')[id].checked = false;
			}*/
		}
	}
...
function addToCenterTable() {
	...
	for (var i=0; i<center_chkboxes.length-1; i++) {
		...
		setHiddenValue(id, "supp_center_id", "_");
...
function isDuplicate(type, id) {
	var els = document.getElementsByName(type == 'S' ? 'supplier_id' : 'center_id');
	var delete_el = document.getElementsByName(type == 'S' ? 'supplier_delete' : 'cntr_delete');
...
function setCenterRowStyle(i) {
	...
	var packCenterId = getIndexedValue("supp_center_id", i);
...
function cancelCenter(obj) {
	...
	var isNew = getIndexedValue("supp_center_id", id) == '_';
...
function getIndexedPaise(name, index) {
	return getElementPaise(getIndexedFormElement(document.supplier_center_association_form, name, index));
}

function setIndexedValue(name, index, value) {
	var obj = getIndexedFormElement(document.supplier_center_association_form, name, index);
	if (obj)
		obj.value = value;
	return obj;
}
...
function setHiddenValue(index, name, value) {
	var el = getIndexedFormElement(document.supplier_center_association_form, name, index);
	if (el) {
