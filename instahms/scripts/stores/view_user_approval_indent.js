var toolbar = {}
	toolbar.View= {
		...
		href: 'stores/StoresUserIndentApproval.do?_method=show',
		onclick: null,
	...
	toolbar.Print= {
		...
		href: 'stores/StoresUserIndentApproval.do?_method=generateReport&target=_blank',
		onclick: null,
		target:"_blank",
...
function init() {
	theForm = document.indentApprovalSearchForm;
...
function checkstoreallocation() {
  if(indentApprovalBy != 'R'){
	if(gRoleId != 1 && gRoleId != 2) {
		if(!empty(isUserHavingSuperStore) && isUserHavingSuperStore == 'N') {
