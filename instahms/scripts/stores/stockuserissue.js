	...
	if(gItemType == 'orderkit') {
		...
		if(document.getElementById("itemListtable").rows.length>1){
			for(var i=1;i<=document.getElementById("itemListtable").rows.length-1;i++){
				...
				med_id=currentMedicineId;
				batch=currentbatchNo;
				...
				if (('undefined' == document.getElementById("expdt").value) ||
					...
					if (null != gAllowExpiredSale){
						if ((gAllowExpiredSale != 'Y') && ( daysToExpire <= 0 )) {
							...
							return false;
						}var daysToExpire = itemBatchDetails.exp_dt != null ? getDaysToExpire(itemBatchDetails.exp_dt) : gExpiryWarnDays+1;
						 if (daysToExpire <= gExpiryWarnDays && daysToExpire > 0) {
							...
							} if(daysToExpire < 0)
							 {
								 msg=(getString("js.sales.issues.issuereturn.warning.someitemsfor")
											...
											+ med_id
											+ " "
											+ getString("js.sales.issues.issuereturn.arepastexpirydate"));
									alert(msg);
								 }
						if (nearingExpiryQty > 0) {
							msg = getString("js.sales.issues.storesuserissues.warning.someitemsfor")+" " + med_id +" "+ getString("js.sales.issues.storesuserissues.aresoontoexpire")+"\n";
							alert(msg);
						}
					}
				}
...
function validate(button,type){
	...
	//validate IP patient credit limit check
	/*if(type == 'Patient') {
...
function validateIPCreditLimit() {
	...
	var visitType = document.patientissueform.visitType.value;
	if(visitType != 'i') {
		return true;
	}
	...
	var newPatientAmt = 0;
	var newPatientAmtEl = document.getElementById('totPatAmt');
	...
	if(ip_credit_limit_rule == 'B') {
		if((visitTotalNewPatDueAmt > gVisitCreditLimitAmt) && gVisitCreditLimitAmt > 0) {
			var msg=getString("js.sales.issues.ipcreditlimitis");
			msg+=' '+ gVisitCreditLimitAmt;
			msg+="\n";
			msg+=getString("js.sales.issues.and.below.currentoutstanding");
			msg+=' '+ visitTotalNewPatDueAmt;
			alert(msg);
			return false;
		}
...
function getPatientDetails() {
	...
	var url = 'StockPatientIssue.do?_method=getPatientDetailsJSON&mrno='+mrno+"&storeId="+document.getElementById("store").value;
...
function getPatientFromBill(){
	if ((null != visitIdFromBill) && (visitIdFromBill != '') && !noAccess ){
		...
		var url = 'StockPatientIssue.do?_method=getPatientDetailsJSON&mrno='+visitIdFromBill+"&storeId="+indentStore;
...
function setPatientDetails(patient) {
	...
	document.patientissueform.visitId.value = patient.patient_id;
	...
	gVisitPatientDue = patient.visitTotalPatientDue != null && patient.visitTotalPatientDue != '' ? parseFloat(patient.visitTotalPatientDue) : 0;
}
...
function setItems(){
	...
	if ((typeof(jMedicineNames) != 'undefined') && (jMedicineNames != null)){
		...
		oItemAutoComp.minQueryLength = 0;
		oItemAutoComp.forceSelection = true;
		oItemAutoComp.resultTypeList = false;
		oItemAutoComp.filterResults = Insta.queryMatchWordStartsWith;
		var reArray = [];
		oItemAutoComp.formatResult = function(oResultData, sQuery, sResultMatch) {
			var escapedComp = Insta.escape(sQuery);
			reArray[0] = new RegExp( '^' + escapedComp, 'i');
			reArray[1] = new RegExp( "\\s" + escapedComp, 'i');
			var title = oResultData.cust_item_code_with_name;
			var det = highlight(oResultData.cust_item_code_with_name,reArray);
...
function onSelectItem(type, args) {
	var selItemName = args[2]["medicine_name"];
	...
	if ((isNotNullObj(issueRateExpr) || isNotNullObj(visitSellingPriceExpr) || isNotNullObj(storeSellingPriceExpr)) && isNotNullObj(medicine_id)) {
		...
		if(itemBatchId != undefined && itemBatchId != 'undefined' && itemBatchId != '' && itemBatchId != null) {
			...
			var param = 'item_batch_id='+itemBatchId+"&storeId="+document.getElementById("store").value+"&qty="+qty+
							"&medicine_id="+medicine_id+"&bed_type="+gBedType+"&discount="+discount;
...
function setOrderDetails(type, args) {
	...
	if(visitId != undefined && visitId != '' ) {
		...
		var visitId =  patient.patient_id;
...
function setSellingPrice(batch, qtyForBatch, itemRate, medicineId, discount, patient, id) {
    ...
    if ((isNotNullObj(issueRateExpr)) || (isNotNullObj(visitSellingPriceExpr)) || (isNotNullObj(storeSellingPriceExpr))) {
    	...
    	var param = 'item_batch_id='+batchId+
						...
						"&medicine_id="+medicineId+"&bed_type="+gBedType+"&discount="+discount;
		...
		if (ajaxReqObject.readyState == 4) {
			if ( (ajaxReqObject.status == 200) && (ajaxReqObject.responseText!=null) ) {
				itemRate = eval(ajaxReqObject.responseText);
