...
function saveForm() {
	...
	document.doc_templ_center_association_form.submit();
	return true;
}
...
function addToCenterTable() {
	...
	for (var i=0; i<center_chkboxes.length-1; i++) {
		...
		setHiddenValue(id, "doc_template_center_id", "_");
...
function setCenterRowStyle(i) {
	...
	var packCenterId = getIndexedValue("doc_template_center_id", i);
...
function cancelCenter(obj) {
	...
	var isNew = getIndexedValue("doc_template_center_id", id) == '_';
...
function getIndexedPaise(name, index) {
	return getElementPaise(getIndexedFormElement(document.doc_templ_center_association_form, name, index));
}

function setIndexedValue(name, index, value) {
	var obj = getIndexedFormElement(document.doc_templ_center_association_form, name, index);
	if (obj)
		obj.value = value;
	return obj;
}
...
function setHiddenValue(index, name, value) {
	var el = getIndexedFormElement(document.doc_templ_center_association_form, name, index);
	if (el) {
