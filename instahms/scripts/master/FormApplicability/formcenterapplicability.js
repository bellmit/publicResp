function init() {
	...
	if (max_centers > 1) {
		var app_for_centers = document
				.getElementsByName('applicable_for_centers');
		var checked = false;
...
function toggleCenterAddIcon(enableAdd) {
	...
	document.getElementById('centerAddIconEnabled').style.display = enableAdd ? 'block'
			: 'none';
	document.getElementById('centerAddIconDisabled').style.display = !enableAdd ? 'block'
			: 'none';
}
...
function saveForm() {
	...
	if (max_centers > 1) {
		...
		if (!checked) {
			alert("Please select the form component applicability for centers");
			return false;
		}
	...
	document.form_components_center_association_form.submit();
	return true;
}
...
function populateCities() {
	...
	var stateId = document.getElementById('fc_state').value;
	var city = document.getElementById('fc_city');
...
function initAddCenterDialog() {
	...
	YAHOO.util.Event.addListener('fc_center_ok', 'click', addToCenterTable,
			addCenterDialog, true);
	YAHOO.util.Event.addListener('fc_center_cancel', 'click',
			cancelCenterDialog, addCenterDialog, true);
	YAHOO.util.Event.addListener('fc_search_centers', 'click', getCenters,
			addCenterDialog, true);
...
function showAllCenters(obj) {
	...
	document.getElementById('fc_state').value = '';
	document.getElementById('fc_city').value = '';
	if (obj.checked) {
		searchCenters(1);
	} else {
...
function searchCenters(currentPage) {
	...
	var stateId = document.getElementById('fc_state').value;
	var cityId = document.getElementById('fc_city').value;
	...
	if (!empty(centers)) {
		for (var i = offSet; i < offSet + 10; i++) {
			...
			setHiddenValue(id, "fc_center_name", centers[i].center_name);
			setHiddenValue(id, "fc_state_name", centers[i].state_name);
			setHiddenValue(id, "fc_city_name", centers[i].city_name);

			document.getElementsByName('fc_center_chkbox')[id].value = centers[i].center_id;
	...
	if (numPages <= 1) {
		...
		if (curPage > 1) {
			...
			label.setAttribute('style',
					'text-decoration:none; color:#336699; cursor: pointer');
...
function addToCenterTable() {
	var center_chkboxes = document.getElementsByName('fc_center_chkbox');
	var state_name = document.getElementsByName('fc_state_name');
	var city_name = document.getElementsByName('fc_city_name');
	var center_name = document.getElementsByName('fc_center_name');
	for (var i = 0; i < center_chkboxes.length - 1; i++) {
		var center_id = center_chkboxes[i].value;

		if (!center_chkboxes[i].checked)
			continue;
		if (isDuplicate('C', center_id))
			continue;
		...
		setHiddenValue(id, "newly_added", "true");
		setHiddenValue(id, "cntr_added", "true");
...
function isDuplicate(type, id) {
	var els = document.getElementsByName(type == 'S' ? 'form_component_id'
			: 'center_id');
	var delete_el = document
			.getElementsByName(type == 'S' ? 'form_component_delete'
					: 'cntr_delete');
...
function initEditCenterDialog() {
	...
	YAHOO.util.Event.addListener('edit_center_ok', 'click', editCenterRow,
			editCenterDialog, true);
	YAHOO.util.Event.addListener('edit_center_cancel', 'click',
			cancelEditCenterDialog, editCenterDialog, true);
	YAHOO.util.Event.addListener('edit_center_previous', 'click',
			openPreviousCenter, editCenterDialog, true);
	YAHOO.util.Event.addListener('edit_center_next', 'click', openNextCenter,
			editCenterDialog, true);
...
function showEditCenterDialog(obj) {
	...
	document.getElementById('ed_center_label').textContent = getIndexedValue(
			'center_name', id);
	document.getElementById('ed_state_label').textContent = getIndexedValue(
			'state_name', id);
	document.getElementById('ed_city_label').textContent = getIndexedValue(
			'city_name', id);
	document.getElementById('ed_center_status').value = getIndexedValue(
			'center_status', id);
...
function setCenterRowStyle(i) {
	...
	var packCenterId = getIndexedValue("form_components_center_id", i);
	...
	var added = getIndexedValue("cntr_added", i) == 'true';
...
function cancelCenter(obj) {
	...
	var isNew = getIndexedValue("newly_added", id) == 'true'
...
function getTemplateRow(tableId) {
	// gets the hidden template row index: this follows header row + num
	// charges.
...
function getIndexedPaise(name, index) {
	return getElementPaise(getIndexedFormElement(
			document.form_components_center_association_form, name, index));
}

function setIndexedValue(name, index, value) {
	var obj = getIndexedFormElement(
			document.form_components_center_association_form, name, index);
	if (obj)
		obj.value = value;
	return obj;
