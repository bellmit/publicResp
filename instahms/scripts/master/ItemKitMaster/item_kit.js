...
function newItem() {
	var item = {
		kit_item_id: '', kit_id: '', qty: '',medicine_name: ''
		};
...
function dialogValidate(){
	...
	if (!validateRequired(dlgForm.qty, 'Enter Quantity')) return false;
	if (!isValidNumber(dlgForm.qty, 'Y')) return false;

	if(dlgForm.qty.value == 0 ){
...
function saveDialogItemToGrid(rowIndex) {
	...
	getElementByName(row,'kit_item_id').value = dlgForm.medicine_id.value;
	getElementByName(row,'qty').value = dlgForm.qty.value;
	...
	setNodeText(row.cells[2], dlgForm.qty.value);
	row.className = '';
}
...
function openEditDialog(row) {
    ...
    dlgForm.medicine_id.value = getElementByName(row,'kit_item_id').value;
...
function onSubmit(){
	if ( validate() ){
		document.otkitform.submit();
	}
}

function validate(){
	if ( document.otkitform.kit_name.value == '' ) {
		alert("Please enter kit name");
		document.otkitform.kit_name.focus();
		return false;
	}
	...
	if ( !hasItems ) {
		alert("Add atleast one item to kit");
		return false;
	} else {
...
function onDeleteRow(obj) {
    ...
    var detailId  = getElementByName(row,"kit_item_detail_id").value;
	var rowIndex = getRowItemIndex(row);
	var newEl = empty(detailId);
