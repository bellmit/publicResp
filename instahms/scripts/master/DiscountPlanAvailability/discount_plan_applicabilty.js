...
function saveForm(theForm) {
	...
	if (max_centers > 1) {
		...
		if (!checked) {
			alert("Please select discount plan applicability for centers");
			return false;
		}
...
function init() {
	...
	setUniqCenterId("discount_plan_center_id");
	if (max_centers > 1) {
		var app_for_centers = document.getElementsByName('applicable_for_centers');
		var checked = false;
		for (var i=0; i<app_for_centers.length; i++) {
...
function addToCenterTable() {
	...
	for (var i=0; i<center_chkboxes.length-1; i++) {
		...
		setHiddenValue(id, "discount_plan_center_id", "_");
...
function isDuplicate(type, id) {
	var els = document.getElementsByName(type == 'S' ? 'discount_plan_id' : 'center_id');
