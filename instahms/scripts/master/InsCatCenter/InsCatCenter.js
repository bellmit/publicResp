...
function saveForm() {
	...
	if (max_centers > 1) {
		...
		if (!checked) {
			alert("Please select the insurance category applicability for centers");
			return false;
		}
	...
	document.inscat_center_association_form.submit();
	return true;
}
...
function addToCenterTable() {
	...
	for (var i=0; i<center_chkboxes.length-1; i++) {
		...
		setHiddenValue(id, "inscat_center_id", "_");
...
function isDuplicate(type, id) {
	var els = document.getElementsByName(type == 'S' ? 'category_id' : 'center_id');
	var delete_el = document.getElementsByName(type == 'S' ? 'category_delete' : 'cntr_delete');
...
function setCenterRowStyle(i) {
	...
	var packCenterId = getIndexedValue("inscat_center_id", i);
...
function cancelCenter(obj) {
	...
	var isNew = getIndexedValue("inscat_center_id", id) == '_';
...
function getIndexedPaise(name, index) {
	return getElementPaise(getIndexedFormElement(document.inscat_center_association_form, name, index));
}

function setIndexedValue(name, index, value) {
	var obj = getIndexedFormElement(document.inscat_center_association_form, name, index);
	if (obj)
		obj.value = value;
	return obj;
}
...
function setHiddenValue(index, name, value) {
	var el = getIndexedFormElement(document.inscat_center_association_form, name, index);
	if (el) {
