...
function toggleCenterAddIcon(enableAdd) {
	...
	if(enableAdd) {
		document.getElementById('enabledisableforclaimformat').style.display = "none";
	}
		document.getElementById('centerAddIconDisabled').style.display = !enableAdd ? 'block' : 'none';
	if(!enableAdd) {
		document.getElementById('enabledisableforclaimformat').style.display = "table-row";
	}
...
function saveForm() {
	...
	if (max_centers > 1) {
		...
		if (!checked) {
			alert("Please select the tpa association for centers");
			return false;
		}
	...
	document.tpa_center_association_form.submit();
	return true;
}
...
var centerIndex = 0;
var CNTR_STATE_NAME = centerIndex++, CNTR_CITY_NAME = centerIndex++, CENTER_NAME = centerIndex++,CLAIM_FORMAT = centerIndex++
...
function addToCenterTable() {
	...
	var claim_format = document.getElementsByName('d_claim_format');
	for (var i=0; i<center_chkboxes.length-1; i++) {
		...
		setNodeText(row.cells[CLAIM_FORMAT], claim_format[i].value);
		setNodeText(row.cells[CENTER_STATUS], 'Active');

		setHiddenValue(id, "tpa_center_id", "_");
		...
		setHiddenValue(id, "claim_format", claim_format[i].value);
...
function showEditCenterDialog(obj) {
	...
	document.getElementById('ed_claim_format').value = getIndexedValue('claim_format', id);
...
function editCenterRow() {
	...
	var claim_format = document.getElementById('ed_claim_format').value;
	setNodeText(row.cells[CENTER_STATUS], status == 'A' ? 'Active' : 'Inactive');
	setNodeText(row.cells[CLAIM_FORMAT],claim_format);
	
	setHiddenValue(id, "center_status", status);
	setHiddenValue(id,"claim_format",claim_format);
...
function setCenterRowStyle(i) {
	...
	var packCenterId = getIndexedValue("tpa_center_id", i);
...
function cancelCenter(obj) {
	...
	var isNew = getIndexedValue("tpa_center_id", id) == '_';
...
function getIndexedPaise(name, index) {
	return getElementPaise(getIndexedFormElement(document.tpa_center_association_form, name, index));
}

function setIndexedValue(name, index, value) {
	var obj = getIndexedFormElement(document.tpa_center_association_form, name, index);
	if (obj)
		obj.value = value;
	return obj;
}
...
function setHiddenValue(index, name, value) {
	var el = getIndexedFormElement(document.tpa_center_association_form, name, index);
	if (el) {
