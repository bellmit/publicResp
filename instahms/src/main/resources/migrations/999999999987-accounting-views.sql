...
-- changeset adityabhatia02:accounting-views.sql splitStatements:false runAlways:true
-- validCheckSum: ANY
...
SELECT b.bill_no, b.finalized_date, date(bc.posted_date) as voucher_date, b.is_tpa, tm.tpa_name as primary_sponsor_name,
	...
	sid.cust_item_code,pdocdept.dept_name as prescribing_doctor_dept_name, b.status as bill_status
...
DROP VIEW IF EXISTS acc_all_sponsor_to_cash_sponsor_posting_charges CASCADE;
CREATE VIEW acc_all_sponsor_to_cash_sponsor_posting_charges AS
-- Join bill charge adjustments with the inner window query to pull out all records where the sponsor = prev sponsor
-- but the adjustment id prior to the sponsor switch. This represents all adjustments that happened against the old
-- sponsor. We also select the new sponsor, simultaneously so that we can use the same view for posting a contra against
-- the old sponsor and an adjustment against the new one.
SELECT bca.charge_id, bca.bill_charge_adjustment_id, fbca.primary_sponsor_id as primary_sponsor_id, fbca.prev_sponsor_id as new_sponsor_id,
bca.posted_date as new_posted_date, bca.mod_time as new_mod_time
FROM bill_charge_adjustment bca JOIN
	(SELECT * from
-- select all bill charge adjustment records grouped by charge id and ordered by adjustment id within the group
-- along with the previous sponsor id and the next sponsor id and filter all the records where previous sponsor was
...
DROP VIEW IF EXISTS acc_all_sponsor_to_cash_sponsor_reversal_charges CASCADE;
CREATE VIEW acc_all_sponsor_to_cash_sponsor_reversal_charges AS
-- Join bill charge adjustments with the inner window query to pull out all records where the sponsor = prev sponsor
-- but the adjustment id prior to the sponsor switch. This represents all adjustments that happened against the old
-- sponsor. We also select the new sponsor, simultaneously so that we can use the same view for posting a contra against
-- the old sponsor and an adjustment against the new one.
...
DROP VIEW IF EXISTS acc_all_sponsor_to_cash_posting_charges CASCADE;
CREATE VIEW acc_all_sponsor_to_cash_posting_charges AS
-- Join bill charge adjustments with the inner window query to pull out all records where the sponsor = prev sponsor
-- but the adjustment id prior to the sponsor switch. This represents all adjustments that happened against the old
-- sponsor. We also select the new sponsor, simultaneously so that we can use the same view for posting a contra against
-- the old sponsor and an adjustment against the new one.
...
''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
null::int as job_transaction,now()::timestamp created_at,''::text as sale_bill_no, ''::text as charge_reference_id, ''::text as primary_id,''::text as secondary_id,
''::text as primary_id_reference_table,null::date as bill_last_finalized_date, ''::text as patient_name,''::text as ha_item_code,''::text as ha_code_type,
apvv.voucher_sub_type as voucher_sub_type
...
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	...
	pmd.cust_item_code as cust_item_code,''::text as prescribing_doctor_dept_name,''::text as custom_8, ''::text as custom_9,
	''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,null::int as job_transaction,now()::timestamp created_at, 
	''::text as sale_bill_no, ''::text as charge_reference_id, ''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
	null::date as bill_last_finalized_date, ''::text as patient_name,sic.item_code as ha_item_code,hic.code_type as ha_code_type
FROM store_invoice i
	...
	LEFT JOIN ha_item_code_type hic ON (hcm.health_authority= hic.health_authority and pmd.medicine_id = hic.medicine_id )
	LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and pmd.medicine_id = sic.medicine_id)
...
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	...
	''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
	i.invoice_no||'/'||gm.grn_no as voucher_no, 'PURCHASE'::text as voucher_type, i.invoice_date as voucher_date,
	''::text as item_code, '' as item_name, s.dept_name as receipt_store, ''::text as issue_store,
	''::text as currency, 0.00 as currency_conversion_rate,
	0 as quantity, 0 as unit, 0 as unit_rate,
	i.tcs_amount as gross_amount, 0.00 as round_off_amount, 0 as discount_amount,
	0 as points_redeemed, 0.00 as points_redeemed_rate, 0.00 as points_redeemed_amt,
	0 as item_category_id, 0.00 as purchase_vat_amount, 0.00 as purchase_vat_percent,
	0.00 as sales_vat_amount, 0.00 as sales_vat_percent,
	'TCS Paid Supplier A/C'::text as debit_account, sup.supplier_name as credit_account,
	0.00 as tax_amount, i.tcs_amount as net_amount,
	''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
	''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
	''::text as admitting_department, ''::text as conducting_department, 0.00 as cost_amount, ''::text as supplier_name,
	i.invoice_no as invoice_no, i.invoice_date as invoice_date, gm.grn_no::text as voucher_ref, ''::text as remarks, i.date_time as mod_time, ''::text as counter_no,
	...
	''::text as insurance_co, ''::text as old_mr_no, ''::text as issue_store_center, ''::text as receipt_store_center,
	i.po_no::text as po_number, p.po_date::date as po_date, 'N'::character as transaction_type,
	i.tcs_amount as transaction_amount, ''::text as sponsor_name, ''::text as sponsor_type, 
	'TCS'::text as account_name, sup.cust_supplier_code as cust_supplier_code,gm.grn_date as grn_date,
	'' as cust_item_code,''::text as prescribing_doctor_dept_name,''::text as custom_8, ''::text as custom_9,''::text as custom_10, 
	''::text as custom_11,''::text as guid,0 as update_status,null::int as job_transaction,now()::timestamp created_at, 
	''::text as sale_bill_no, ''::text as charge_reference_id, ''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
	null::date as bill_last_finalized_date, ''::text as patient_name,''::text as ha_item_code,''::text as ha_code_type
FROM store_invoice i
	JOIN store_grn_main gm USING (supplier_invoice_id)
	JOIN stores s on s.dept_id=gm.store_id
	JOIN hospital_center_master hcm ON s.center_id=hcm.center_id
	JOIN supplier_master sup ON (i.supplier_id = sup.supplier_code)
	LEFT JOIN store_po_main p ON (p.po_no = i.po_no) 
WHERE i.consignment_stock=false and i.status!='O' and i.tcs_amount>0

UNION ALL
-- Supplier invoice discount post as different voucher
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	''::text as charge_group, ''::text as charge_head, s.account_group,
	''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
	...
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
	null::date as bill_last_finalized_date, ''::text as patient_name,''::text as ha_item_code,''::text as ha_code_type
FROM store_invoice i
	JOIN store_grn_main gm USING (supplier_invoice_id)
	--JOIN store_grn_details g USING (grn_no)
	...
	--JOIN store_category_master icm ON (pmd.med_category_id=icm.category_id)
	LEFT JOIN store_po_main p ON (p.po_no = i.po_no)
WHERE i.consignment_stock=false and i.status!='O' AND i.discount > 0

UNION ALL
-- Round off from Supplier invoices 
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	''::text as charge_group, ''::text as charge_head, s.account_group,
	''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
	i.invoice_no||'/'||gm.grn_no as voucher_no, 'PURCHASE'::text as voucher_type, i.invoice_date as voucher_date,
	...
	''::text as currency, 0.00 as currency_conversion_rate,
	0 as quantity, 0 as unit, 0.00 as unit_rate,
	ABS(i.round_off) as gross_amount, 0.00 as round_off_amount, 0.00 as discount_amount,
	0 as points_redeemed, 0.00 as points_redeemed_rate, 0.00 as points_redeemed_amt,
	0 as item_category_id, 0.00 as purchase_vat_amount, 0 as purchase_vat_percent,
	0.00 as sales_vat_amount, 0.00 as sales_vat_percent,
	CASE WHEN i.round_off < 0 THEN sup.supplier_name ELSE 'Purchase Roundoff A/C'::text END as debit_account,
	CASE WHEN i.round_off > 0 THEN sup.supplier_name ELSE 'Purchase Roundoff A/C'::text END as credit_account,
	0 as tax_amount, ABS(i.round_off) as net_amount,
	''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
	''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
	''::text as admitting_department, ''::text as conducting_department, 0.00 as cost_amount, ''::text as supplier_name,
	i.invoice_no as invoice_no, i.invoice_date as invoice_date, gm.grn_no::text as voucher_ref, ''::text as remarks, i.date_time as mod_time, ''::text as counter_no,
	...
	''::text as insurance_co, ''::text as old_mr_no, ''::text as issue_store_center, ''::text as receipt_store_center,
	i.po_no::text as po_number, p.po_date::date as po_date, 'N'::character as transaction_type,
	ABS(i.round_off) as transaction_amount, ''::text as sponsor_name, ''::text as sponsor_type, ''::text as account_name,
	sup.cust_supplier_code as cust_supplier_code,gm.grn_date,''::text as cust_item_code,''::text as prescribing_doctor_dept_name,
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
...
FROM store_invoice i
	...
	JOIN supplier_master sup ON (i.supplier_id = sup.supplier_code)
	LEFT JOIN store_po_main p ON (p.po_no = i.po_no)
WHERE i.consignment_stock=false and i.status!='O' AND i.round_off != 0

UNION ALL
-- Supplier  discount post as different voucher
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	''::text as charge_group, ''::text as charge_head, s.account_group,
	''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
	i.invoice_no||'/'||gm.grn_no as voucher_no, 'PURCHASE'::text as voucher_type, i.invoice_date as voucher_date,
	...
	''::text as currency, 0.00 as currency_conversion_rate,
	0 as quantity, 0 as unit, 0.00 as unit_rate,
	g.discount as gross_amount, 0.00 as round_off_amount, 0.00 as discount_amount,
	0 as points_redeemed, 0.00 as points_redeemed_rate, 0.00 as points_redeemed_amt,
	0 as item_category_id, 0.00 as purchase_vat_amount, 0 as purchase_vat_percent,
	0.00 as sales_vat_amount, 0.00 as sales_vat_percent,
	sup.supplier_name as debit_account,'Discount A/C'::text  as credit_account,
	0 as tax_amount, g.discount as net_amount,
	''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
	''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
	''::text as admitting_department, ''::text as conducting_department, 0.00 as cost_amount, ''::text as supplier_name,
	i.invoice_no as invoice_no, i.invoice_date as invoice_date, gm.grn_no::text as voucher_ref, ''::text as remarks, i.date_time as mod_time, ''::text as counter_no,
	...
	''::text as insurance_co, ''::text as old_mr_no, ''::text as issue_store_center, ''::text as receipt_store_center,
	i.po_no::text as po_number, p.po_date::date as po_date, 'N'::character as transaction_type,
	g.discount as transaction_amount, ''::text as sponsor_name, ''::text as sponsor_type, ''::text as account_name,
	sup.cust_supplier_code as cust_supplier_code,gm.grn_date,''::text as cust_item_code,''::text as prescribing_doctor_dept_name,
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
...
FROM store_invoice i
	...
	LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and pmd.medicine_id = sic.medicine_id)
	LEFT JOIN store_po_main p ON (p.po_no = i.po_no)
WHERE i.consignment_stock=false and i.status!='O' AND g.discount > 0

UNION ALL 
-- Other charges for Supplier invoices 
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
''::text as charge_group, ''::text as charge_head, s.account_group,
''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
i.invoice_no||'/'||gm.grn_no as voucher_no, 'PURCHASE'::text as voucher_type, i.invoice_date as voucher_date,
...
''::text as currency, 0.00 as currency_conversion_rate,
0 as quantity, 0 as unit, 0.00 as unit_rate,
(i.other_charges + i.transportation_charges) as gross_amount, 0.00 as round_off_amount, 0.00 as discount_amount,
0 as points_redeemed, 0.00 as points_redeemed_rate, 0.00 as points_redeemed_amt,
0 as item_category_id, 0.00 as purchase_vat_amount, 0 as purchase_vat_percent,
0.00 as sales_vat_amount, 0.00 as sales_vat_percent,
'Other Charges A/C'::text as debit_account, sup.supplier_name as credit_account,
0.00 as tax_amount, (i.other_charges + i.transportation_charges) as net_amount,
''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
''::text as admitting_department, ''::text as conducting_department, 0.00 as cost_amount, ''::text as supplier_name,
i.invoice_no as invoice_no, i.invoice_date as invoice_date, gm.grn_no::text as voucher_ref, ''::text as remarks, i.date_time as mod_time, ''::text as counter_no, 
...
''::text as insurance_co, ''::text as old_mr_no, ''::text as issue_store_center, ''::text as receipt_store_center,
i.po_no::text as po_number, p.po_date::date as po_date, 'N'::character as transaction_type,
(i.other_charges + i.transportation_charges) as transaction_amount, ''::text as sponsor_name, ''::text as sponsor_type, ''::text as account_name,
sup.cust_supplier_code as cust_supplier_code,gm.grn_date, ''::text as cust_item_code,''::text as prescribing_doctor_dept_name,
''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,
...
JOIN supplier_master sup ON (i.supplier_id = sup.supplier_code)
LEFT JOIN store_po_main p ON (p.po_no = i.po_no) 
WHERE  i.consignment_stock=false and i.status!='O' AND (i.other_charges + i.transportation_charges) >0) as foo
...
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	...
	''::text as currency, 0.00 as currency_conversion_rate,
	g.billed_qty as quantity, g.grn_pkg_size as unit, g.cost_price as unit_rate,
	abs(trunc((g.cost_price*g.billed_qty/g.grn_pkg_size),2)) as gross_amount, sdn.round_off as round_off_amount,
	...
	0.00 as tax_amount, abs(trunc((g.cost_price*g.billed_qty/g.grn_pkg_size) - (g.discount+g.scheme_discount), 2)) as net_amount,
	''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
	''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
	...
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,null::date as bill_last_finalized_date, 
	''::text as patient_name,sic.item_code as ha_item_code, hic.code_type as ha_code_type
FROM store_debit_note sdn
	...
	LEFT JOIN ha_item_code_type hic ON (hcm.health_authority= hic.health_authority and sid.medicine_id = hic.medicine_id )
	LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and sid.medicine_id = sic.medicine_id)
WHERE sdn.status='C'
...
--	AND bill_type in ('P', 'C') AND restriction_type in ('N') AND b.is_tpa != true AND bc.insurance_claim_amount = 0
--
--UNION ALL
...
---- Sponsor A -> Sponsor B Switch, Reverse Sponsor A
...
--        WHEN bc.insurance_claim_amount <= 0::numeric THEN bc.insurance_claim_amount
--        ELSE 0.00 - bc.insurance_claim_amount
--    END as transaction_amount, tpa.tpa_name as sponsor_name, tpa.sponsor_type,
...
--
--UNION ALL
---- Sponsor A -> Sponsor B Switch, Post to Sponsor B
--SELECT hcm.center_id, hcm.center_name, b.visit_type, pr.mr_no, b.visit_id, cg.chargegroup_name AS charge_group,
--	cc.chargehead_name AS charge_head, b.account_group, sg.service_group_name AS service_group,
-- 	ssg.service_sub_group_name AS service_sub_group, b.bill_no, b.audit_control_number, b.bill_no AS voucher_no,
--	'HOSPBILLS'::text AS voucher_type, date(v.new_posted_date) AS voucher_date, mbc.act_description_id::text AS item_code,
...
--
--UNION ALL
---- Ins to Cash conversion Insurance Reversal
--SELECT hcm.center_id, hcm.center_name, b.visit_type, pr.mr_no, b.visit_id, cg.chargegroup_name AS charge_group,
--	cc.chargehead_name AS charge_head, b.account_group, sg.service_group_name AS service_group,
--	ssg.service_sub_group_name AS service_sub_group, b.bill_no, b.audit_control_number, b.bill_no AS voucher_no,
--	'HOSPBILLS'::text AS voucher_type, date(v.new_posted_date) AS voucher_date, mbc.act_description_id::text AS item_code,
...
--	''::text as cust_item_code,pdocdept.dept_name as prescribing_doctor_dept_name
--FROM bill_charge_adjustment bc
--	JOIN acc_all_sponsor_to_cash_sponsor_reversal_charges v ON (v.bill_charge_adjustment_id = bc.bill_charge_adjustment_id and v.charge_id = bc.charge_id)
--	JOIN bill_charge mbc ON mbc.charge_id::text = bc.charge_id::text
--	JOIN chargehead_constants cc ON cc.chargehead_id::text = bc.charge_head::text
--	JOIN chargegroup_constants cg ON cg.chargegroup_id::text = bc.charge_group::text
--	JOIN bill_account_heads bah ON cc.account_head_id = bah.account_head_id
...
--WHERE (bc.charge_head NOT IN ('PHMED', 'PHRET', 'PHCMED', 'PHCRET', 'INVITE', 'INVRET'))
--	AND (b.bill_type = ANY (ARRAY['P'::bpchar, 'C'::bpchar]))
--	AND b.restriction_type = 'N'::bpchar AND b.is_tpa = false AND bc.insurance_claim_amount <> 0.00
--
--UNION ALL
---- Ins -> Cash conversion Insurance Posting for already converted bill (If export was not already done)
--SELECT hcm.center_id, hcm.center_name, b.visit_type, pr.mr_no, b.visit_id, cg.chargegroup_name AS charge_group,
--	cc.chargehead_name AS charge_head, b.account_group, sg.service_group_name AS service_group,
-- 	ssg.service_sub_group_name AS service_sub_group, b.bill_no, b.audit_control_number, b.bill_no AS voucher_no,
--	'HOSPBILLS'::text AS voucher_type, date(v.new_posted_date) AS voucher_date, mbc.act_description_id::text AS item_code,
...
--	''::text as cust_item_code,pdocdept.dept_name as prescribing_doctor_dept_name
--FROM bill_charge_adjustment bc
--	JOIN acc_all_sponsor_to_cash_sponsor_posting_charges v ON (v.bill_charge_adjustment_id = bc.bill_charge_adjustment_id and v.charge_id = bc.charge_id)
--	JOIN bill_charge mbc ON mbc.charge_id::text = bc.charge_id::text
--	JOIN chargehead_constants cc ON cc.chargehead_id::text = bc.charge_head::text
--	JOIN chargegroup_constants cg ON cg.chargegroup_id::text = bc.charge_group::text
--	JOIN bill_account_heads bah ON cc.account_head_id = bah.account_head_id
...
--
--UNION ALL
---- Ins -> Cash conversion Cash Posting for already converted bill (If export was not already done)
--SELECT hcm.center_id as center_id, hcm.center_name as center_name, b.visit_type, pr.mr_no, b.visit_id,
--	cg.chargegroup_name as charge_group, cc.chargehead_name as charge_head, b.account_group,
--	sg.service_group_name as service_group, ssg.service_sub_group_name as service_sub_group, b.bill_no, b.audit_control_number,
--	b.bill_no as voucher_no, 'HOSPBILLS'::text as voucher_type, date(bc.posted_date) as voucher_date,
...
--	LEFT JOIN doctors cdoc ON (mbc.payee_doctor_id = cdoc.doctor_id)
--WHERE bc.charge_head not in ('PHMED','PHRET','PHCMED','PHCRET','INVITE', 'INVRET')
--	AND bill_type in ('P', 'C') AND restriction_type in ('N') AND b.is_tpa = false
--	AND bc.insurance_claim_amount <> bc.amount AND bc.insurance_claim_amount <> 0.00
--ORDER BY bill_no;
...
-- All store transfers
SELECT fhcm.center_id, fhcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
...
WHERE std.qty - std.qty_rejected != 0;
...
--  Accounting for stock adjustment
DROP VIEW IF EXISTS acc_all_stock_adjustment_view CASCADE ;
CREATE VIEW acc_all_stock_adjustment_view AS
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	''::text as charge_group, ''::text as charge_head, s.account_group,
	''::text as service_group, ''::text as service_sub_group, ''::text as bill_no, ''::text as audit_control_number,
	sam.adj_no as voucher_no, 'STOCKADJUSTMENT'::text as voucher_type,sam.date_time as voucher_date,
    sid.medicine_id::text as item_code, sid.medicine_name as item_name, ''::text as receipt_store, sam.store_id as issue_store,
	''::text as currency, 0.00 as currency_conversion_rate,
	sad.qty as quantity, ''::text as unit, 0.00 as unit_rate,
	sad.cost_value as gross_amount, 0.00 as round_off_amount,
	0.00 as discount_amount, 0 as points_redeemed, 0.00 as points_redeemed_rate, 0.00 as points_redeemed_amt,scm.category_id as item_category_id, 0.00 as purchase_vat_amount, 0.00 as purchase_vat_percent,
	0.00 as sales_vat_amount, 0.00 as sales_vat_percent,
	CASE WHEN sad.type = 'A' THEN 'Inventory A/C' ELSE 'Stock Adjustment A/C' END as debit_account,
	CASE WHEN sad.type = 'A' THEN 'Stock Adjustment A/C' ELSE 'Inventory A/C' END as credit_account,
	0.00 as tax_amount, sad.cost_value as net_amount,
	''::text as admitting_doctor, ''::text as prescribing_doctor, ''::text as conductiong_doctor,
	''::text as referral_doctor, ''::text as payee_doctor, ''::text as outhouse_name, ''::text as incoimng_hospital,
	''::text as admitting_department, ''::text as conducting_department, sad.cost_value as cost_amount, ''::text as supplier_name,
	''::text as invoice_no, NULL::timestamp as invoice_date, ''::text as voucher_ref, ssd.remarks_avbl as remarks, sam.date_time as mod_time,
	''::text as counter_no, null::date as bill_open_date, null::date as bill_finalized_date, ''::text as is_tpa,
	''::text as insurance_co, ''::text as old_mr_no, hcm.center_name as issue_store_center, ''::text as receipt_store_center,
	''::text as po_number, null::date as po_date, 'N'::character as transaction_type,
	CASE WHEN sad.type = 'R' THEN (sad.cost_value * -1)  ELSE sad.cost_value END as custom_1, ''::text as custom_2, ''::text as custom_3, ''::text as custom_4,
	''::text as cust_supplier_code,NULL::timestamp as grn_date,sid.cust_item_code as cust_item_code,''::text as prescribing_doctor_dept_name,
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,null::date as bill_last_finalized_date,
	''::text as patient_name,sic.item_code as ha_item_code,hic.code_type as ha_code_type,CASE WHEN sad.type ='A' THEN 'ADJUSTMENT_INCREASE' 
	ELSE 'ADJUSTMENT_DECREASE' END as voucher_sub_type	 
	FROM store_adj_details sad
JOIN store_adj_main sam on (sad.adj_no = sam.adj_no)
JOIN store_item_batch_details sibd ON (sibd.item_batch_id=sad.item_batch_id)
JOIN store_item_details sid ON (sid.medicine_id=sibd.medicine_id)
JOIN store_category_master scm ON (sid.med_category_id=scm.category_id)
JOIN stores s ON (sam.store_id=s.dept_id)
JOIN hospital_center_master hcm on (s.center_id = hcm.center_id)
LEFT JOIN ha_item_code_type hic ON (hcm.health_authority= hic.health_authority and sid.medicine_id = hic.medicine_id )
LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and sid.medicine_id = sic.medicine_id)
LEFT JOIN store_transaction_lot_details stld ON(stld.transaction_id = sad.adj_detail_no)
LEFT JOIN store_stock_details ssd ON (ssd.item_lot_id=stld.item_lot_id and ssd.dept_id = sam.store_id)
WHERE sad.qty != 0;
...
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	...
	''::text as custom_8, ''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, 
	''::text as primary_id, ''::text as secondary_id, ''::text as primary_id_reference_table,null::date as bill_last_finalized_date, ''::text as patient_name,
	sic.item_code as ha_item_code,hic.code_type as ha_code_type
...
FROM store_invoice i
	...
	LEFT JOIN ha_item_code_type hic ON (hcm.health_authority= hic.health_authority and pmd.medicine_id = hic.medicine_id )
	LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and pmd.medicine_id = sic.medicine_id)
WHERE i.consignment_stock=false and i.status!='O' AND COALESCE(gt.tax_amt, g.tax, 0) != 0;
...
FROM store_consignment_invoice sci
	...
	LEFT JOIN store_po_main p ON (p.po_no = si.po_no)
	LEFT JOIN hospital_center_master hcm ON (hcm.center_id = s.center_id)
WHERE si.consignment_stock=true AND COALESCE(gt.tax_amt, g.tax, 0) != 0;
...
SELECT hcm.center_id, hcm.center_name, ''::text as visit_type, ''::text as mr_no, ''::text as visit_id,
	...
	sid.cust_item_code as cust_item_code,''::text as prescribing_doctor_dept_name, ''::text as custom_8, 
	''::text as custom_9,''::text as custom_10, ''::text as custom_11,''::text as guid,0 as update_status,
	null::int as job_transaction,now()::timestamp created_at, ''::text as sale_bill_no, ''::text as charge_reference_id, ''::text as primary_id, ''::text as secondary_id,
	''::text as primary_id_reference_table,null::date as bill_last_finalized_date, ''::text as patient_name,sic.item_code as ha_item_code,hic.code_type as ha_code_type
FROM store_debit_note sdn
	JOIN store_grn_main sgm ON (sgm.debit_note_no = sdn.debit_note_no)
	JOIN store_grn_details g ON (g.grn_no=sgm.grn_no)
	JOIN store_item_details sid ON (sid.medicine_id=g.medicine_id)
	...
	LEFT JOIN ha_item_code_type hic ON (hcm.health_authority= hic.health_authority and sid.medicine_id = hic.medicine_id )
	LEFT JOIN store_item_codes sic ON (sic.code_type = hic.code_type and sid.medicine_id = sic.medicine_id)
WHERE sdn.status='C' AND gt.tax_amt != 0
ORDER BY sdn.debit_note_no;
...
BEGIN
	set application.username='_system';
...
--acc_all_consolidate_sponsor_receipts_view
	insert into hms_accounting_info
	select * from acc_all_consolidate_sponsor_receipts_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_payment_vouchers_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,currency_conversion_rate,quantity,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,payee_doctor,outhouse_name,cost_amount,supplier_name,voucher_ref,mod_time,counter_no,transaction_type,custom_1,cust_supplier_code,ha_item_code,ha_code_type,voucher_sub_type) 
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,currency_conversion_rate,quantity,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,payee_doctor,outhouse_name,cost_amount,supplier_name,voucher_ref,mod_time,counter_no,transaction_type,transaction_amount,cust_supplier_code,ha_item_code,ha_code_type,voucher_sub_type  from acc_all_payment_vouchers_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_payments_due_view
	insert into hms_accounting_info
	select * from acc_all_payments_due_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_invoices_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,custom_1,custom_4,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,transaction_amount,account_name,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type  from acc_all_invoices_view where mod_time > start_time AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;


--acc_all_returns_with_debitnote_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,custom_1,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,transaction_amount,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type from acc_all_returns_with_debitnote_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_cs_issued_view
	insert into hms_accounting_info
...
--acc_all_stock_transfers_view
	insert into hms_accounting_info
	select * from acc_all_stock_transfers_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_reagents_consumption_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,mod_time,issue_store_center,transaction_type,custom_1,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,mod_time,issue_store_center,transaction_type,transaction_amount,cust_item_code,ha_item_code,ha_code_type from acc_all_reagents_consumption_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;
	
--acc_all_stock_adjustment_view
	insert into hms_accounting_info(center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,remarks,mod_time,issue_store_center,transaction_type,custom_1,cust_item_code,ha_item_code,ha_code_type,voucher_sub_type) 
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,remarks,mod_time,issue_store_center,transaction_type,custom_1,cust_item_code,ha_item_code,ha_code_type,voucher_sub_type from acc_all_stock_adjustment_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_remittance_receipts_view
	insert into hms_accounting_info
	select * from acc_all_remittance_receipts_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_user_issues_returns_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,mod_time,issue_store_center,transaction_type,custom_1,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,mod_time,issue_store_center,transaction_type,transaction_amount,cust_item_code,ha_item_code,ha_code_type  from acc_all_user_issues_returns_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;
...
--acc_all_insurance_limits_adjustments_view
	insert into hms_accounting_info
...
--acc_all_invoices_tax_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,custom_1,custom_4,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,transaction_amount,account_name,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type from acc_all_invoices_tax_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_cs_issued_tax_view
	insert into hms_accounting_info
	select * from acc_all_cs_issued_tax_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;

--acc_all_returns_with_debitnote_tax_view
	insert into hms_accounting_info (center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,custom_1,custom_4,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type)
	select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,transaction_amount,account_name,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type  from acc_all_returns_with_debitnote_tax_view where mod_time > start_time
	AND mod_time <= end_time AND credit_account IS NOT null AND debit_account IS NOT null;
...
BEGIN
	...
	SELECT max(mod_time) FROM hms_accounting_info where job_transaction is null OR job_transaction=-1
		INTO start_time;
...
BEGIN
	...
	IF recs IS NULL OR recs = 0 THEN
		...
		insert into hms_accounting_info
		select * from acc_all_consolidate_sponsor_receipts_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;
		...
		insert into hms_accounting_info
		select * from acc_all_payments_due_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info(center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,custom_1,custom_4,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type)
		select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,transaction_amount,account_name,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type  from acc_all_invoices_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info(center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,custom_1,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type)
		select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,transaction_amount,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type from acc_all_returns_with_debitnote_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info
		select * from acc_all_cs_issued_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_hospital_bills_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_incoming_sample_bills_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_pharmacy_bills_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_hospital_pharma_bills_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info
		select * from acc_all_stock_transfers_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;
		...
		insert into hms_accounting_info
		select * from acc_all_remittance_receipts_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;
...
--		insert into hms_accounting_info
--		select * from acc_all_insuarnce_adjustments_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info
		select * from acc_all_insurance_limits_adjustments_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_pharmacy_bills_tax_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

--		insert into hms_accounting_info
--		select * from acc_all_hospital_pharma_bills_tax_view where
--		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info(center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,custom_1,custom_4,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type)
		select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,receipt_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,invoice_no,invoice_date,voucher_ref,mod_time,po_number,po_date,transaction_type,transaction_amount,account_name,cust_supplier_code,grn_date,cust_item_code,ha_item_code,ha_code_type  from acc_all_invoices_tax_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info
		select * from acc_all_cs_issued_tax_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;

		insert into hms_accounting_info(center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amount,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,custom_1,custom_4,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type)
		select center_id,center_name,account_group,voucher_no,voucher_type,voucher_date,item_code,item_name,issue_store,currency_conversion_rate,quantity,unit,unit_rate,gross_amount,round_off_amount,discount_amount,points_redeemed,points_redeemed_rate,points_redeemed_amt,item_category_id,purchase_vat_amount,purchase_vat_percent,sales_vat_amount,sales_vat_percent,debit_account,credit_account,tax_amount,net_amount,cost_amount,supplier_name,remarks,mod_time,issue_store_center,transaction_type,transaction_amount,account_name,cust_supplier_code,cust_item_code,ha_item_code,ha_code_type  from acc_all_returns_with_debitnote_tax_view where
		mod_time <= end_time  AND credit_account is not null AND debit_account is not null;
