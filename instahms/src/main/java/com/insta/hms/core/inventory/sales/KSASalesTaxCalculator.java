...
@Component
public class KSASalesTaxCalculator extends SalesTaxCalculator {

	private static final String[] SUPPORTED_GROUPS = new String[]{"KSACEX"};
	public KSASalesTaxCalculator() {
	...
	protected BigDecimal applyTaxRate(ItemTaxDetails itemTaxDetails, BigDecimal taxRate, TaxContext taxContext) {
		...
		if(null != billBean && !billBean.getMap().isEmpty() && (boolean)billBean.get("is_tpa") && 
				null != tpaBean && !tpaBean.getMap().isEmpty() && (boolean)tpaBean.get("claim_amount_includes_tax").equals("N")){
			// Kludge : We will subtract the net amount since the net tax amount in this case is 0, but the call to the super
