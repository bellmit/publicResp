...
@Component
public class ServiceAppointmentCategory implements AppointmentCategory {
  ...
  private static final String GET_APPOINTMENT_DETAILS = "SELECT ap.mr_no,ap.visit_id,"
      ...
      + " where doctor_id=ap.presc_doc_id) as presc_doctor, r.dept_id as dept_id,"
      ...
      + " as lang_code, "
      + " CASE WHEN apit.resource_type ='SRID' THEN (select serv_resource_name from "
      ...
      + " WHEN apit.resource_type ='SNP' THEN (select service_name from services "
      + "where service_id::text=apit.resource_id ) "
  ...
  private static final String category = ResourceCategory.SNP.name();

  private static final String PRIMARY_RESOURCE_TYPE = "SRID";

  private static final String SECONDARY_RESOURCE_TYPE = "SNP";
  ...
  private static final String SECONDARY_REOSURCE_MASTER_QUERY = " SELECT distinct "
      ...
      + " s.service_name as resource_name,'SNP'::text as resource_type, "
      + " '' as scheduler_resource_type, dept.serv_dept_id,dept.department"
      + " FROM services s "
      + " LEFT JOIN services_departments dept ON(dept.serv_dept_id=s.serv_dept_id)"
      + " WHERE s.status='A' #CENTER_FILTER# #RESOURCE_FILTER# #DEPT_FILTER#";

  private static final String PRIMARY_REOSURCE_MASTER_QUERY = " SELECT distinct "
      + "srm.serv_res_id::text as resource_id, srm.serv_resource_name as "
      + "resource_name,srm.overbook_limit,'SRID'::text as resource_type, "
      + " '' as scheduler_resource_type,dept.serv_dept_id,dept.department as dept_name"
      + " FROM service_resource_master srm "
      + " LEFT JOIN services_departments dept ON(dept.serv_dept_id=srm.serv_dept_id)"
      + " WHERE srm.schedule = true AND srm.status='A' #CENTER_FILTER# "
  ...
  public void setAppointmentAdditionalResourceData(Appointment appointment,
    ...
    AppointmentResource res = new AppointmentResource(appointment.getAppointmentId(), "SRID",
  ...
  public Integer getResourceOverbookLimit(String resourceId, String resType) {
    switch (resType) {
      ...
      case "SRID":
        return serviceResourcesService.getOverbookLimit(resourceId);
      default:
        return doctorService.getDoctorOverbookLimit(resourceId);
    }
  }
  ...
  public String getResourceName(String resourceId, String resType) {
    switch (resType) {
      case "DOC":
        ...
        return null;
      case "SRID":
        return serviceResourcesService.getEquipmentName(resourceId);
      default:
  ...
  public String getDeptFilterClause() {
    return "AND srm.serv_dept_id IN (:deptIds)";
  }

  @Override
  public String validatePrimRes() {
    return "select serv_res_id,center_id from service_resource_master where serv_res_id"
        + " =  ?::Integer AND status = 'A'";
  }

  @Override
  public String validateSecRes() {
    return "select service_id from services where service_id =  ? AND status = 'A'";
  }

  public static final String GET_DURATION_FROM_SEC_RES = "SELECT service_duration as "
      + "duration from services where service_id = ?";

  @Override
  public int getAppointmentDuration(String serviceId, String srId) {
    ...
    if (bean != null) {
      ...
      if (serviceId != null) {
        duration = getSlotDurationOfPrimRes(serviceId);
      } else {
        duration = -1;
      }
    }
  ...
  @Override
  public int getSlotDurationOfPrimRes(String srId) {
    ...
    if (bean != null) {
      ...
      bean = DatabaseHelper
          .queryToDynaBean(GET_DURATION_FROM_PRIM_RES, new Object[] { "*", "SRID" });
      duration = (int) bean.get("duration");
    }
    return duration;
  }

  public static final String GET_PRIM_RES = "SELECT distinct srm.serv_res_id::text as resource_id,"
      + " srm.serv_resource_name as resource_name, ssrm.service_id "
      + "FROM service_resource_master srm  "
      + "LEFT JOIN service_service_resources_mapping ssrm ON (srm.serv_res_id = ssrm.serv_res_id) "
      + "LEFT JOIN services_departments dept ON(dept.serv_dept_id=srm.serv_dept_id) "
      + "WHERE srm.schedule = true AND srm.status='A' AND dept.status = 'A' "
      + "AND dept.serv_dept_id = ? AND srm.center_id IN (0,?) ";

  public static final String GET_PRIM_RES_WITHOUT_DEPT = "SELECT distinct srm.serv_res_id::text "
      + "as resource_id, srm.serv_resource_name as resource_name, ssrm.service_id "
      + "FROM service_resource_master srm  "
      + "LEFT JOIN service_service_resources_mapping ssrm ON (srm.serv_res_id = ssrm.serv_res_id) "
      + "WHERE srm.schedule = true AND srm.status='A' AND srm.center_id IN (0,?) ";
  ...
  public List<Map> getPrimResApplicableForSecRes(String secResId, int centerId, String deptId) {
    List<Map> maplist = ConversionUtils.copyListDynaBeansToMap(DatabaseHelper.queryToDynaList(
        GET_PRIM_RES, new Object[] { Integer.parseInt(deptId), centerId }));
    List<Map> filteredBySecRes = new ArrayList<>();
    if (maplist.size() > 0) {
      for (Map m: maplist) {
        if (m.get("service_id") != null && m.get("service_id").equals(secResId)) {
