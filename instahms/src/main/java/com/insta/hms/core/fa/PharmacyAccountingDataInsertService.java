...
/**
 * The Class PharmacyAccountingDataInsertService. This class is useful for processing accounting for
 * pharmacy bills which are having restrictionType in N, T
 */
@Service
public class PharmacyAccountingDataInsertService {
  ...
  /** The pharmacy only accounting data insert service. */
  @Autowired
  ...
  /** The logger. */
  private static Logger logger = LoggerFactory.getLogger(PharmacyAccountingDataInsertService.class);

  /**
   * Process accounting for pharmacy bill.
   *
   ...
   */
  @Transactional
  public void processAccountingForPharmacyBill(String phBillNo, Integer jobTransaction,
      boolean reversalsOnly, Date createdAt) {
    postReversalsForPharmacyItems(phBillNo, jobTransaction, createdAt);
    insertAccountingDataForPharmacyItems(phBillNo, jobTransaction, createdAt);
  }

  /**
   * Insert accounting data for pharmacy items.
   *
   * @param phBillNo       the ph bill no
   * @param jobTransaction the job log id
   * @param createdAt      creation timestamp for voucher
   */
  public void insertAccountingDataForPharmacyItems(String phBillNo, Integer jobTransaction,
      Date createdAt) {
    StoreSalesMainModel storeSalesMainModel = pharmacyAccountingDataInsertRepo
        .getPharmacyBillModel(phBillNo);
    List<HmsAccountingInfoModel> accountingDataToInsert = getPharmacyAccountingDataToInsert(
        storeSalesMainModel, jobTransaction, createdAt);
    logger.info("Pharmacy Bill Accounting data insert started for bill : " + phBillNo);
    accountingDataInsertRepo.insertAccountingDataFromList(accountingDataToInsert);
    logger.info("Pharmacy Bill Accounting data insert successful for bill : " + phBillNo);
  }

  /**
   * Gets the pharmacy accounting data to insert.
   *
   * @param storeSalesMainModel the store sales main model
   * @param jobTransaction      the job log id
   * @param createdAt           creation timestamp for voucher
   * @return the pharmacy accounting data to insert
   */
  public List<HmsAccountingInfoModel> getPharmacyAccountingDataToInsert(
    ...
    String billType = String.valueOf(billModel.getBillType());
    String restrictionType = String.valueOf(billModel.getRestrictionType());

    if (!billStatus.equals("X") && (billType.equals("C") || billType.equals("P"))) {
      if (restrictionType.equals("N") || restrictionType.equals("T")) {
        return this.getAccountingDataToInsert(storeSalesMainModel, jobTransaction, createdAt);
      } else if (restrictionType.equals("P")) {
        return pharmacyOnlyAccountingDataInsertService
            .getAccountingDataToInsert(storeSalesMainModel, jobTransaction, createdAt);
      }
  ...
  private HmsAccountingInfoModel getPatientAmountAccountingEntry(StoreSalesDetailsModel saleItem,
    ...
    if (grossAmount.compareTo(BigDecimal.ZERO) != 0) {
      ...
      patientAmountEntry.setDebitAccount(getSaleItemAmount(saleItem).compareTo(BigDecimal.ZERO) >= 0
      ...
      patientAmountEntry
          .setCreditAccount(getSaleItemAmount(saleItem).compareTo(BigDecimal.ZERO) >= 0
              ? getSaleItemAccountHeadName(saleItem, storeSalesMainModel)
              : accountingDataInsertRepo.getLedgerDefinition("ACCOUNT_TYPE_COUNTER_RECEIPTS"));
   ...
   * @param storeSalesMainModel
   *          the store sales main model
   * @param claimAmountSumMap
   *          the claim amount sum map
  ...
  private List<HmsAccountingInfoModel> getInsurancePatientTaxAmountAccountingEntry(
    ...
    for (StoreSalesTaxDetailsModel saleItemTax : saleItemTaxes) {
      ...
      String itemSubGroupId = String.valueOf(saleItemTax.getItemSubGroupsModel() != null
          ? saleItemTax.getItemSubGroupsModel().getItemSubgroupId() : null);
...
discountAccountData
  ...
  /**
   * Post reversals for pharmacy items.
   *
   * @param phBillNo              the ph bill no
   * @param currentJobTransaction the current job log id
   */
  public void postReversalsForPharmacyItems(String phBillNo, Integer currentJobTransaction,
      Date createdAt) {
    ...
    logger.info("Accounting reversals process started for sales bill : " + phBillNo);
    Integer lastJobTransaction = pharmacyAccountingDataInsertRepo
        .getLastJobTransactionIdForReversalPosts(phBillNo);
    if (lastJobTransaction != null && phBillNo != null) {
      List<HmsAccountingInfoModel> reversalsDataList = new ArrayList<>();
      List<HmsAccountingInfoModel> lastDataForBill = pharmacyAccountingDataInsertRepo
          .getLastAccountingDataForBill(phBillNo, lastJobTransaction);
    ...
    logger.info("Accounting reversals process completed for sales bill : " + phBillNo);
    // Post the reversals for cron job data if data exist from cron job
    postReversalsForMigratedDataOfBill(phBillNo);
  }

  private void postReversalsForMigratedDataOfBill(String phBillNo) {
    StoreSalesMainModel salesMainModel = pharmacyAccountingDataInsertRepo
        .getPharmacyBillModel(phBillNo);
    String billNo = salesMainModel.getBillNo().getBillNo();
