 ...
 */
@Component
public class IGSTPurchaseTaxCalculator extends PurchaseTaxCalculator {
	
	@LazyAutowired
	TaxSubGroupService taxSubGroupService;

	private static final String[] SUPPORTED_TAX_GROUPS = new String[] {"IGST"};

	protected IGSTPurchaseTaxCalculator() {
		super(SUPPORTED_TAX_GROUPS);
	}
	...
	protected BigDecimal getBasePrice(ItemTaxDetails itemTaxDetails) {
		if (null != itemTaxDetails && null != itemTaxDetails.getCostPrice()) {
			return itemTaxDetails.getCostPrice();
		}
