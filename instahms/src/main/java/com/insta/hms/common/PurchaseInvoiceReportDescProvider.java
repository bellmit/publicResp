...
/**
 * The Class PurchaseInvoiceReportDescProvider.
 */
public class PurchaseInvoiceReportDescProvider extends StdReportDescJsonProvider {
  ...
  public StdReportDesc getReportDesc(String descName) throws Exception {
    ...
    qu.setDynamicFieldsPartitionedOn("rpiv.grn_nos");
    qu.setJoinTables(jt);
  ...
  public List<JoinTable> addTaxCustomJoinTables() {
    ...
    jt1.setExpression("ON (sgtd.grn_no = rpiv.grn_nos)");
    jt1.setDependsOn("rpiv");
    jt1.setType("left");
  ...
  private Map<String, Field> addTaxAmtCustomFields() throws SQLException {
    ...
    for (BasicDynaBean b : list) {
      ...
      field.setExpression("SUM(CASE WHEN isg.item_group_id = " + taxGroupId
          + " THEN sgtd.tax_amt ELSE 0 END) OVER (PARTITION BY rpiv.grn_nos)");
      field.setTable("isg");
