...
/**
 * The Class POPendingItemReportDescProvider.
 */
public class POPendingItemReportDescProvider extends StdReportDescJsonProvider {
  ...
  public StdReportDesc getReportDesc(String descName) throws Exception {
    ...
    qu.setDynamicFieldsPartitionedOn("po.po_no,po.medicine_id");
    qu.setJoinTables(jt);
  ...
  public List<JoinTable> addTaxCustomJoinTables() {
    ...
    jt1.setAlias("sptd");
    jt1.setName("store_po_tax_details");
    jt1.setExpression("ON (sptd.po_no = po.po_no AND sptd.medicine_id = po.medicine_id)");
    jt1.setDependsOn("po");
    jt1.setType("left");
    ...
    jt.setExpression("ON (isg.item_subgroup_id = sptd.item_subgroup_id)");
    jt.setDependsOn("sptd");
    jt.setType("left");
    jts.add(jt);
    return jts;
  }
  ...
  private Map<String, Field> addTaxAmtCustomFields() throws SQLException {
    ...
    for (BasicDynaBean b : list) {
      ...
      field.setExpression("SUM(CASE WHEN isg.item_group_id = " + taxGroupId
          + " THEN sptd.tax_amt ELSE 0 END) OVER (PARTITION BY po.po_no,po.medicine_id)");
      field.setTable("isg");
