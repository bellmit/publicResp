 ...
 * conditionally or unconditionally based on the <code>ignore</code> initialization parameter. By
 * default this is set to UTF-8.</li>
...
public class CharacterEncodingFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (null == clientLocale) {
      clientLocale = Locale.ENGLISH;
    }

    // Check if requested locale is supported by application else fallback to english
    java.util.ResourceBundle rb = java.util.ResourceBundle.getBundle("java.resources.application",
        clientLocale);

    if (!rb.getLocale().getLanguage().equals(clientLocale.getLanguage())) {
      clientLocale = Locale.ENGLISH;
    }

    String textDirection = "ltr";
    if (ComponentOrientation.getOrientation(clientLocale) == ComponentOrientation.RIGHT_TO_LEFT) {
      textDirection = "rtl";
    }

    request.setAttribute("language", clientLocale.getLanguage());
    request.setAttribute("textDirection", textDirection);
