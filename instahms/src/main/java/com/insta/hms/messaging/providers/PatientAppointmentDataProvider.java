...
/**
 * The Class PatientAppointmentDataProvider.
 */
public class PatientAppointmentDataProvider extends QueryDataProvider {

  /** The logger. */
  static Logger logger = LoggerFactory
      .getLogger(PatientAppointmentDataProvider.class);

  /** The this name. */
  private static String THIS_NAME = "Patient Appointments";
  ...
  /**
   * Instantiates a new patient appointment data provider.
   */
  public PatientAppointmentDataProvider() {
    super(THIS_NAME);
  ...
  public List<Map> getMessageDataList(MessageContext ctx) throws SQLException, ParseException {
    ...
    // This will be available only if the message type is sms_appointment_reminder

    Map configParams = ctx.getConfigParams();

    // add the status param as a filter if it exists

    if (null != configParams) {

      Map statusFilter = new HashMap();
      String configStatus = (String) configParams.get("status");

      if (null != configStatus) {
        statusFilter.put("status", new String[] { configStatus.toString() });
      }

      addCriteriaFilter(statusFilter);
