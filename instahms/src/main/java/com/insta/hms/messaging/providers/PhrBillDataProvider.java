...
public class PhrBillDataProvider extends QueryDataProvider {
  static Logger logger = LoggerFactory
      .getLogger(PhrBillDataProvider.class);
  private static String THIS_NAME = "PHR Bills ";
  private static final String selectFields = "SELECT * ";
  private static final String selectCount = "SELECT COUNT(*)";
  private static final String fromTables = "FROM (select b.bill_no, pr.visit_type ,"
      + " COALESCE(pr.patient_id, prc.customer_id) AS visit_id , "
      + " sm.salutation||' '||pd.patient_name||' '||coalesce(pd.middle_name,'')||' '||"
      + "coalesce(pd.last_name,'')  as patient_full_name, "
      + " to_char((pr.reg_date + pr.reg_time) AT TIME ZONE ("
      + " SELECT  current_setting('TIMEZONE')) AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"'"
      + " ) as visit_date, "
      + " coalesce(to_char(b.closed_date AT TIME ZONE (SELECT  current_setting('TIMEZONE')) "
      + " AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"'),"
      + " to_char(b.finalized_date AT TIME ZONE (SELECT  current_setting('TIMEZONE'))"
      + " AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"')) AS bill_date, "
      + " to_char(pdm.mod_time AT TIME ZONE (SELECT  current_setting('TIMEZONE'))"
      + " AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') AS mod_time, pd.patient_phone, "
      ...
      + " (select hospital_name from generic_preferences) as hospital_name  " + " from bill b "
      ...
      + " as ref_email_id FROM referral ORDER BY ref_name ) as ref"
      + " ON(ref.ref_id=pr.reference_docto_id) "
      + " LEFT JOIN doctors d on(d.doctor_id = pr.doctor) ) as foo ";

  public PhrBillDataProvider() {
    super(THIS_NAME);
  ...
  public List<Map> getMessageDataList(MessageContext ctx) throws SQLException, ParseException {
    ...
    if (null != eventData) {
      ...
      if (null != eventData.get("bill_no")) {
        String[] reportId = { eventData.get("bill_no").toString() };
        filter.put("bill_no", reportId);
        filter.put("bill_no@type", new String[] { "text" });
        filter.put("bill_no@cast", new String[] { "y" });
        filter.put("bill_no@op", new String[] { "in" });
      }
