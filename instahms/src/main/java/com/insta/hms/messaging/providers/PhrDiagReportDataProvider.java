...
public class PhrDiagReportDataProvider extends QueryDataProvider {
  static Logger logger = LoggerFactory
      .getLogger(PhrDiagReportDataProvider.class);
  ...
  private String fromTables = "FROM (SELECT tvr.report_id as key, "
      ...
      + "to_char(tvr.report_date  AT TIME ZONE (SELECT  current_setting('TIMEZONE')) "
      + "AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') as report_date_yyyy_mm_dd , "
      + "to_char(tvr.report_date, 'yyyy-MM-dd') as report_date_search__,  "
      + " coalesce(to_char((pr.reg_date + pr.reg_time) AT TIME ZONE ("
      + "SELECT  current_setting('TIMEZONE')) AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"'"
      + "), " + " to_char(isr.date AT TIME ZONE (SELECT  current_setting('TIMEZONE')) "
      ...
      + "(select tp.pat_id from tests_prescribed tp where tp.report_id = tvr.report_id and "
      + "tp.coll_prescribed_id IS NULL  LIMIT 1) as user_name, "
      ...
      + "d.doctor_mail_id as prescribing_doctor_email,d.doctor_id, d.doctor_name,d.specialization,"
      + "ref.ref_id, ref.ref_name, ref.ref_email_id as referral_email,tvr.category, "
      + " (select current_schema()) as group_id, "
      + " (select hospital_name from generic_preferences) as hospital_name "
      + " FROM test_visit_reports tvr " + " LEFT JOIN ( SELECT DISTINCT(SELECT report_id  "
      + "       FROM tests_prescribed WHERE report_id = tppresc.report_id LIMIT 1 ) as report_id,"
      + "       ( SELECT pres_doctor  FROM tests_prescribed "
      + "         WHERE report_id = tppresc.report_id LIMIT 1 ) as pres_doctor "
      + "     FROM ( SELECT DISTINCT report_id,pres_doctor "
      + "FROM tests_prescribed where pres_doctor "
      + "IS NOT NULL AND report_id in #) as tppresc ) as tp USING(report_id)"
      + " LEFT JOIN patient_registration pr ON(pr.patient_id = (select pat_id "
      + "from tests_prescribed where report_id = tvr.report_id "
      + "and coll_prescribed_id is null limit 1) )"
      + " LEFT JOIN incoming_sample_registration isr ON(tvr.patient_id=isr.incoming_visit_id)"
      + " INNER JOIN patient_details pd ON(COALESCE(pr.mr_no, isr.mr_no) = pd.mr_no)"
      + " INNER JOIN patient_demographics_mod pdm ON(pd.mr_no = pdm.mr_no) "
      ...
      + "       as ref_email_id" + "       FROM referral ORDER BY ref_name"
      + " ) as ref ON(ref.ref_id=coalesce(pr.reference_docto_id,isr.referring_doctor)) "
      + " WHERE report_id in # AND tvr.signed_off = 'Y' AND pr.visit_type='o' "
      + " AND  (select net_patient_due from diag_report_sharing_on_bill_payment "
      + "where report_id = tvr.report_id limit 1) IS NULL order by report_date_yyyy_mm_dd )"
      + " as foo ";

  public PhrDiagReportDataProvider() {
    super(THIS_NAME);
