...
public class EMRAccessRightDAO  extends GenericDAO{
	static Logger log = LoggerFactory.getLogger(EMRAccessRightDAO.class);
	Connection con = null;

	public EMRAccessRightDAO() {
		super("emr_access_rule");
	}
	
	private static String rules = 
		"SELECT rule_type FROM emr_access_rule WHERE role_id=? and rule_type='ROLE' " +
		"UNION ALL "+
		"SELECT distinct rule_type FROM emr_access_rule WHERE rule_type='DOC'";
	public List<BasicDynaBean> getRules(int role_id) throws SQLException {
		return DataBaseUtil.queryToDynaList(rules, role_id+"");
	}

	/*public String getNextRuleId() throws SQLException {
		String ruleID = null;
		ruleID =	AutoIncrementId.getNewIncrUniqueId("RULE_ID", "EMR_ACCESS_RULE", "Rule Number");
		return ruleID;
	}*/
	...
	/*public static String DOCS_BY_DOCUMENTTYPEID =" SELECT ear.rule_id, ear.rule_type, ear.doc_type_id, ear.doc_sub_type, ear.center_access,"+
