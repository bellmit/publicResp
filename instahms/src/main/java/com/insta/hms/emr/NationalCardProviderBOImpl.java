...
import com.insta.hms.documentpersitence.NationalCardDocumentAbstractImpl;
 ...
 *
 */
public class NationalCardProviderBOImpl implements EMRInterface {
	...
	public List<EMRDoc> listDocumentsByVisit(String visitId) throws Exception {
		List<BasicDynaBean> l =
			new NationalCardDocumentAbstractImpl().getDocumentsList("patient_id", visitId, false, null);
		return listDocuments(l);
	}
	...
	private List<EMRDoc> listDocuments(List<BasicDynaBean> list) throws Exception {
		...
		for (BasicDynaBean bean: list) {
			...
			doc.setProvider(EMRInterface.Provider.NationalCardProvider);

			String displayUrl = "/Registration/GeneralRegistrationNationalCard.do?_method=viewNationalCardImage&visitId="+doc.getVisitid();
	...
	public List<EMRDoc> listVisitDocumentsForMrNo(String mrNo) throws Exception {
		List<BasicDynaBean> l =
			InsuranceDAO.getAllVisitForMrNoNationalCardDocuments(mrNo);
		return listDocuments(l);
	}
