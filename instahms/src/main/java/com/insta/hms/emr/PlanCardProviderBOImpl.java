...
import com.insta.hms.documentpersitence.PlanCardDocumentAbstractImpl;
...
public class PlanCardProviderBOImpl implements EMRInterface{

	public List<EMRDoc> listDocumentsByVisit(String visitId) throws Exception {
		List<BasicDynaBean> l =
			new PlanCardDocumentAbstractImpl().getDocumentsList("patient_id", visitId, false, null);
		return listDocuments(l);
	}

	public List<EMRDoc> listVisitDocumentsForMrNo(String mrNo) throws Exception {
		List<BasicDynaBean> l =
			InsuranceDAO.getAllVisitForMrNoPlanCardDocuments(mrNo);
		return listDocuments(l);
	}

	public List<EMRDoc> listDocumentsByMrno(String mrNo) throws Exception {
		List<BasicDynaBean> l =
			new PlanCardDocumentAbstractImpl().getDocumentsList("mr_no", mrNo, false, null);
		return listDocuments(l);
	}
	...
	private List<EMRDoc> listDocuments(List<BasicDynaBean> list) throws Exception {
		...
		for (BasicDynaBean bean: list) {
			...
			for (BasicDynaBean b: list) {
				...
				if (documentId != null && b.get("doc_id").toString().equals(documentId)
						&& documentName != null && b.get("doc_name").toString().equals(documentName)
							&& documentDate != null && bean.get("doc_date") != null && ((Date)bean.get("doc_date")).compareTo(documentDate) == 0
								&& planID != null && planID.equals(bean.get("plan_id"))) {
					exists = true;
					break;
				}
			}
			...
			doc.setProvider(EMRInterface.Provider.PlanCardProvider);

			String displayUrl = "/Registration/GeneralRegistrationPlanCard.do?_method=viewInsuranceCardImage&visitId="+doc.getVisitid();
