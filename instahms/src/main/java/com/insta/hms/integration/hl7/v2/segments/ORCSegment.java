...
@Component
public class ORCSegment extends InstaHl7Segment {
  ...
  public void getSegment(Message message, String messageVersion, Map<String, Object> dataMap,
    ...
    segmentTemplate = getSegmentTemplate("ORC", messageVersion, (int) dataMap.get(INTERFACE_ID));
    int count = 0;
    for (Map<String, Object> orcData : dataList) {
      segDataMap = new HashMap<>();
      segDataMap.putAll(orcData);
      ...
      try {
        if ("OMP_O09".equals(msgClassName) || "VXU_V04".equals(msgClassName)
            || "RDS_013".equals(msgClassName)) {
          addSegmentData((AbstractSegment) ((AbstractGroup) msg.get("ORDER", count)).get("ORC"),
              segDataMap, segmentTemplate);
        } else if ("ORU_R01".equals(msgClassName)) {
          addSegmentData(
              (AbstractSegment) ((AbstractGroup) ((AbstractGroup) msg.get("PATIENT_RESULT", count))
                  .get("ORDER_OBSERVATION")).get("ORC"),
              segDataMap, segmentTemplate);
        }
      } catch (Exception exception) {
        log.error("Exception in adding segment to ORC segment for message {} : {}", msgClassName,
            exception);
        throw new HL7Exception(
            "Exception in adding segment to ORC segment " + exception.getMessage());
      }
      if (!repeatSegment) {
        break;
      }
  ...
  public List<Map<String, Object>> getSegmentData(Map<String, Object> dataMap) {
    ...
    } else if (MEDICINE_PRESC.equals(dataMap.get(ITEM_TYPE))) {
