...
@OhsrsFunctionProcessor(supports = "hospOptDischargesER")
public class HospOptDischargesErOhsrsFunction extends GenericOhsrsFunction {
  ...
  private static final String OPD_ICD_1 = "SELECT foo.icd_code, sum(foo.cn)::int cn FROM ("
      ...
      + "  AND pr1.admitted_dept in ";     
  
  private static final String OPD_ICD_2 = "  GROUP BY md1.icd_code"
      + "  UNION ALL"
      ...
      + "  AND pr2.admitted_dept in ";   
  
  private static final String OPD_ICD_3 = "  AND md2.visit_id not in ("
      + "    SELECT mdi.visit_id"
      ...
      + "    AND pri.admitted_dept in "; 
  
  private static final String OPD_ICD_4 = "  ) GROUP BY md2.icd_code"
  ...
  protected List<Map<String, Object>> getData(int year, int centerId) {
    ...
    if (erMapping.size() > 0) {
      ...
      baseQuery = OPD_ICD_1 + placeHolders + OPD_ICD_2 + placeHolders + OPD_ICD_3 + placeHolders
          + OPD_ICD_4;
  ...
  protected boolean submit(int year, OhsrsdohgovphSettings settings,
    ...
    for (Map<String, String> data : uploadData) {
      updateIcdFields(data, "icd10code", "erconsultations", "icd10category");
      if (settings.isTrainingMode()) {
        responseXml = client.getTrainingPort().hospOptDischargesER(
            settings.getHfhudCode(), 
            data.get("erconsultations"),
            data.get("number"),
            data.get("icd10code"),
            data.get("icd10category"),
            reportYear);
      } else {
        responseXml = client.getProductionPort().hospOptDischargesER(
            settings.getHfhudCode(), 
            data.get("erconsultations"),
            data.get("number"),
            data.get("icd10code"),
