...
@Component
public class IN1Segment extends InstaHl7Segment {
  ...
  public void getSegment(Message message, String messageVersion, Map<String, Object> dataMap,
    ...
    segmentTemplate = getSegmentTemplate("IN1", messageVersion, (int) dataMap.get(INTERFACE_ID));
    int count = 0;
    for (Map<String, Object> in1Data : dataList) {
      segDataMap = new HashMap<>();
      segDataMap.putAll(in1Data);
      segDataMap.putAll(dataMap);
      segDataMap.put("sequence", String.valueOf(count + 1));
      try {
        if (validateSegmentData("IN1", segmentTemplate, segDataMap)) {
          addSegmentData((AbstractSegment) ((AbstractGroup) msg.get("INSURANCE", count)).get("IN1"),
              segDataMap, segmentTemplate);
        }
      } catch (Exception exception) {
        log.error("Exception in adding segment to IN1 segment for message {} : {}", msgClassName,
            exception);
        throw new HL7Exception(
            "Exception in adding segment to IN1 segment " + exception.getMessage());
      }
      if (!repeatSegment) {
        break;
      }
  ...
  public List<Map<String, Object>> getSegmentData(Map<String, Object> dataMap) {
    if (null != dataMap.get(INSURANCE_DATA)) {
      return (List<Map<String, Object>>) dataMap.get(INSURANCE_DATA);
    } else {
      segDataMapList = ConversionUtils.copyListDynaBeansToMap(hl7Repository
          .getInsuranceData((String) dataMap.get("visit_id"), (int) dataMap.get("center_id")));
      dataMap.put(INSURANCE_DATA, segDataMapList);
