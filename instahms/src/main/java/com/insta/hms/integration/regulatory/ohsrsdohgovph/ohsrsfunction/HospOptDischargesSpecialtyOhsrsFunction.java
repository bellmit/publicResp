...
@OhsrsFunctionProcessor(supports = "hospOptDischargesSpecialty")
public class HospOptDischargesSpecialtyOhsrsFunction extends GenericOhsrsFunction {

  private static final String ADMDEPT_STAY_LENGTH =  "SELECT "
      + " CASE "
      + "   WHEN pr.admitted_dept IN #GENMED# THEN 'medicine'"
      + "   WHEN pr.admitted_dept IN #OB# THEN 'obstetrics'"
      + "   WHEN pr.admitted_dept IN #GYN# THEN 'gynecology'"
      + "   WHEN pr.admitted_dept IN #PED# THEN 'pediatrics'"
      + "   WHEN (pr.admitted_dept IN #SUR# AND "
      + "     ((pr.discharge_date - COALESCE(pd.dateofbirth, pd.expected_dob))/365.25) < 19)"
      + "     THEN 'pedia'"
      + "   WHEN (pr.admitted_dept IN #SUR# AND "
      + "     ((pr.discharge_date - COALESCE(pd.dateofbirth, pd.expected_dob))/365.25) >= 19)"
      + "     THEN 'adult'"
      + "   ELSE 'others' END service,"
      + " count(pr.admitted_dept) cn, "
  ...
  protected List<Map<String, Object>> getData(int year, int centerId) {
    ...
    if (genMedDeptIds == null || genMedDeptIds.size() == 0) {
      genMedDeptIds = INVALID_ID_LIST;
    }
    String genMedPlaceholders = getDeptInClause(genMedDeptIds);
    List<String> obDeptIds = deptMapping.get("obstetrics");
    if (obDeptIds == null || obDeptIds.size() == 0) {
      obDeptIds = INVALID_ID_LIST;
    }
    String obPlaceholders = getDeptInClause(obDeptIds);
    String stayQuery = ADMDEPT_STAY_LENGTH.replaceAll("#GENMED#", genMedPlaceholders);
    stayQuery = stayQuery.replaceAll("#OB#", obPlaceholders);
    String accQuery = ADMDEPT_ACC_SPLIT.replaceAll("#GENMED#", genMedPlaceholders);
    accQuery = accQuery.replaceAll("#OB#", obPlaceholders);
    String dischargeQuery = ADMDEPT_DISCHARGE_SPLIT.replaceAll("#GENMED#", genMedPlaceholders);
    dischargeQuery = dischargeQuery.replaceAll("#OB#", obPlaceholders);
    String deathSplitQuery = ADMDEPT_DEATH_SPLIT.replaceAll("#GENMED#", genMedPlaceholders);
    deathSplitQuery = deathSplitQuery.replaceAll("#OB#", obPlaceholders);

    List<String> gynDeptIds = deptMapping.get("gynecology");
    if (gynDeptIds == null || gynDeptIds.size() == 0) {
      gynDeptIds = INVALID_ID_LIST;
    }
    String gynPlaceholders = getDeptInClause(gynDeptIds);
    stayQuery = stayQuery.replaceAll("#GYN#", gynPlaceholders);
    accQuery = accQuery.replaceAll("#GYN#", gynPlaceholders);
    dischargeQuery = dischargeQuery.replaceAll("#GYN#", gynPlaceholders);
    deathSplitQuery = deathSplitQuery.replaceAll("#GYN#", gynPlaceholders);

    List<String> pedDeptIds = deptMapping.get("pediatrics");
    if (pedDeptIds == null || pedDeptIds.size() == 0) {
      pedDeptIds = INVALID_ID_LIST;
    }
    String pedPlaceholders = getDeptInClause(pedDeptIds);
    stayQuery = stayQuery.replaceAll("#PED#", pedPlaceholders);
    accQuery = accQuery.replaceAll("#PED#", pedPlaceholders);
    dischargeQuery = dischargeQuery.replaceAll("#PED#", pedPlaceholders);
    deathSplitQuery = deathSplitQuery.replaceAll("#PED#", pedPlaceholders);

    List<String> surDeptIds = deptMapping.get("ipsurgery");
    if (surDeptIds == null || surDeptIds.size() == 0) {
      surDeptIds = INVALID_ID_LIST;
    }
    String surPlaceholders = getDeptInClause(surDeptIds);
    stayQuery = stayQuery.replaceAll("#SUR#", surPlaceholders);
    accQuery = accQuery.replaceAll("#SUR#", surPlaceholders);
    dischargeQuery = dischargeQuery.replaceAll("#SUR#", surPlaceholders);
    deathSplitQuery = deathSplitQuery.replaceAll("#SUR#", surPlaceholders);
    ...
    map.put("medicine", createDefaultRowMap("medicine"));
    map.put("pediatrics", createDefaultRowMap("pediatrics"));
    map.put("gynecology", createDefaultRowMap("gynecology"));
    map.put("obstetrics", createDefaultRowMap("obstetrics"));
    map.put("others", createDefaultRowMap("others"));
    map.put("pedia", createDefaultRowMap("pedia"));
    map.put("adult", createDefaultRowMap("adult"));
    ...
    if (beans != null) {
      for (BasicDynaBean bean : beans) {
        Map<String, Object> rowMap = map.get((String) bean.get("service"));
  ...
  @Override
    ...
    for (Map<String, String> data : uploadData) {
      ...
      if (settings.isTrainingMode()) {
        responseXml = client.getTrainingPort().hospOptDischargesSpecialty(
            settings.getHfhudCode(), 
            data.get("typeofservice"),
            data.get("nopatients"),
