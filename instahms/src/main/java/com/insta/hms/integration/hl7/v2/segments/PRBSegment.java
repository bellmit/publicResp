...
@Component
public class PRBSegment extends InstaHl7Segment {
  ...
  public void getSegment(Message message, String messageVersion, Map<String, Object> dataMap,
    ...
    segmentTemplate = getSegmentTemplate("PRB", messageVersion, (int) dataMap.get(INTERFACE_ID));
    int count = 0;
    for (Map<String, Object> prbData : dataList) {
      segDataMap = new HashMap<>();
      segDataMap.putAll(prbData);
      segDataMap.putAll(dataMap);
      segDataMap.put("sequence", String.valueOf(count + 1));
      try {
        addSegmentData((AbstractSegment) ((AbstractGroup) msg.get("PROBLEM", count)).get("PRB"),
            segDataMap, segmentTemplate);
      } catch (HL7Exception exception) {
        log.error("HL7 Exception in adding segment to PRB segment for message {} : {}",
            msgClassName, exception);
        throw new HL7Exception(
            "HL7 Exception in adding segment to PRB segment " + exception.getMessage());
      } catch (Exception exception) {
        log.error("Exception in adding segment to PRB segment for message {} : {}", msgClassName,
            exception);
        throw new HL7Exception(
            "Exception in adding segment to PRB segment " + exception.getMessage());
      }
      if (!repeatSegment) {
        break;
      }
