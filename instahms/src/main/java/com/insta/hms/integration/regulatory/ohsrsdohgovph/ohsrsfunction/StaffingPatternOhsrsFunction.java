...
@OhsrsFunctionProcessor(supports = "staffingPattern")
public class StaffingPatternOhsrsFunction extends GenericOhsrsFunction {
  ...
  @Override
    ...
    for (Map<String, String> data : uploadData) {
      if (settings.isTrainingMode()) {
        responseXml = client.getTrainingPort().staffingPattern(
        ...
        responseXml = client.getProductionPort().staffingPattern(
            settings.getHfhudCode(), 
            data.get("professiondesignation"),
            data.get("specialtyboardcertified"),
            data.get("fulltime40permanent"),
