...
public class StoresIndentApprovalAction extends BaseAction {
  ...
  public ActionForward list(ActionMapping am, ActionForm af, HttpServletRequest req,
    ...
    isUserHavingSuperStore = (indentApprovalBy.equals("R") || isSuperStoreUser(indentStores))
        ? "Y"
        : "N";

    if (roleId == 1 || roleId == 2) {
      pagedList = StoresIndentDAO.searchIndentList(map, ConversionUtils.getListingParameter(map),
          null, null);
    } else {
      if (indentApprovalBy.equals("R") && requestStores == null) {
        req.setAttribute("error",
            "There is no assigned store, hence you dont have any access to this screen");
      }

      if (indentApprovalBy.equals("I") && indentStores != null) {
        pagedList = StoresIndentDAO.searchIndentList(map, ConversionUtils.getListingParameter(map),
            indentStores);
      } else if (indentApprovalBy.equals("R") && requestStores != null) {
        pagedList = StoresIndentDAO.searchIndentList(map, ConversionUtils.getListingParameter(map),
            null, requestStores);
      }
    }
  ...
  public String getIndentStores(String userName, String indentStore, String indentApprovalBy)
    ...
    if ("I".equalsIgnoreCase(indentApprovalBy)) {
      indentStores = getUserStores(userName); // Approval by Indent store user, return all user
                                              // stores
    }
  ...
  public String getRequestingStores(String userName, String requestingStrore,
    ...
    if ("R".equalsIgnoreCase(indentApprovalBy)) {
      deptFrom = getUserStores(userName); // Approval by Requesting store user, return all user
                                          // stores
    }
  ...
  public ActionForward update(ActionMapping m, ActionForm f, HttpServletRequest req,
    ...
    List<Map<String, Object>> cacheTransferIndentTxns = new ArrayList<>();
    ...
    String indentStatus = req.getParameter("status_main");
    ...
    if (indentStatus.equals("A")) {
      fields.put("approver_remarks", req.getParameter("remarks"));
    } else if (indentStatus.equals("R")) {
    ...
    if (rejectedItems == itemid.length) {
      ...
      fields.put("status", indentStatus);
    }
    ...
    if (success) {
      BasicDynaBean mainBean = storeIndentMainDAO.findByKey(con, "indent_no", indent_no);
      Map filterMap = new HashMap();
      filterMap.put("indent_no", indent_no);
      filterMap.put("status", "A");
      List<BasicDynaBean> detailsBean = dao.listAll(con, null, filterMap, "medicine_name");
      for (BasicDynaBean bean :detailsBean) {
        Map<String, Object> data = scmOutService.getStockIndentMap(mainBean, bean);
        if (!data.isEmpty()) {
          cacheTransferIndentTxns.add(data);
        }
      }
    }

    if (success) {
      if (!cacheTransferIndentTxns.isEmpty() && module != null
          && ((String) module.get("activation_status")).equals("Y")) {
        scmOutService.scheduleStockTransferIndentTxns(cacheTransferIndentTxns);
      }
    ...
    redirect.addParameter("indent_type", "S");
    return redirect;
  }
