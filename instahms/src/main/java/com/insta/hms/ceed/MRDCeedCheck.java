 ...
 */
public class MRDCeedCheck extends AbstractCeedCheck {

  /*
   * This method returns a list of all activities.
   */
  @Override
  public List<BasicDynaBean> getActivities(String consultationId, String visitId)
      throws SQLException {
    List<BasicDynaBean> treatmentCodes = CeedDAO.getTreatmentCodesList(visitId);
    return treatmentCodes;
  }

  /*
   * This method inserts activities in ceed_integration_details.
   */
  @Override
  public Map insertActivitiesInCeedIntegrationDetails(Connection con,
      List<BasicDynaBean> allActivities, int claimId, Integer consultationId, String patientId,
      String serviceType) throws SQLException, IOException {
    List<DynaBean> activitiesToSend = new ArrayList<DynaBean>();
    ...
    while (itr.hasNext()) {
      ...
      first = false;
      if (!CeedDAO.insertMrdCeedIntegrationDetails(con, claimId,
          (String) bean.get("patient_presc_id"), null, (Integer) null,
  ...
  public BasicDynaBean getEncounterDetails(String visitId) throws SQLException {
    return CeedDAO.getEncounterDetails(visitId);
  }
    ...
    if (response.value.getClaimEdit().size() > 0) {
      ...
      while (aeitr.hasNext()) {
        ...
        for (String claimEditId : ae.getEditId()) {
          ...
          if (index == 0) {
            if (!CeedDAO.updateMrdCeedIntegrationDetails(con, claimId, activityId, claimEditRefNum,
            ...
            if (!CeedDAO.insertMrdCeedIntegrationDetails(con, claimId, activityId, claimEditRefNum,
                Integer.parseInt(claimEditId), claimEditSubType, claimEditCode, claimEditRank,
                claimEditResponseComments)) {
              logger.error(ceedTransactionFailureMessage);
              return returnTransactionFailureMessageMap(ceedTransactionFailureMessage);
  ...
  /*
   * This method returns request in ceed_integration_main
   */
  @Override
  public boolean insertRequestDetailsInCeedMain(Connection con, int claimId, Integer consultationId,
    ...
    return CeedDAO.insertMrdRequestDetailsInCeedMain(con, claimId, visitId, status, serviceType);
  }

  /*
   * This method returns a list of observations for a visit.
   */
  @Override
  public Map getObservations(String visitId) throws SQLException {
    List<BasicDynaBean> observationsList = CeedDAO.getObservations(visitId);
    List<BasicDynaBean> validObservationsList = new ArrayList<BasicDynaBean>();

    Iterator<BasicDynaBean> iter = observationsList.iterator();
    while (iter.hasNext()) {
      BasicDynaBean bean = iter.next();
      if (validateObservationType((String) bean.get("observation_type"))) {
        validObservationsList.add(bean);
      }
    }

    Map observations = ConversionUtils.listBeanToMapListMap(validObservationsList, "charge_id");
    return observations;
  }
}
