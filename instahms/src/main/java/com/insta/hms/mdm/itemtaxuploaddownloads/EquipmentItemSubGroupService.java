...
import com.insta.hms.mdm.equipmentcharges.EquipmentChargesCsvBulkDataEntity;
...
@Service
public class EquipmentItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(EquipmentItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private EquipmentItemSubGroupRepository equipmentItemSubGroupRepository;

  public EquipmentItemSubGroupService(EquipmentItemSubGroupRepository repository,
      EquipmentItemSubGroupValidator validator,
      EquipmentChargesCsvBulkDataEntity csvBulkDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> equipMap = equipmentItemSubGroupRepository.getEquipMap();
    ...
    try {
      ...
      if (!headers[0].matches("eq_id")) {
        return "Incompatible file for equipment upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("eq_id")) {
              itemId = equipMap.get(fieldValue);
        ...
        if (bean.get("eq_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue.contains(itemgrpId.toString() + "_" + bean.get("eq_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("eq_id").toString());
          } else {
      ...
      try {
        equipmentItemSubGroupRepository.deleteAllRecords();
        equipmentItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Equipment processed successfully", lineNumber - 1);
