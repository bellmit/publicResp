...
import com.insta.hms.mdm.diagnosticcharges.DiagnosticChargeCsvBulkDataEntity;
...
@Service
public class DiagnosticsItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(DiagnosticsItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private DiagnosticsItemSubGroupRepository diagnosticsItemSubGroupRepository;

  public DiagnosticsItemSubGroupService(DiagnosticsItemSubGroupRepository repository,
      DiagnosticsItemSubGroupValidator validator, DiagnosticChargeCsvBulkDataEntity csvDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> diagMap = diagnosticsItemSubGroupRepository.getDiagMap();
    ...
    headersMap.put("test_name", "test_name");
    ...
    try {
      ...
      if (!headers[0].matches("test_id")) {
        return "Incompatible file for diagnostics type upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        if (bean.get("test_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("test_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("test_id").toString());
          } else {
      ...
      try {
        diagnosticsItemSubGroupRepository.deleteAllRecords();
        diagnosticsItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Diagnostics processed Successfully", lineNumber - 1);
