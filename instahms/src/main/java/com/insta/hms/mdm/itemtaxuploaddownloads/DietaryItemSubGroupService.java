...
@Service
public class DietaryItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(DietaryItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private DietaryItemSubGroupReposiroty dietaryItemSubGroupReposiroty;

  public DietaryItemSubGroupService(DietaryItemSubGroupReposiroty repository,
      DietaryItemSubGroupValidator validator,
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> dietMap = dietaryItemSubGroupReposiroty.getDietaryMap();
    ...
    try {
      ...
      if (!headers[0].matches("diet_id")) {
        return "Incompatible file for dietary upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("diet_id")) {
              itemId = dietMap.get(fieldValue);
              if (itemId == null || itemId.equals("")) {
                ...
                itemId = fieldValue;
                Integer dietId = Integer.parseInt(itemId);
                bean.set("diet_id", dietId);
              }
        ...
        if (bean.get("diet_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("diet_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("diet_id").toString());
          } else {
      ...
      try {
        dietaryItemSubGroupReposiroty.deleteAllRecords();
        dietaryItemSubGroupReposiroty.batchInsert(beans);
      ...
      meta.add("Dietary processed successfully", lineNumber - 1);
