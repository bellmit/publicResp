...
@Service
public class PackageItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(PackageItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private PackageItemSubGroupRepository packageItemSubGroupRepository;

  public PackageItemSubGroupService(PackageItemSubGroupRepository repository,
      PackageItemSubGroupValidator validator,
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> packMap = packageItemSubGroupRepository.getPackIdMap();
    ...
    try {
      ...
      if (!headers[0].matches("package_id")) {
        return "Incompatible file for package upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("package_id")) {
              itemId = packMap.get(fieldValue);
              if (itemId == null || itemId.equals("")) {
                ...
                // itemId = fieldValue;
                int packId = Integer.parseInt(itemId);
                bean.set("package_id", packId);
              }
        ...
        if (bean.get("package_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("package_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("package_id").toString());
          } else {
      ...
      try {
        packageItemSubGroupRepository.deleteAllRecords();
        packageItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Packages processed successfully", lineNumber - 1);
