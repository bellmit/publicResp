...
@Repository
public class PerdiemCodeItemSubGroupRepository extends BulkDataRepository<String> {

  public PerdiemCodeItemSubGroupRepository() {
    super("perdiem_code_item_sub_groups", "per_diem_code", "item_subgroup_id");
  }

  private static final String query = "SELECT  pdc.per_diem_code , pdc.per_diem_description,"
      + "ig.item_group_name, isg.item_subgroup_name " + " FROM per_diem_codes_master pdc "
      + " LEFT JOIN perdiem_code_item_sub_groups pcisg USING(per_diem_code) "
      + " LEFT JOIN item_sub_groups isg using(item_subgroup_id) "
      + " LEFT JOIN item_groups ig using(item_group_id) "
      + "WHERE pdc.status='A' ORDER BY per_diem_code";
  ...
  /**
   * Gets the per diem code map.
   *
   * @return the per diem code map
   */
  public Map<String, String> getPerDiemCodeMap() {
    Map<String, String> itemMaps = new HashMap<String, String>();
    List<BasicDynaBean> list = getPerDiemMap();
    for (BasicDynaBean bean : list) {
      itemMaps.put((String) bean.get("per_diem_code"), (String) bean.get("per_diem_code"));
    }
    return itemMaps;
  }

  public List<BasicDynaBean> getPerDiemMap() {
    return DatabaseHelper.queryToDynaList("select per_diem_code, per_diem_code  from"
        + " per_diem_codes_master where status='A' order by per_diem_code");
  }
