...
@Service
public class ConsultationItemSubGroupsService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(ConsultationItemSubGroupsService.class);
  @LazyAutowired
  ...
  @Autowired
  private ConsultationItemSubGroupsRepository consultationItemSubGroupsRepository;

  public ConsultationItemSubGroupsService(ConsultationItemSubGroupsRepository repository,
      ConsultationItemSubGroupsValidator validator,
      ConsultationItemSubGroupsCsvBulkDataEntity csvBulkDataEnty) {
    super(repository, validator, csvBulkDataEnty);
  }
  ...
  public Map<String, List<BasicDynaBean>> getMasterData() {
    ...
    // masterData.put("item_group_id", taxGroupService.lookup(false));
    // masterData.put("item_subgroup_id", taxSubGroupService.lookup(false));
    // masterData.put("consultation_type_id", taxConsultationTypeService.lookup(false));
    return masterData;
  }
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> consultIdMap = consultationItemSubGroupsRepository.getConsultIdMap();
    ...
    try {
      ...
      if (!headers[0].matches("consultation_type_id")) {
        return "Incompatible file for consultation type upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("consultation_type_id")) {
              itemId = consultIdMap.get(fieldValue);
              if (itemId == null || itemId.equals("")) {
                ...
                // itemId = fieldValue;
                int consultId = Integer.parseInt(itemId);
                bean.set("consultation_type_id", consultId);
              }
            }
        ...
        if (bean.get("consultation_type_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("consultation_type_id").toString())) {
            beans.add(bean);
            duplicateValue
                .add(itemgrpId.toString() + "_" + bean.get("consultation_type_id").toString());
          } else {
      ...
      try {
        consultationItemSubGroupsRepository.deleteAllRecords();
        consultationItemSubGroupsRepository.batchInsert(beans);
      ...
      meta.add("Consultation types processed Successfully", lineNumber - 1);
