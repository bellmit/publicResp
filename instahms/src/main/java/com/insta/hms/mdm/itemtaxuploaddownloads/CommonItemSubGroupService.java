...
@Service
public class CommonItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(CommonItemSubGroupService.class);
  @LazyAutowired
  ...
  private CommonItemSubGroupRepository commonItemSubGroupRepository;

  public CommonItemSubGroupService(CommonItemSubGroupRepository repository,
      CommonItemSubGroupValidator validator, CommonItemSubGroupCsvBulkDataEntity cscDataEntity) {
    super(repository, validator, cscDataEntity);
    this.commonItemSubGroupRepository = repository;
  }
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> commonMap = commonItemSubGroupRepository.getCommonMap();
    ...
    try {
      ...
      if (!headers[0].matches("charge_name")) {
        return "Incompatible file for other charges upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("charge_name")) {
              itemId = commonMap.get(fieldValue);
        ...
        if (bean.get("charge_name") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("charge_name").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("charge_name").toString());
          } else {
      ...
      try {
        commonItemSubGroupRepository.deleteAllRecords();
        commonItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Other charges processed successfully", lineNumber - 1);
