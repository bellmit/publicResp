...
@Service
public class PerdiemCodeItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(PerdiemCodeItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private PerdiemCodeItemSubGroupRepository perdiemCodeItemSubGroupRepository;

  public PerdiemCodeItemSubGroupService(PerdiemCodeItemSubGroupRepository repository,
      PerdiemCodeItemSubGroupValidator validator,
      PackageItemSubGroupCsvBulkDataEntity csvBulkdataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> perDiemMap = perdiemCodeItemSubGroupRepository.getPerDiemCodeMap();
    ...
    headersMap.put("per_diem_code", "per_diem_code");
    headersMap.put("per_diem_description", "per_diem_description");
    ...
    try {
      ...
      if (!headers[0].matches("per_diem_code")) {
        return "Incompatible file for perdiem code upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("per_diem_code")) {
              itemId = perDiemMap.get(fieldValue);
        ...
        if (bean.get("per_diem_code") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("per_diem_code").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("per_diem_code").toString());
          } else {
        ...
        // bean.get("per_diem_code").toString())) {
        // beans.add(bean);
      ...
      try {
        perdiemCodeItemSubGroupRepository.deleteAllRecords();
        perdiemCodeItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("perdiem code processed successfully", lineNumber - 1);
