...
@Repository
public class DiagnosticsItemSubGroupRepository extends BulkDataRepository<String> {

  public DiagnosticsItemSubGroupRepository() {
    super("diagnostics_item_sub_groups", "test_id", "item_subgroup_id");
  }

  private static final String query = " SELECT  d.test_id, d.test_name ,"
      + "ig.item_group_name, isg.item_subgroup_name "
      + " FROM diagnostics d " + " LEFT JOIN diagnostics_item_sub_groups disg USING(test_id) "
      + " LEFT JOIN item_sub_groups isg using(item_subgroup_id) "
      + " LEFT JOIN item_groups ig using(item_group_id) "
      + " WHERE d.status='A' ORDER BY test_name";
   ...
   * @return the diag map
   */
  public Map<String, String> getDiagMap() {
    Map<String, String> itemMaps = new HashMap<String, String>();
    List<BasicDynaBean> list = getTestMap();
    for (BasicDynaBean bean : list) {
      itemMaps.put((String) bean.get("test_id"), (String) bean.get("test_id"));
    }
    return itemMaps;
  }

  public List<BasicDynaBean> getTestMap() {
    return DatabaseHelper.queryToDynaList(
        "select test_id, test_id  from" + " diagnostics where status='A' order by test_id");
  }
