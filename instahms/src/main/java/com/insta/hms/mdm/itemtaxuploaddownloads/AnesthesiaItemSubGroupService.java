...
@Service
public class AnesthesiaItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(AnesthesiaItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private AnesthesiaItemSubGroupRepository anesthesiaItemSubGroupRepository;

  public AnesthesiaItemSubGroupService(AnesthesiaItemSubGroupRepository repository,
      AnesthesiaItemSubGroupValidator validator,
      AnesthesiaItemSubGroupCsvBulkDataEntity csvDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> anesthMap = anesthesiaItemSubGroupRepository.getAnesthMap();
    ...
    try {
      ...
      if (!headers[0].matches("anesthesia_type_id")) {
        return "Incompatible file for anesthesia upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("anesthesia_type_id")) {
              itemId = anesthMap.get(fieldValue);
              if (itemId == null || itemId.equals("")) {
                ...
                itemId = fieldValue;
                bean.set("anesthesia_type_id", itemId);
              }
            }
        ...
        if (bean.get("anesthesia_type_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("anesthesia_type_id").toString())) {
            beans.add(bean);
            duplicateValue
                .add(itemgrpId.toString() + "_" + bean.get("anesthesia_type_id").toString());
          } else {
        ...
        // bean.get("anesthesia_type_id").toString())) {
        // beans.add(bean);
      ...
      try {
        anesthesiaItemSubGroupRepository.deleteAllRecords();
        anesthesiaItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Anesthesia processed successfully", lineNumber - 1);
