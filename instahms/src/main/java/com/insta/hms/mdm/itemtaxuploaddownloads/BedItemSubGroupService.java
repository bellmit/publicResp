...
@Service
public class BedItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(BedItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private BedItemSubGroupRepository bedItemSubGroupRepository;

  public BedItemSubGroupService(BedItemSubGroupRepository repository,
      BedItemSubGroupValidator validator, BedItemSubGroupCsvBulkDataEntity csvDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> bedMap = bedItemSubGroupRepository.getBedMap();
    ...
    try {
      ...
      if (!headers[0].matches("bed_type_name")) {
        return "Incompatible file for bed type upload";
      }
      if (headers.length < 1) {
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("bed_type_name")) {
              itemId = bedMap.get(fieldValue);
        ...
        if (bean.get("bed_type_name") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("bed_type_name").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("bed_type_name").toString());
          } else {
        ...
        // bean.get("bed_type_name").toString())) {
        // beans.add(bean);
      ...
      try {
        bedItemSubGroupRepository.deleteAllRecords();
        bedItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Bed types processed Successfully", lineNumber - 1);
