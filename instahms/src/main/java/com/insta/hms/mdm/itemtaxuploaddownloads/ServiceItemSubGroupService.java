...
@Service
public class ServiceItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(ServiceItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private ServiceItemSubGroupRepository serviceItemSubGroupRepository;

  public ServiceItemSubGroupService(ServiceItemSubGroupRepository repository,
      ServiceItemSubGroupValidator validator,
      ServicesItemSubGroupCsvBulkDataEntity csvBulkDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> servicesMap = serviceItemSubGroupRepository.getServicesMap();
    ...
    try {
      ...
      if (!headers[0].matches("service_id")) {
        return "Incompatible file for services upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("service_id")) {
              serviceId = servicesMap.get(fieldValue);
        ...
        if (bean.get("service_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("service_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("service_id").toString());
          } else {
      ...
      try {
        serviceItemSubGroupRepository.deleteAllRecords();
        serviceItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Services processed successfully", lineNumber - 1);
