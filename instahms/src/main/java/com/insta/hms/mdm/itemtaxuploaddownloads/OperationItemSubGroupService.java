...
@Service
public class OperationItemSubGroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(OperationItemSubGroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private OperationItemSubGroupRepository operationItemSubGroupRepository;

  public OperationItemSubGroupService(OperationItemSubGroupRepository repository,
      OperationItemSubGroupValidator validator,
      OperationItemSubGroupCsvBulkDataEntity csvDataEntity) {
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> operationMap = operationItemSubGroupRepository.getOperationMap();
    ...
    try {
      ...
      if (!headers[0].matches("op_id")) {
        return "Incompatible file for operation upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("op_id")) {
              opId = operationMap.get(fieldValue);
        ...
        if (bean.get("op_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue.contains(itemgrpId.toString() + "_" + bean.get("op_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("op_id").toString());
          } else {
      ...
      try {
        operationItemSubGroupRepository.deleteAllRecords();
        operationItemSubGroupRepository.batchInsert(beans);
      ...
      meta.add("Operation processed successfully", lineNumber - 1);
