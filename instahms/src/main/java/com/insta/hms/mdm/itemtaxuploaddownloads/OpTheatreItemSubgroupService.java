...
@Service
public class OpTheatreItemSubgroupService extends BulkDataService {

  static Logger logger = LoggerFactory.getLogger(OpTheatreItemSubgroupService.class);
  @LazyAutowired
  ...
  @Autowired
  private OpTheatreItemSubgroupRepository opTheatreItemSubgroupRepository;

  public OpTheatreItemSubgroupService(OpTheatreItemSubgroupRepository respository,
      OpTheatreItemSubgroupValidator validator,
      OperationItemSubGroupCsvBulkDataEntity csvBulkDataEntity) {
    super(respository, validator, csvBulkDataEntity);
  }
  ...
  public String parseAndImportCsV(MultipartFile file,
    ...
    Map<String, String> opTMap = opTheatreItemSubgroupRepository.getOpTheatreMap();
    ...
    try {
      ...
      if (!headers[0].matches("theatre_id")) {
        return "Incompatible file for operation theatre upload";
      }
      ...
      while (null != (row = csvReader.readNext())) {
        ...
        for (Integer columnIndex = 0; columnIndex < headers.length
          ...
          if ((null != fieldValue) && !(fieldValue.isEmpty())) {
            ...
            if (fieldName.equals("theatre_id")) {

              itemId = opTMap.get(fieldValue);
        ...
        if (bean.get("theatre_id") != null && bean.get("item_subgroup_id") != null) {
          // Here check duplicate values in the csv file
          if (!duplicateValue
              .contains(itemgrpId.toString() + "_" + bean.get("theatre_id").toString())) {
            beans.add(bean);
            duplicateValue.add(itemgrpId.toString() + "_" + bean.get("theatre_id").toString());
          } else {
      ...
      try {
        opTheatreItemSubgroupRepository.deleteAllRecords();
        opTheatreItemSubgroupRepository.batchInsert(beans);
      ...
      meta.add("Operation theatre processed successfully", lineNumber - 1);
