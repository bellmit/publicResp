...
import com.insta.hms.master.MedicineDosage.MedicineDosageDAO;
 ...
 *
 */
public class ERxRequestGenerator extends ERequestXMLGenerator<ERxRequest, ERxRequestValidator> {

  ERxPrescriptionDAO erxdao = new ERxPrescriptionDAO();
  ClaimSubmissionDAO claimsubdao = new ClaimSubmissionDAO();
  static Logger log = LoggerFactory.getLogger(ERxRequestGenerator.class);
  ...
  public ERxRequestValidator getRequestValidator() {
    return new ERxRequestValidator();
  }
   ...
   * in the FTL to finally generate an XML.
   *
   * Note : This method should not do any data validation, that should be done only in the
   * validator. Also it should not do any updates to the existing data. That should be done before
   * or after the XML generation, as appropriate.
   *
   * @param requestId
   * @param requestType
   * @return
  ...
  private ERxRequest createNewERxRequest(Object consId, String requestType) {
    ...
    try {
      ...
      int pbmPrescId = (consId instanceof String) ? erxdao.getErxVisitPBMId(consId.toString()) : erxdao.getErxConsPBMId((int)consId);
      eRxPresc.setRequestId(consId);

      // Get erx cons or discharge medication bean.
      ...
      if (isSelfpaySponsor || tpaId == null) {
        isSelfpay = true;
      }
      ...
      // TODO: Need to check about SelfPay
      ...
      prescription.setClinician((String) erxheaderBean.get("doctor_license_number"));

      EPrescriptionPatient patient = new EPrescriptionPatient();

      BasicDynaBean weightVitalBean = VisitVitalsDAO.getVisitVitalWeightBean(patientId);
      // Set weight
      int weight = 0;
      if (weightVitalBean != null && (Integer) weightVitalBean.get("param_id") != 0) {
        if (SearchQueryBuilder.isInteger((String) weightVitalBean.get("param_value"))) {
          weight = new Integer((String) weightVitalBean.get("param_value"));
        } else if (SearchQueryBuilder.isFloat((String) weightVitalBean.get("param_value"))) {
          weight = Math.round(new Float((String) weightVitalBean.get("param_value")));
        }
      }
      ...
      if (!isSelfpay) {
        ...
        // set member id as mrNo#FacilityId
        patient.setMemberId(
            (String) erxConsBean.get("mr_no") + "#" + (String) erxheaderBean.get("provider_id"));
      }
      patient.setEmiratesIDNumber((String) erxConsBean.get("emirates_id_number"));
      ...
      encounter.setType(
          erxConsBean.get("encounter_type") != null ? (Integer) erxConsBean.get("encounter_type")
              : 0);
      encounter.setFacilityID((String) erxheaderBean.get("provider_id"));

      prescription.setEncounter(encounter);

      List<BasicDynaBean> prescDiagnosis = new PBMPrescriptionsDAO().findAllDiagnosis(patientId);
      ...
      if (!requestType.equals("eRxCancellation")) {
        ...
        List<BasicDynaBean> prescMedList = erxdao.getErxPrescribedActivities(pbmPrescId,
            healthAuthority);
        ...
        for (BasicDynaBean erxmed : prescMedList) {
          ...
          activity.setMedicineID(
              erxmed.get("medicine_id") != null ? (Integer) erxmed.get("medicine_id") : 0);
          ...
          activity.setActivityCode(
              (erxmed.get("item_code") != null && !((String) erxmed.get("item_code")).equals(""))
          ...
          activity.setActivityStart((String) erxmed.get("activity_prescribed_date"));
          activity.setDuration(new BigDecimal((Integer) erxmed.get("duration")));
          activity.setInstructions((String) erxmed.get("medicine_remarks"));
          activity.setQuantity(new BigDecimal((Integer) erxmed.get("medicine_quantity")));
          ...
          activity.setRoutOfAdmin(
              erxmed.get("route_code") != null ? (String) erxmed.get("route_code") : null);

          if (null != erxmed.get("granular_units") && erxmed.get("granular_units").equals("Y")) {
            String frequency = erxmed.get("frequency") != null ? (String) erxmed.get("frequency")
                : null;
            if (frequency != null && !frequency.trim().equals("")) {
              MedicineDosageDAO medDosageDAO = new MedicineDosageDAO();
              BasicDynaBean freqBean = medDosageDAO.findByKey("dosage_name", frequency);
              ...
              if (null != freqBean) {
                ...
                String strength = erxmed.get("strength") != null ? (String) erxmed.get("strength")
                    : "";
                BigDecimal freqUnit = ERxPrescriptionDAO.getStrengthBigDecimal(strength);

                EPrescriptionActivity.Frequency actFreq = new EPrescriptionActivity().new Frequency();
      ...
      log.error("Exception while creating new ERx Request", e);
