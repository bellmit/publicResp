...
/**
 * The Class AccountingPharmacyBillsExporter.
 *
 * @author krishna
 */
public class AccountingPharmacyBillsExporter extends GenericAccountingExporter {

  /** The dept dao. */
  DepartmentMasterDAO deptDao = new DepartmentMasterDAO();

  /** The Constant THIS_VOUCHER_TYPE. */
  private static final String THIS_VOUCHER_TYPE = "PHARMACY_BILLS";

  /** The dept map. */
  private Map<String, Map> deptMap = new HashMap<String, Map>();

  /**
   * Instantiates a new accounting pharmacy bills exporter.
   *
   * @param format
   *          the format
   */
  public AccountingPharmacyBillsExporter(String format) {
    super(THIS_VOUCHER_TYPE, "bill_no", "voucher_date", "bills", "pharmacy_bill_vtype",
        "PHARMACYBILL", format);
  }
  ...
  protected void export(int centerId, List voucherList, OutputStream stream) throws SQLException,
    ...
    try {
      ...
      obj = ChargeDAO.getPharmacyBillSummary(con, fromDate, toDate, accountGroup, centerId,
          voucherList);
      rs = (ResultSet) obj[1];
  ...
  protected Map getDeptsMap() throws SQLException {
    ...
    List listCols = deptDao.listAll(cols);
    schemaDeptMap = ConversionUtils.listBeanToMapMap(listCols, "dept_id");
