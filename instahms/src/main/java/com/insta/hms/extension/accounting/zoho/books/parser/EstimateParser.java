...
public class EstimateParser {
   ...
   *
   * @param response
   *          This json response contains the estimate details.
  ...
  public Estimate getEstimate(String response) throws Exception {
    ...
    for (int i = 0; i < lineItems.length(); i++) {
      ...
      if (lineItems.getJSONObject(i).has("discount_amount")) {
        // No I18N
        lineItem.setDiscountAmount(lineItems.getJSONObject(i).getDouble("discount_amount"));
      }
      ...
      if (paymentOptions.has("payment_gateways")) {
        ...
        for (int i = 0; i < paymentGateways.length(); i++) {
          if (paymentGateways.getJSONObject(i).has("gateway_name")) {
            paymentGateway.setGatewayName(paymentGateways.getJSONObject(i)
                .getString("gateway_name"));
          }
          if (paymentGateways.getJSONObject(i).has("additional_field1")) {
            paymentGateway.setAdditionalField1(paymentGateways.getJSONObject(i).getString(
                "additional_field1"));
          }
          if (paymentGateways.getJSONObject(i).has("configured")) {
            // No I18N
            paymentGateway.setConfigured(paymentGateways
                .getJSONObject(i).getBoolean("configured"));
          }
   ...
   *
   * @param response
   *          This json response contains list of estimates details.
  ...
  public EstimateList getEstimates(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      if (jsonArray.getJSONObject(i).has("is_viewed_by_client")) {
        // No I18N
        estimate.setIsViewedByClient(jsonArray.getJSONObject(i).getBoolean("is_viewed_by_client"));
      }
   ...
   *
   * @param response
   *          This json response contains the email content details for estimate.
   ...
   *
   * @param response
   *          This json response contains list of templates details for estimate.
   ...
   *
   * @param response
   *          This json response contains list of comments details for estimate.
   ...
   *
   * @param response
   *          This json response contains the comment details for estimate.
