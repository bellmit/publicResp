...
import com.insta.hms.extension.accounting.zoho.books.model.CustomerPayment;
import com.insta.hms.extension.accounting.zoho.books.model.Expense;
import com.insta.hms.extension.accounting.zoho.books.model.Transaction;
import com.insta.hms.extension.accounting.zoho.books.model.TransactionList;
import com.insta.hms.extension.accounting.zoho.books.model.VendorPayment;
import com.insta.hms.extension.accounting.zoho.books.parser.BankTransactionParser;
import com.insta.hms.extension.accounting.zoho.books.parser.CreditNoteParser;
import com.insta.hms.extension.accounting.zoho.books.parser.CustomerPaymentParser;
import com.insta.hms.extension.accounting.zoho.books.parser.ExpenseParser;
import com.insta.hms.extension.accounting.zoho.books.parser.VendorPaymentParser;
 ...
 * The list of all associated transactions.<br>
 * <br>
 * It is used to update the details of a transaction.<br>
 * <br>
 * It is used to:<br>
 * <br>
 ...
 * Uncategorize a categorized transaction.<br>
 * <br>
...
public class BankTransactionsApi extends API {
   ...
   * <tr>
   * <td>account_id*</td>
   * <td>Mandatory Account id for which transactions are to be listed.</td>
   * </tr>
   * <tr>
   * <td>filter_by</td>
   * <td>Filters the transactions based on the allowed types.<br>
   * Allowed Values: <i>Status.All, Status.Uncategorized, Status.Categorized, Status.ManuallyAdded,
   * Status.Excluded</i> and <i>Status.Matched</i></td>
   * </tr>
   * <tr>
   * <td>date</td>
   * <td>Start and end date, to provide a range within which the transaction date exist.<br>
   * Variants: <i>date_start</i> and <i>date_end</i></td>
   * </tr>
   * <tr>
   * <td>amount</td>
   * <td>Start and end amount, to provide a range within which the transaction amount exist.<br>
   * Variants: <i>amount_start</i> and <i>amount_end</i></td>
   * </tr>
   * <tr>
   * <td>status</td>
   * <td>Transaction status wise list view - All, uncategorized, manually_added, matched, excluded,
   * categorized</td>
   * </tr>
   * <tr>
   * <td>reference_number</td>
   * <td>Reference Number of the transaction</td>
   * </tr>
   * <tr>
   * <td>sort_column</td>
   * <td>Sorts the transactions based on the allowed sort types. <br>
   * Allowed Values: <i>date</i></td>
   * </tr>
   * </tbody>
   * </table>
   *
   * @param queryMap
   *          It contains the querystring values in the form of key-value pair.
   ...
   * The Transaction object which contains transactionId is the mandatory parameter for which bank
   * transaction has to be updated.
   * It returns the Transaction object.
   *
   * @param transaction
   *          Transaction object.
   ...
   * <td>The type of transaction. <br>
   * Allowed Values: <i>expense, deposit, refund, transfer_fund, card_payment,
   * sales_without_invoices, expense_refund, owner_contribution, interest_income, other_income,
   * owner_drawings, invoice, bill, credit_notes, creditnote_refund, customer_payment</i> and
   * <i>vendor_payment</i></td>
   * </tr>
   * <tr>
   * <td>date</td>
   * <td>Start and end date, to provide a range within which the transaction date exist.<br>
   ...
   * <tr>
   * <td>amount</td>
   * <td>Initial and final amount range within which the search amount exists.<br>
   * Variants: <i>amount_start</i> and <i>amount_end</i></td>
   * </tr>
   * <tr>
   * <td>contact</td>
   * <td>Contact person name, involved in the transaction.</td>
   * </tr>
   * <tr>
   * <td>reference_number</td>
   * <td>Reference Number of the transaction.</td>
  ...
  public TransactionList getMatchingTransactions(String transactionId,
   ...
   * The Transaction objects which contains transactionId and transactionType are the mandatory
   * parameters.
   * If the transaction is uncategorized it returns the success message.
   * The success message is
   * "The Uncategorized transaction is linked to the selected transaction(s) in Zoho Books."
   *
   * @param transactionId
   *          ID of the transaction.
   * @param transactions
  ...
  public String matchTransaction(String transactionId, List<Transaction> transactions)
      throws Exception {
  ...
  /**
   * Get a list of all the associated transactions that were matched or categorized to the given
   * imported transaction.
   * Pass the transactionId and sortColumn to get the associated transaction based on the sort
   * column.
   * It returns the Transaction object.
   *
   * @param transactionId
   *          ID of the transaction.
   * @param sortColumn
   *          Sort column for the associate transaction.
   * @return Returns the Transaction object.
   * @throws Exception
   *           the exception
   */

  public Transaction getAssociateTransaction(String transactionId, String sortColumn)
      throws Exception {
  ...
  public Transaction categorizeAnUncategorizeTransaction(Transaction transaction) throws Exception {
    // No I18N
    String urlString = url + "/uncategorized/" + transaction.getTransactionId() + "/categorize";
   ...
   * Pass the transactionId and CreditnoteRefund object to categorize the transaction as a refund
  ...
  public CreditnoteRefund categorizeAsCreditnoteRefunds(String transactionId,
      CreditnoteRefund creditnoteRefund) throws Exception {
    // No I18N
    String urlString = url + "/uncategorized/" + transactionId + "/categorize/creditnoterefunds";

    HashMap<String, Object> requestBody = getQueryMap();

    requestBody.put("JSONString",
        creditnoteRefund.toJSON().put("creditnote_id", creditnoteRefund.getCreditnoteId())
            .toString());
   ...
   * Pass the transactionId and VendorPayment object to categorize the transaction as vendor
   * payment.
   ...
   *          ID of the transaction.
   * @param vendorPayment
   *          VendorPayment object.
   * @return Returns the VendorPayment object.
   * @throws Exception
   *           the exception
   */

  public VendorPayment categorizeAsVendorPayment(String transactionId, VendorPayment vendorPayment)
      throws Exception {
    // No I18N
    String urlString = url + "/uncategorized/" + transactionId + "/categorize/vendorpayments";
   ...
   * Pass the transactionId and CustomerPayment object to categorize the transaction as customer
   * payment.
  ...
  public CustomerPayment categorizeAsCustomerPayment(String transactionId,
      CustomerPayment customerPayment) throws Exception {
    // No I18N
    String urlString = url + "/uncategorized/" + transactionId + "/categorize/customerpayments";
   ...
   * Pass the transactionId, Expense object, and File object to categorize the transaction as
   * expense.
   * The Expense object which contains accountId, paidThroughAccountId, and amount are the mandatory
   * parameters.
   ...
   *          Expense object.
   * @param file
   *          Receipt file for the expense.
   * @return Returns the Expense object.
   * @throws Exception
   *           the exception
   */

  public Expense categorizeAsExpense(String transactionId, Expense expense, File file)
      throws Exception {
