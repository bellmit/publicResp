...
import com.insta.hms.extension.accounting.zoho.books.model.Criteria;
import com.insta.hms.extension.accounting.zoho.books.model.Instrumentation;
...
public class BankTransactionParser {
   ...
   *
   * @param response
   *          This json response contains list of transactions details.
  ...
  public TransactionList getTransactions(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      transaction.setImportedTransactionId(jsonArray.getJSONObject(i).getString(
    ...
    if (jsonObject.has("instrumentation")) {
      ...
      // No I18N
      instrumentation.setPageContextWriteTime(instrumen.getInt("page_context_write_time"));
   ...
   *
   * @param response
   *          This json response contains the transaction details.
  ...
  public Transaction getAssociateTransaction(String response) throws Exception {
    ...
    for (int i = 0; i < assTransaction.length(); i++) {
      ...
      associateTransaction.setTransactionId(assTransaction.getJSONObject(i).getString(
          "transaction_id"));
      associateTransaction.setDate(assTransaction.getJSONObject(i).getString("date"));
      associateTransaction.setDebitOrCredit(assTransaction.getJSONObject(i).getString(
          "debit_or_credit"));
      associateTransaction.setTransactionType(assTransaction.getJSONObject(i).getString(
          "transaction_type"));
      // No I18N
      associateTransaction.setAmount(assTransaction.getJSONObject(i).getDouble("amount"));
      associateTransaction.setCustomerId(assTransaction.getJSONObject(i).getString("customer_id"));
      associateTransaction.setCustomerName(assTransaction.getJSONObject(i).getString(
          "customer_name"));
