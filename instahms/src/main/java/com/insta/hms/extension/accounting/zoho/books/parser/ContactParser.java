...
import com.insta.hms.extension.accounting.zoho.books.model.ContactList;
import com.insta.hms.extension.accounting.zoho.books.model.ContactPerson;
import com.insta.hms.extension.accounting.zoho.books.model.ContactPersonList;
...
import com.insta.hms.extension.accounting.zoho.books.model.DefaultTemplate;
import com.insta.hms.extension.accounting.zoho.books.model.Email;
import com.insta.hms.extension.accounting.zoho.books.model.FromEmail;
import com.insta.hms.extension.accounting.zoho.books.model.PageContext;
import com.insta.hms.extension.accounting.zoho.books.model.ToContact;
...
public class ContactParser {
   ...
   *
   * @param response
   *          This json Response contains the contact details.
  ...
  public Contact getContact(String response) throws Exception {
    ...
    // No I18N
    contacts.setOutstandingReceivableAmount(contact.getDouble("outstanding_receivable_amount"));
    contacts.setOutstandingReceivableAmountBcy(contact
        .getDouble("outstanding_receivable_amount_bcy")); // No I18N
    // No I18N
    contacts.setOutstandingPayableAmount(contact.getDouble("outstanding_payable_amount"));
    // No I18N
    contacts.setOutstandingPayableAmountBcy(contact.getDouble("outstanding_payable_amount_bcy"));
    // No I18N
    contacts
        .setUnusedCreditsReceivableAmount(contact.getDouble("unused_credits_receivable_amount"));
    contacts.setUnusedCreditsReceivableAmountBcy(contact
        .getDouble("unused_credits_receivable_amount_bcy"));// No I18N
    // No I18N
    contacts.setUnusedCreditsPayableAmount(contact.getDouble("unused_credits_payable_amount"));
    contacts.setUnusedCreditsPayableAmountBcy(contact
        .getDouble("unused_credits_payable_amount_bcy")); // No I18N
    ...
    for (int i = 0; i < contactPersons.length(); i++) {
      ...
      contactPerson.setContactPersonId(contactPersons.getJSONObject(i).getString(
          "contact_person_id"));
      ...
      contactPerson.setIsPrimaryContact(contactPersons.getJSONObject(i).getBoolean(
          "is_primary_contact")); // No I18N
      if (contactPersons.getJSONObject(i).has("is_added_in_portal")) {
        contactPerson.setAddedInPortal(contactPersons.getJSONObject(i).getBoolean(
            "is_added_in_portal")); // No I18N
      }
      if (contactPersons.getJSONObject(i).has("can_invite")) {
        // No I18N
        contactPerson.setCanInvite(contactPersons.getJSONObject(i).getBoolean("can_invite"));
      }
    ...
    defaultTemplatesObj.setEstimateTemplateName(defaultTemplates
        .getString("estimate_template_name"));
    defaultTemplatesObj.setCreditnoteTemplateId(defaultTemplates
        .getString("creditnote_template_id"));
    defaultTemplatesObj.setCreditnoteTemplateName(defaultTemplates
        .getString("creditnote_template_name"));
    defaultTemplatesObj.setInvoiceEmailTemplateId(defaultTemplates
        .getString("invoice_email_template_id"));
    defaultTemplatesObj.setInvoiceEmailTemplateName(defaultTemplates
        .getString("invoice_email_template_name"));
    defaultTemplatesObj.setEstimateEmailTemplateId(defaultTemplates
        .getString("estimate_email_template_id"));
    defaultTemplatesObj.setEstimateEmailTemplateName(defaultTemplates
        .getString("estimate_email_template_name"));
    defaultTemplatesObj.setCreditnoteEmailTemplateId(defaultTemplates
        .getString("creditnote_email_template_id"));
    defaultTemplatesObj.setCreditnoteEmailTemplateName(defaultTemplates
        .getString("creditnote_email_template_name"));
   ...
   *
   * @param response
   *          This Response contains list of contacts details.
  ...
  public ContactList getContacts(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      contact.setOutstandingReceivableAmount(jsonArray.getJSONObject(i).getDouble(
          "outstanding_receivable_amount")); // No I18N
      contact.setOutstandingPayableAmount(jsonArray.getJSONObject(i).getDouble(
          "outstanding_payable_amount")); // No I18N
      contact.setUnusedCreditsReceivableAmount(jsonArray.getJSONObject(i).getDouble(
          "unused_credits_receivable_amount"));// No I18N
      contact.setUnusedCreditsPayableAmount(jsonArray.getJSONObject(i).getDouble(
          "unused_credits_payable_amount"));// No I18N
      ...
      if (jsonArray.getJSONObject(i).has("is_linked_with_zohocrm")) {
        contact.setLinkedWithZohocrm(jsonArray.getJSONObject(i)
            .getBoolean("is_linked_with_zohocrm")); // No I18N
      }
   ...
   *
   * @param response
   *          This json response contains the contact person details.
   ...
   *
   * @param response
   *          This json response contains list of contact persons details.
  ...
  public ContactPersonList getContactPersons(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      // No I18N
      contactPerson
          .setIsPrimaryContact(jsonArray.getJSONObject(i).getBoolean("is_primary_contact"));
      if (jsonArray.getJSONObject(i).has("is_added_in_portal")) {
        // No I18N
        contactPerson.setAddedInPortal(jsonArray.getJSONObject(i)
            .getBoolean("is_added_in_portal"));
      }
   ...
   *
   * @param response
   *          This json response contains the email content details.
  ...
  public Email getEmailContent(String response) throws Exception {
    ...
    for (int j = 0; j < fromEmails.length(); j++) {
      ...
      if (fromEmails.getJSONObject(j).has("is_org_email_id")) {
        // No I18N
        fromEmail.setOrgEmailId(fromEmails.getJSONObject(j).getBoolean("is_org_email_id"));
      }
   ...
   *
   * @param response
   *          This json response contains list of creditnoteRefunds details for the contact.
  ...
  public CreditnoteRefundList getRefunds(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      creditnoteRefund.setCreditnoteRefundId(jsonArray.getJSONObject(i).getString(
      ...
      creditnoteRefund.setCreditnoteNumber(jsonArray.getJSONObject(i)
          .getString("creditnote_number"));
   ...
   *
   * @param response
   *          This json response contains list of comments details for the contact.
  ...
  public CommentList getComments(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      // No I18N
      comment.setIsEntityDeleted(jsonArray.getJSONObject(i).getBoolean("is_entity_deleted"));
