...
/**
 * BaseCurrencyAdjustmentParser is used to parse the json response and make it into the respective
 * objects.
 */
...
public class BaseCurrencyAdjustmentParser {
   ...
   *
   * @param response
   *          The json response contains the basecurrency details.
  ...
  public BaseCurrencyAdjustment getBaseCurrencyAdjustment(String response) throws Exception {
    ...
    baseCurrencyAdjustment.setBaseCurrencyAdjustmentId(data
        .getString("base_currency_adjustment_id"));
    ...
    for (int i = 0; i < accounts.length(); i++) {
      ...
      // No I18N
      account.setAdjustedBalance(accounts.getJSONObject(i).getDouble("adjustment_balance"));
      account.setAccountId(accounts.getJSONObject(i).getString("account_id"));
      account.setAdjustedBalanceFormatted(accounts.getJSONObject(i).getString(
   ...
   *
   * @param response
   *          This json response contains list of the basecurrencies details.
  ...
  public BaseCurrencyAdjustmentList getBaseCurrencyAdjustments(String response) throws Exception {
    ...
    for (int i = 0; i < bca.length(); i++) {
      ...
      baseCurrencyAdjustment.setBaseCurrencyAdjustmentId(bca.getJSONObject(i).getString(
          "base_currency_adjustment_id"));
      baseCurrencyAdjustment.setAdjustmentDate(bca.getJSONObject(i).getString("adjustment_date"));
      // No I18N
      baseCurrencyAdjustment.setExchangeRate(bca.getJSONObject(i).getDouble("exchange_rate"));
      ...
      // No I18N
      baseCurrencyAdjustment.setGainOrLoss(bca.getJSONObject(i).getDouble("gain_or_loss"));
