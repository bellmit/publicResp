...
public class CustomerPaymentParser {
   ...
   *
   * @param response
   *          This json response contains the customer payment details.
  ...
  public CustomerPayment getCustomerPayment(String response) throws Exception {
    ...
    for (int i = 0; i < invoices.length(); i++) {
      ...
      // No I18N
      invoice.setTaxAmountWithheld(invoices.getJSONObject(i).getDouble("tax_amount_withheld"));
   ...
   *
   * @param response
   *          This json response contains list of customer payments details.
  ...
  public CustomerPaymentList getCustomerPayments(String response) throws Exception {
    ...
    for (int i = 0; i < customerpayments.length(); i++) {
      ...
      customerPayment.setPaymentNumber(customerpayments.getJSONObject(i)
          .getString("payment_number"));
      customerPayment.setInvoiceNumbers(customerpayments.getJSONObject(i).getString(
          "invoice_numbers"));
      ...
      // No I18N
      customerPayment.setBcyAmount(customerpayments.getJSONObject(i).getDouble("bcy_amount"));
      // No I18N
      customerPayment.setUnusedAmount(customerpayments
          .getJSONObject(i).getDouble("unused_amount"));
      customerPayment.setBcyUnusedAmount(customerpayments.getJSONObject(i).getDouble(
          "bcy_unused_amount")); // No I18N
      ...
      customerPayment.setReferenceNumber(customerpayments.getJSONObject(i).getString(
          "reference_number"));
