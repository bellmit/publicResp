...
public class ChartOfAccountParser {
   ...
   *
   * @param response
   *          This json response contains the chartofaccount details.
   ...
   *
   * @param response
   *          This json response contains list of chartofaccounts details.
  ...
  public ChartOfAccountList getChartOfAccounts(String response) throws Exception {
    ...
    for (int i = 0; i < accounts.length(); i++) {
      ...
      // No I18N
      chartofAccount.setIsUserCreated(accounts.getJSONObject(i).getBoolean("is_user_created"));
      chartofAccount.setIsInvolvedInTransaction(accounts.getJSONObject(i).getBoolean(
          "is_involved_in_transaction")); // No I18N
      // No I18N
      chartofAccount.setIsSystemAccount(accounts.getJSONObject(i).getBoolean("is_system_account"));
   ...
   *
   * @param response
   *          This json response contains ist of transactions details.
  ...
  public TransactionList getTransactions(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      transaction.setCategorizedTransactionId(jsonArray.getJSONObject(i).getString(
      ...
      transaction.setTransactionTypeFormatted(jsonArray.getJSONObject(i).getString(
      ...
      if (!jsonArray.getJSONObject(i).getString("debit_amount").equals("")) {
        // No I18N
        transaction.setDebitAmount(jsonArray.getJSONObject(i).getDouble("debit_amount"));
      }
      if (!jsonArray.getJSONObject(i).getString("credit_amount").equals("")) {
        // No I18N
        transaction.setCreditAmount(jsonArray.getJSONObject(i).getDouble("credit_amount"));
      }
