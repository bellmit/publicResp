...
import com.insta.hms.extension.accounting.zoho.books.model.CustomField;
import com.insta.hms.extension.accounting.zoho.books.model.Email;
import com.insta.hms.extension.accounting.zoho.books.model.Emailtemplate;
...
import com.insta.hms.extension.accounting.zoho.books.model.Payment;
import com.insta.hms.extension.accounting.zoho.books.model.PaymentGateway;
...
import com.insta.hms.extension.accounting.zoho.books.model.Template;
...
public class InvoiceParser {
   ...
   *
   * @param response
   *          This json respone contains list of invoices details.
  ...
  public InvoiceList getInvoices(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      if (jsonArray.getJSONObject(i).has("is_viewed_by_client")) {
        // No I18N
        inv.setViewedByClient(jsonArray.getJSONObject(i).getBoolean("is_viewed_by_client"));
      }
   ...
   *
   * @param response
   *          This json respone contains the invoice details.
  ...
  public Invoice getInvoice(String response) throws Exception {
    ...
    // No I18N
    invoice.setPaymentReminderEnabled(invoiceDetails.getBoolean("payment_reminder_enabled"));
    ...
    // No I18N
    invoice.setAllowPartialPayments(invoiceDetails.getBoolean("allow_partial_payments"));
    ...
    for (int l = 0; l < paymentGateways.length(); l++) {
      ...
      if (paymentGateways.getJSONObject(l).has("additional_field1")) {
        paymentGateway.setAdditionalField1(paymentGateways.getJSONObject(l).getString(
            "additional_field1"));
      }
      if (paymentGateways.getJSONObject(l).has("configured")) {
        // No I18N
        paymentGateway.setConfigured(paymentGateways.getJSONObject(l).getBoolean("configured"));
      }
    ...
    if (invoiceDetails.has("stop_reminder_until_payment_expected_date")) {
      invoice.setStopReminderUntilPaymentExpectedDate(invoiceDetails
          .getBoolean("stop_reminder_until_payment_expected_date"));// No I18N
    }
   ...
   *
   * @param response
   *          This json response contains the email content details for invoice.
  ...
  public Email getEmailContent(String response) throws Exception {
    ...
    if (data.has("emailtemplates")) {
      ...
      for (int i = 0; i < emailtemplates.length(); i++) {
        ...
        // No I18N
        emailtemplate.setSelected(emailtemplates.getJSONObject(i).getBoolean("selected"));
        emailtemplate.setName(emailtemplates.getJSONObject(i).getString("name"));
        emailtemplate.setEmailTemplateId(emailtemplates.getJSONObject(i).getString(
            "email_template_id"));
    ...
    for (int k = 0; k < fromEmails.length(); k++) {
      ...
      if (fromEmails.getJSONObject(k).has("is_org_email_id")) {
        // No I18N
        fromEmail.setOrgEmailId(fromEmails.getJSONObject(k).getBoolean("is_org_email_id"));
      }
   ...
   *
   * @param response
   *          This json response contains list of payments details for invoice.
  ...
  public PaymentList getPayments(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      // No I18N
      payment.setTaxAmountWithheld(jsonArray.getJSONObject(i).getDouble("tax_amount_withheld"));
      payment.setIsSingleInvoicePayment(jsonArray.getJSONObject(i).getBoolean(
          "is_single_invoice_payment")); // No I18N
   ...
   *
   * @param response
   *          This json response contains list of templates details for invoice.
   ...
   *
   * @param response
   *          This json response contains list of comments details for invoice.
   ...
   *
   * @param response
   *          This json response contains the comment details for invoice.
   ...
   *
   * @param response
   *          This json response contains list of credits details for invoice.
  ...
  public CreditList getCreditsApplied(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      credit
          .setCreditnotesInvoiceId(jsonArray.getJSONObject(i).getString("creditnotes_invoice_id"));
   ...
   *
   * @param response
   *          This json response contains the credit details for invoice.
  ...
  public Credit getApplyCredits(String response) throws Exception {
    ...
    for (int i = 0; i < invoicePayments.length(); i++) {
      ...
      invoicePayment.setInvoicePaymentId(invoicePayments.getJSONObject(i).getString(
          "invoice_payment_id"));
      ...
      // No I18N
      invoicePayment.setAmountUsed(invoicePayments.getJSONObject(i).getDouble("amount_used"));
    ...
    for (int j = 0; j < applyCreditnotes.length(); j++) {
      ...
      applyCreditnote.setCreditnotesInvoiceId(applyCreditnotes.getJSONObject(j).getString(
      ...
      applyCreditnote.setAmountApplied(applyCreditnotes.getJSONObject(j)
          .getDouble("amount_applied")); // No I18N
