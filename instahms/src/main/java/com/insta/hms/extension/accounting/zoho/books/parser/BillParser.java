...
public class BillParser {
   ...
   *
   * @param response
   *          This json response contains the bill details.
  ...
  public Bill getBill(String response) throws Exception {
    ...
    // No I18N
    bills.setUnusedCreditsPayableAmount(bill.getDouble("unused_credits_payable_amount"));
    ...
    for (int k = 0; k < payments.length(); k++) {
      ...
      payment.setPaidThroughAccountId(payments.getJSONObject(k)
          .getString("paid_through_account_id"));
      payment.setPaidThroughAccountName(payments.getJSONObject(k).getString(
          "paid_through_account_name"));
      // No I18N
      payment
          .setIsSingleBillPayment(payments.getJSONObject(k).getBoolean("is_single_bill_payment"));
   ...
   *
   * @param response
   *          This json response contains list of bills details.
   ...
   *
   * @param response
   *          This json response contains list of payments details of the bill.
  ...
  public PaymentList getPayments(String response) throws Exception {
    ...
    for (int i = 0; i < jsonArray.length(); i++) {
      ...
      payment.setIsSingleBillPayment(jsonArray.getJSONObject(i)
          .getBoolean("is_single_bill_payment")); // No I18N
   ...
   *
   * @param response
   *          This json response contains list of comments details of the bill.
   ...
   *
   * @param response
   *          This json response contains the comment details of the bill.
