...
/**
 * The Class SampleRegistrationForm.
 *
 * @author krishna
 */
public class SampleRegistrationForm extends ActionForm {
  ...
  public List getAd_test_file_upload() {
    // delete the disabled file upload elements from the actual list.
    int len = this.ad_test_file_upload.size();

    for (int i = 0; i < len && rowIndexes.size() > 0; i++) {
      if (!rowIndexes.contains(i))
        this.ad_test_file_upload.remove(i);
    }
    rowIndexes.clear();
  ...
  /** The row indexes. */
  private List rowIndexes = new ArrayList();
   ...
   * @param file  the file
   */
  // this method doesnt get called when input file is disabled.
  public void setAd_test_file_upload(int index, FormFile file) {
    // formfile elements doesn't come in the order mentioned in jsp,
    // we will get randomly, first we may get 10th element then 20th and then 1st etc.,
    // so first add null values if the index is greater than the elements of the list.
    // when we get actual indexed element, replace the null with element.

    // another case: in incoming sample registraton if any of the test which needs document marked
    // for delete then
    // we are disabling the file element. This index has to be filled up with the null value.
