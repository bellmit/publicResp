...
public class NumberToStringConversionIndoLang {
   ...
   */
  public NumberToStringConversionIndoLang(String one, String ten, String eleven, String hundred) {
   ...
   */
  public String toWord(int amount) {
  ...
  public String toRupeesPaise(BigDecimal amount) throws SQLException {
    ...
    amount = amount.abs();
    int rupeeValue = amount.intValue();
    int paiseValue = amount.multiply(new BigDecimal(paiseDividedBy)).intValue() % paiseDividedBy;

    String strRupees = dto.getWhole() + " " + toWord(rupeeValue);
    String strPaise = "";
    if (paiseValue > 0) {
      strPaise = this.sep + dto.getDecimal() + " " + toWord(paiseValue);
    }
