...
public class InventoryReturnsAccountingDataInsertServiceTest extends
  ...
  public void init(Method testMethod) throws IOException {
    ...
    ReflectionTestUtils.setField(invReturnsAccountingDataInsertService, "accountingDataInsertRepo",
        accountingDataInsertRepo);
    ReflectionTestUtils.setField(invReturnsAccountingDataInsertService, "genPrefService",
        genPrefService);
  ...
  @Test
  public void inventoryReturnsInsuranceBillAccountingDataTest() {
    ...
    BillChargeModel charge = (BillChargeModel) accountingDataInsertRepo.get(BillChargeModel.class,
        "CH259819");
    StoreIssueReturnsDetailsModel storeIssueReturnsDetailsModel = (StoreIssueReturnsDetailsModel) accountingDataInsertRepo
        .load(StoreIssueReturnsDetailsModel.class, 140);
    ...
    for (HmsAccountingInfoModel accModel : resultList) {
      if (accModel.getDebitAccount().equals(
          getInventoryChargeAccountHeadName(charge, storeItemDetails))
          && accModel.getCreditAccount().equals(accountingDataInsertRepo.getLedgerDefinition("ACCOUNT_TYPE_COUNTER_RECEIPTS"))
          && accModel.getChargeReferenceId().equals("CH259819")) {
        accountingModel = accModel;
        break;
      }
    }
    assertSponsorBillPatientAmountAccountingEntry(accountingModel, billModel, charge,
        storeIssueReturnsDetailsModel, storeItemDetails);
    ...
    for (HmsAccountingInfoModel accModel : resultList) {
      if (accModel.getChargeReferenceId().equals("CH259819")
          && accModel.getPrimaryId().equals("507")) {
        accountingModel = accModel;
      }
    }
    assertSponsorBillPatientTaxAmountAccountingEntry(accountingModel, billModel, charge,
        bcTaxModel, storeIssueReturnsDetailsModel, storeItemDetails, chargeTaxIdClaimTaxMap);
  }
  ...
  private void assertSponsorBillPatientAmountAccountingEntry(
    ...
    assertTrue(accountingModel.getSalesVatAmount().compareTo(BigDecimal.ZERO) == 0);
    assertTrue(accountingModel.getSalesVatPercent().compareTo(BigDecimal.ZERO) == 0);
    ...
    assertTrue(accountingModel.getQuantity().compareTo(storeIssueReturnsDetailsModel.getQty()) == 0);
    ...
    assertEquals(accountingModel.getIssueStore(), getIssueStore(storeIssueReturnsDetailsModel));
    assertEquals(accountingModel.getIssueStoreCenter(),
        getIssueStoreCenterName(storeIssueReturnsDetailsModel));
    ...
    assertTrue(accountingModel.getCostAmount().compareTo(
        storeIssueReturnsDetailsModel.getCostValue() == null ? BigDecimal.ZERO
            : storeIssueReturnsDetailsModel.getCostValue()) == 0);
    ...
    assertTrue(accountingModel.getGrossAmount().compareTo(grossPatAmount.negate()) == 0);
    assertTrue(accountingModel.getNetAmount().compareTo(netPatAmount.negate()) == 0);
  ...
  private void assertSponsorBillPatientTaxAmountAccountingEntry(
      HmsAccountingInfoModel accountingModel, BillModel billModel, BillChargeModel charge,
      BillChargeTaxModel bcTax, StoreIssueReturnsDetailsModel storeIssueReturnsDetailsModel,
    ...
    assertTrue(accountingModel.getGrossAmount().compareTo(patTaxAmount.negate()) == 0);
    assertTrue(accountingModel.getNetAmount().compareTo(patTaxAmount.negate()) == 0);
    assertEquals(accountingModel.getTransactionType(), AccountingConstants.TRANSACTION_TYPE_N);
    assertEquals(accountingModel.getDebitAccount(),
  ...
  private String getIssueStoreCenterName(StoreIssueReturnsDetailsModel storeIssueReturnsDetailsModel) {
    if (storeIssueReturnsDetailsModel != null
        && storeIssueReturnsDetailsModel.getStoreIssueReturnsMainModel() != null
        && storeIssueReturnsDetailsModel.getStoreIssueReturnsMainModel().getStoresModel() != null
