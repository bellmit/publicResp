...
<head>
<title>Patient Visit Trends - Insta HMS</title>
...
<body>
	<div align="center" style="margin-top: 1em; margin-bottom: 0.5em">
		<div class="heading">${trendDisplay[trendPeriod]} Patient Visit Stats Trend: by ${groupByName}</div>
	...
	<table align="center">
			...
			<td>
				<table class="report" border="0" align="center">
					...
					<#list categories as cat>
						<tr>
							<#if cat != "_total">
								<th rowspan="3">${cat?html}</th>
								<th>N</th>
								<#list periods as period>
									<td class="number">${((result[cat]!empty)[period]!empty).N!0}</td>
								</#list>
							</#if>
						</tr>
						...
						<tr>
							<#if cat != "_total">
								<th>R</th>
								<#list periods as period>
									<td class="number">${((result[cat]!empty)[period]!empty).Y!0}</td>
								</#list>
							</#if>
						</tr>
					...
					<tr class="total">
						...
						<th>N</th>
						<#list periods as period>
							<td class="number">${(result._total[period]!empty).N!0}</td>
						</#list>
					</tr>
					...
					<tr class="total">
						<th>R</th>
						<#list periods as period>
							<td class="number">${(result._total[period]!empty).Y!0}</td>
						</#list>
					</tr>
		...
		<tr>
			<td>
				<table align="left">
					<tr>
						<td>Legend:</td><td></td><td>N=New</td><td></td><td>R=Revisit</td><td></td><td>T=Total</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<#if format == "screen">
			<div align="center" style="margin-top: 2em">
				<form method="GET" action="" target="_blank">
					...
					<input type="hidden" name="method" value="visitTrendReport"/>
