...
[#assign prescDoctorAsOrderingClinicianingClinician = healthHAADPref]
	<Claim>
		...
		[#if use_drg == "Y"] [#assign patient_share = claim.deduction!0] [#assign net = gross - patient_share] [/#if]
		<Gross>${gross!0}</Gross>
		...
		<Encounter>
			...
			[#if claim.encounter_end_type?? && (claim.encounter_start_type == 4 || claim.encounter_start_type == 7) && claim.destination_service_regn_no?? && claim.destination_service_regn_no != ""]<TransferDestination>${claim.destination_service_regn_no!""}</TransferDestination>[/#if]
		</Encounter>
		...
		[#list charges as charge]
			<Activity>
				<ID>${charge.activity_charge_id!""}[#if claim.is_resubmission?? && claim.is_resubmission = "Y" && claim.resubmission_type="correction"]-${claim.last_submission_batch_id!""}[#else][/#if]</ID>
		 ...
		 [#else]<Start>${charge.posted_date!""}</Start>[/#if]
				...
				[#if charge.is_drg_charge == 'Y'] [#assign chnet = charge.net!0][#assign chnet = chnet - patient_share]
				<Net>${chnet!0}</Net> [#else] <Net>${charge.net!0}</Net> [/#if]
