...
[#assign prescDoctorAsOrderingClinicianingClinician = healthHAADPref]
	<Claim>
		...
		<Encounter>
			...
			[#if claim.eligibility_reference_number?? && claim.eligibility_reference_number != ""]<EligibilityIDPayer>${claim.eligibility_reference_number}</EligibilityIDPayer>[/#if]
		...
		[#list charges as charge]
		 ...
		 [#else]<Start>${charge.activity_start_datetime!""}</Start>[/#if]
				...
				[#if claim.require_pbm_authorization?? && claim.require_pbm_authorization == "Y" ]
					<Quantity>${(charge.quantity/charge.issue_base_unit)?string("0.0000")}</Quantity>
				[#else]
			...
			[#elseif prior_auth_id?? && prior_auth_id != "0" && prior_auth_id != ""]<PriorAuthorizationID>${prior_auth_id!""}</PriorAuthorizationID>[/#if]
				<VAT>${charge.vat_net!0}</VAT>
				<VATPercent>${charge.vat_tax_rate!0}</VATPercent>
