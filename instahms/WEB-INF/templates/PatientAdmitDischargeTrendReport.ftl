...
<head>
<title>Admit / Discharge Trends - Insta HMS</title>
...
<body>
	<div align="center" style="margin-top: 1em; margin-bottom: 0.5em">
		<div class="heading">${trendDisplay[trendPeriod]} Patient Admission Discharge Stats Trend: by ${groupByName}</div>
	...
	<table align="center">
			...
			<td>
				<table class="report" border="0" align="center">
					...
					<#list categories as cat>
						<tr>
							<#if cat != "_total">
								<th rowspan="2">${cat?html}</th>
								<th>A</th>
								<#list periods as period>
									<td class="number">${((result[cat]!empty)[period]!empty).AD!0}</td>
								</#list>
							</#if>
						</tr>
						...
						<tr>
							<#if cat != "_total">
								<th>D</th>
								<#list periods as period>
									<td class="number">${((result[cat]!empty)[period]!empty).DI!0}</td>
								</#list>
							</#if>
						</tr>
					</#list>
					...
					<tr class="total">
						...
						<th>A</th>
						<#list periods as period>
							<td class="number">${(result._total[period]!empty).AD!0}</td>
					...
					<tr class="total">
						...
						<#list periods as period>
							...
							<td class="number">${(result._total[period]!empty).DI!0}</td>
						</#list>
					</tr>
		...
		<tr>
			<td>
				<table align="left">
					<tr>
						<td>Legend:</td><td></td><td>A=Admission</td><td></td><td>D=Discharge</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<#if format == "screen">
			<div align="center" style="margin-top: 2em">
				<form method="GET" action="" target="_blank">
					...
					<input type="hidden" name="method" value="admitDischargeTrendReport"/>
