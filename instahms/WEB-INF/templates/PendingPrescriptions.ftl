	...
	<tr>
		<td>
			<table cellspacing='0' cellpadding='1' style='margin-top: 10px;' width="100%" >
				...
				<!-- [#list prescriptions as presc] -->
				<tr>
					<td >${(presc.item_name!)?html}</td>
					<td >${(presc.item_remarks!)?html}</td>
					<td>${presc.service_qty!1}</td>
					<td>[#if presc.tooth_unv_number?has_content]
							${presc.tooth_unv_number}
						[#elseif presc.tooth_fdi_number?has_content]
