...
<c:when test="${type eq 'a'}"> <%-- autocomplete --%>
	<input type="text" name="${fieldName}" id="${fieldName}" value="${param[fieldName]}">
...
<c:when test="${type eq 'c'}"> <%-- checkbox --%>
	<insta:checkgroup name="${fieldName}" 
	opvalues="${opvalues}" optexts="${optexts}" selValues="${paramValues[fieldName]}"/>
	<input type="hidden" name="${fieldName}@op" value="in" />
</c:when>
<c:when test="${type eq 't'}"> <%-- text field --%>
	<input type="text" name="${fieldName}" value="${param[fieldName]}" />
</c:when>
<c:when test="${type eq 'selectdb'}"> <%-- select box from database --%>
	<insta:selectdb name="${fieldName}" 
