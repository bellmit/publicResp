...
<%@ attribute name="dataList" required="true" type="com.insta.hms.common.PagedList" %>
...
<c:set var="sortables" value="${null != sortableColumns ? fn:split(sortableColumns, ',') : ''}"/>
<c:set var="keyArray" value="${fn:split(keyField, ',')}"/>
<c:if test="${empty statusFlags}">
	<c:set var="statusFlags" value="I:grey_flag.gif,A:empty_flag.gif,C:red_flag.gif,X:yellow_flag.gif,F:blue_flag.gif,P:yellow_flag.gif"/>
...
<insta:paginate curPage="${dataList.pageNumber}" numPages="${dataList.numPages}" totalRecords="${dataList.totalRecords}"/>
	...
	<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
		<tr onmouseover="hideToolBar();">
			...
			<c:forEach items="${colArray}" varStatus="st">
					...
					<c:set var="displayName" value="${(null == colParts) ? '' : 
		...
		<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
			onclick="showToolbar(${st.index}, event, 'resultTable',
				{${keyArray[0]}: '${record[keyArray[0]]}','${keyArray[1]}': '${record[keyArray[1]]}'},'');" id="toolbarRow${st.index}">
			...
			<td class="${showToolbar == 'N' ? 'noToolbar' : ''}">
				...
				<c:forEach items="${flagArray}" var="sflag" varStatus="t">
					...
					<c:if test="${record[statusCol] eq flagParts[0]}"><img src='${cpath}/images/${flagParts[1]}'> </c:if>
				</c:forEach>
				</c:if>${ifn:cleanHtml(record[fieldName])}</td>
			</c:forEach>
		</tr>
		</c:forEach>
