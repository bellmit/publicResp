...
<c:set var="cb"><fmt:message key='insta.ui.commit.hash'/></c:set>
<c:set var="stylesDir" value="${pageDirection == 'rtl' ? 'cssrtl' : 'css'}"/>
<c:set var="typeDir">
	<c:choose>
		<c:when test="${type eq 'css' || type eq 'style'}">styles</c:when>
		<c:when test="${type eq 'js' || type eq 'script'}">ui</c:when>
