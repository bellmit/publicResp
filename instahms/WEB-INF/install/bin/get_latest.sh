...
date_echo "---- Starting get_latest.sh -----"
#
# Get down the list of builds available to us
#
cd $HMS_DIST_HOME/builds
sudo /usr/bin/rsync -az $HUBVPN::$HOST/builds.list .
[ $? -ne 0 ] && echo "Unable to get the builds list: $?" && exit 1

#
# rsync any builds that we don't have
#
for build in `cat builds.list` ; do
	...
	# do the sync
	sudo /usr/bin/rsync -az --delete $HUBVPN::builds/$build .
...
for build in * ; do
	...
	[ $build == 'builds.list' ] && continue;

	grep -q $build builds.list
	if [ $? -ne 0 ] ; then
