...
my $pidfile = "/tmp/hl7_service_in_$opt_db.$opt_schema.pid";
...
my $serviceImp = Insta::ServiceImport->new( {
...
sub readConfig {
	...
	$interfaces = $dbh->selectall_hashref(
		"SELECT * FROM hl7_lab_interfaces WHERE status='A' AND coalesce(results_import_dir, '') != '' AND interface_type='SERVICE'",
		'interface_name');
...
sub processInterface {
	...
	while ($file = readdir(DIR)) {
		...
		eval {
			...
			$errMsg = $serviceImp->processMessage($interfaces->{$ifName}, $msg);
		};
		if ($@) {
			$log->error($@);
			$errMsg = $@;
