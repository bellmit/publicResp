...
CONTENT="/tmp/streamingcheck.out"
MESSAGE="/tmp/streamingcheck.msg"
echo > $CONTENT
...
if [ $wal_check == "hot_standby" ]; then
    psql -c  "select client_addr, state, sent_location, write_location,flush_location, replay_location from pg_stat_replication" -o "/tmp/streamingcheck.out" 
