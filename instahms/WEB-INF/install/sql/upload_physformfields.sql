...
DROP TABLE IF EXISTS tmp_phys_forms;
CREATE TABLE tmp_phys_forms (
	form_name text NOT NULL,
	field_name text NOT NULL,
	field_type text, allow_others text, allow_normal text, normal_text text, allow_description text,
...
COPY tmp_phys_forms FROM '/tmp/masters/physforms_fields.csv' csv header;
ALTER TABLE tmp_phys_forms ADD COLUMN field_id integer;
ALTER TABLE tmp_phys_forms ADD COLUMN form_id integer;

UPDATE tmp_phys_forms SET form_id=pfd.form_id from physician_form_desc pfd
WHERE upper(trim(pfd.form_title))=upper(trim(tmp_phys_forms.form_name));

UPDATE tmp_phys_forms SET
	...
	allow_description = clean_yn(allow_description, 'Y');


DELETE FROM physician_form_field_desc;
ALTER SEQUENCE physician_form_field_desc_seq RESTART 1;

DELETE FROM physician_form_field_options;
ALTER SEQUENCE physician_form_field_options_seq RESTART 1;

DROP SEQUENCE IF EXISTS tmp_display_order_seq;
CREATE SEQUENCE tmp_display_order_seq;

INSERT INTO physician_form_field_desc (field_id, form_id, field_name, display_order,
	field_type, allow_others, allow_normal, normal_text, allow_description, no_of_lines, status)
SELECT nextval('physician_form_field_desc_seq'), form_id, field_name, nextval('tmp_display_order_seq'),
	field_type, allow_others, allow_normal, coalesce(normal_text, ''), allow_description, no_of_lines, 'A'
FROM tmp_phys_forms;

UPDATE tmp_phys_forms t SET field_id =
