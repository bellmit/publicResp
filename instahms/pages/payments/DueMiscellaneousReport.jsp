...
<head>
	<title>Due Miscellaneous Report - Insta HMS</title>
	<script>
		function onInit() {
		...
		function onSubmit(option) {
			...
			if (option == 'pdf') {
				if ( document.inputform.printerType.value == 'text' ) {
					...
					document.inputform.method.value = 'printDueMiscellaneous';
				}
...
<html>
	<body onload="onInit()">
		<div class="pageHeader">Miscellaneous Dues Report</div>
		<form name="inputform" method="GET" action="${cpath}/payment/DueMiscellaneousReports.do">
			<input type="hidden" name="method" value="printDueMiscellaneous">
			<input type="hidden" name="methodPdf" value="printDueMiscellaneous">
