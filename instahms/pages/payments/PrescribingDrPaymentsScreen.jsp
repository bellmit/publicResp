...
<html>
	<head>
		<title>Prescribing Doctor Payments - Insta HMS</title>
		...
		<insta:link type="script" file="/payments/prescribingdrpayments.js"/>
	...
	<c:set var="methodName" value="prescribingDoctorSearch"/>
...
<div class="pageHeader">Prescribing Doctor ${ifn:cleanHtml(screen)}</div>
<c:set var="actionUrl"
		value="/pages/payments/PrescribingDrPayment.do"/>
<form method=GET" action="${cpath}${actionUrl}" name="PresDoctorPaymentSearchForm" >
...
<div>
	<insta:search form="PresDoctorPaymentSearchForm" optionsId="optionalFilter" closed="${hasResult}"
			validateFunction="getDoctorCharges()" >
		<div class="searchBasicOpts">
			<div class="sboField">
				<div class="sboFieldLabel">Prescribing Doctor:</div>
					<div class="sboFieldInput">
						<div id="doctorsDiv" class="autoComplete">
			...
			<input type="hidden" name="prescribing_dr_id" id="doctor" value="${ifn:cleanHtmlAttribute(param.prescribing_dr_id)}"/>
		</div>
				...
				<tr>
					...
					<td class="last">
						<div class="sfLabel">Charge Head:</div>
							<div class="sfField">
								<insta:selectdb name="chargehead_id" table="chargehead_constants" dummyvalue="-- Select --"	valuecol="chargehead_id" displaycol="chargehead_name" value="${param.chargehead_id}" filtered="false" onchange="getCharges(this.value);"/>
							</div>
...
<form name="prescribingDrPaymentForm" action="${cpath}${actionUrl}" method="POST">
	<input type="hidden" name="_method" value="createPrescribedDrPaymentDetails" id="method"/>
	...
	<input type="hidden" name="prescribing_dr_id" value="${ifn:cleanHtmlAttribute(param.prescribing_dr_id)}"/>
	...
	<input type="hidden" name="_prescribing_dr_amount" value="${ifn:cleanHtmlAttribute(param.prescribing_dr_amount)}"/>
	<input type="hidden" name="act_description" value="${ifn:cleanHtmlAttribute(param.act_description)}"/>

	<insta:paginate curPage="${chargeList.pageNumber}" numPages="${chargeList.numPages}"
		pageNumParam="_chargePageNum" totalRecords="${chargeList.totalRecords}"/>
	<div class="resultList" id="payment_content" >
	<table class="resultList" width="100%" cellspacing="0" cellpadding="0" align="center" id="payResultTable" onmouseover="hideToolBar();">
		...
		<c:forEach items="${charge}" var="dc" varStatus="st">
			...
			<tr class="${st.index == 0 ? 'firstRow':''} ${st.index % 2 == 0 ? 'even' : 'odd'}" title="${title}"
				 onclick="showToolbar(${st.index}, event, 'payResultTable', {billNo: '${dc.bill_no}'})"
				onmouseover="hideToolBar(${st.index})" id="toolbarRow${st.index}">
			<td>
				<input type="checkbox" name="statusCheck" value="${i}" id="statusCheck${i}"
					onclick="editDoctorFees(this,${i})" />
			</td>
			<td>${dc.mr_no}
			...
			<td>${dc.patient_name}&nbsp;${dc.last_name}</td>
			...
			<c:if test="${dc.discount_auth_ref != null && dc.ref_discount_amt > 0}">
				<c:set var="title">
						${title}
						Referral Doctor : ${dc.discount_auth_ref_name}--${dc.ref_discount_amt}
				</c:set>
			</c:if>
			<c:if test="${dc.discount_auth_pres_dr != null && dc.pres_dr_discount_amt > 0}">
				<c:set var="title">
						${title}
						Prescribing Doctor : ${dc.discount_auth_pres_dr_name}--${dc.pres_dr_discount_amt}
				</c:set>
			</c:if>
			<c:if test="${dc.discount_auth_hosp != null && dc.hosp_discount_amt > 0}">
				<c:set var="title">
					${title}
					Hospital User: ${dc.overall_discount_auth_name} --${dc.hosp_discount_amt} </c:set>
			</c:if>
			<td class="number"><label title="${title}">${dc.discount}</label></td>
			<td class="number">
				<label id="doctorFee">${dc.prescribing_dr_amount}</label>
				<input type="hidden" name="_doctorFees" value="${dc.prescribing_dr_amount}"	id="doctorFees${i}" />
				<input type="hidden" id="originalDoctorFees${i}" value="${dc.prescribing_dr_amount}"/>
	...
	<c:if test="${param._method != 'getPaymentDues'}">
	...
	<table width="100%" style="padding-top: 10">
		<tr>
			<td>
				...
				<div style="float: left;">
					<input type="radio" name="_selectItems" id="allItems" value="all"
							onclick="onCheckRadio(this.value);">	Select All Items
				</div>
...
<dl class="accordion">
	...
	<dd class="${not empty doctorpayment ? 'open':' '}" style="width:952px">
		...
		<div id="viewDiv" class="resultList" style="width:100%">
				<table class="resultList" cellpadding="0" cellspacing="0" width="100%" id="paidResultTable"
					onmouseover="hideToolBar('');">
					 <tr onmouseover="hideToolBar('')";>
							...
							<th class="number">Billed Amt</th>
							<th class="number">Discount</th>
							<th class="number">Pres Doctor Fees</th>
					</tr>
					...
					<tr class="${st.index == 0 ? 'firstRow':''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
						onclick="showToolbar(${st.index+10}${j},event,'paidResultTable',{billNo:'${dp.bill_no}'})"
							...
							<td>
									...
									<input type="hidden" name="_delPaymentId" value="${dp.prescribing_dr_payment_id}"/>
							</td>
							...
							<td class="number">${dp.amount}
									...
									<c:set var="title" value=""/>
										...
										<c:if test="${dp.discount_auth_dr != null && dp.dr_discount_amt > 0}">
												<c:set var="title">
													${title}
													Doctor Discount :${dp.discount_auth_dr_name}--${dp.dr_discount_amt}
												</c:set>
										</c:if>
										<c:if test="${dp.discount_auth_ref != null && dp.ref_discount_amt > 0}">
												<c:set var="title">
														${title}
														Referral Doctor : ${dp.discount_auth_ref_name}--${dp.ref_discount_amt}
												</c:set>
										</c:if>
										<c:if test="${dp.discount_auth_pres_dr != null && dp.pres_dr_discount_amt > 0}">
												<c:set var="title">
														${title}
														Prescribing Doctor : ${dp.discount_auth_pres_dr_name}--${dp.pres_dr_discount_amt}
												</c:set>
										</c:if>
										<c:if test="${dp.discount_auth_hosp != null && dp.hosp_discount_amt > 0}">
												<c:set var="title">
														${title}
														Hospital User: ${dp.overall_discount_auth_name} --${dp.hosp_discount_amt}
												</c:set>
										</c:if>
							<td class="number">
							...
							<td class="number">${dp.doctor_paid_amount}
									<input type="hidden" name="_deldoctorFees" value="${dp.doctor_paid_amount}" />
							</td>
					</tr>
					</c:forEach>
				</table>
				...
				<table align="right" style="padding-top: 10">
					<tr>
						<td>Total Amount:&nbsp;</td>
						<td>
							<label id="paidAmount" style="font-weight: bold;">${presDrPaidAmount}</label>
						</td>
					</tr>
				</table>
...
<div class="legend" style="display:${not empty charge or not empty doctorpayment?'block':'none'}">
