...
<title>Rate Plans - Insta HMS</title>
	...
	<script type="text/javascript">
		var toolBar = {
			Edit : {
				...
				href : "/pages/masters/ratePlan.do?_method=showRatePlanDetails",
				onclick : null,
...
<form name="RatePlanForm" method="GET">
	...
	<input type="hidden" name="_method" value="getRatePlanDetails"/>
	<input type="hidden" name="_searchMethod" value="getRatePlanDetails"/>
	...
	<insta:search-lessoptions form="RatePlanForm" >
			<div class="searchBasicOpts" >
				<div class="sboField">
					<div class="sboFieldLabel">Rate Plan Name</div>
				...
				<div class="sboField" style="height:69">
					...
					<div class="sboFieldInput">
						<insta:selectdb name="base_rate_sheet_id" id="base_rate_sheet_id" value="${param.base_rate_sheet_id}"
							...
							orderby="org_name" dummyvalue="---Select---" dummyvalueId=""/>
					</div>
				</div>
			</div>
	...
	<div class="resultList" >
		...
		<tr onmouseover="hideToolBar();">
			<th>#</th>
			<insta:sortablecolumn name="org_name" title="Rate Plan Name"/>
			<c:if test="${mod_reward_points}">
		...
		<c:forEach var="record" items="${pagedList.dtoList}" varStatus="st">
			<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}" id="toolbarRow${st.index}"
				onclick="showToolbar(${st.index}, event, 'resultTable', {org_id: '${record.org_id}'},'');">
		...
		<c:url var="Url" value="ratePlan.do">
			<c:param name="_method" value="addRatePlanDetails"/>
		</c:url>

		<div class="screenActions" style="float: left;">
			<a href="${Url}">Add New Rate Plan</a>
		</div>
