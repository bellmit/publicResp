...
<%@page import="com.bob.hms.common.AutoIncrementId,java.util.ArrayList,java.util.Iterator" %>
	<html>
	<head>
	<title>Search Old MRNo - Insta HMS</title>
	...
	function getMrnoList(){
	  ...
	  for(var i =0;i<document.forms[0].condition.length;i++){
    	...
    	if(reqObject) {
	    	...
	    	var url="../../pages/Common/MrnoSearchPopup.do?method=getOldMrNoSearchResults&searchType="+condition+"&searchVal="+document.forms[0].conditionValue.value.toUpperCase()+"&index=${ifn:cleanURL(index)}";
 	...
 	function populateMrnoGrid() {
 		var len=x.childNodes[0].childNodes.length;
 	 	var resultDivHtml = '<table width="100%" cellpadding=0 cellspacing=0 border=1  style="font-family:verdana;font-size:14;font-weight:bold;color:darkgreen" >'
                         			+'<tr style="font-size:12;font-weight:bold;background:#8FBC8F;color:black"> <td width="5%">&nbsp;SNo</td><td width="20%" align="center">OLD&nbsp;MRNO</td><td width="20%" align="center">MRNO</td><td width ="40%" align="center">PATIENT NAME</TD><td width="15%" align="center">Mobile No/Phone No</td></tr>';
 		...
 		if(len ==0){
	    	...
	    	for(var i=0;i<len;i++){
	    		resultDivHtml+='<tr ><td width="5%" height="20px">&nbsp;'+(i+1)+'</td><td width="20%" height="25px">&nbsp;<a href="javascript:setMrno(\''+nodes[i].attributes.getNamedItem('class2').nodeValue+'\')" style="color:darkgreen">'+nodes[i].attributes.getNamedItem('class2').nodeValue+'</a></td><td width="20%" height="25px">&nbsp;'+nodes[i].attributes.getNamedItem('class1').nodeValue+'</a></td><td width ="40%" style="font-size:13">&nbsp;'+nodes[i].attributes.getNamedItem('class3').nodeValue+'</TD><td width="15%" style="font-size:13">&nbsp;'+nodes[i].attributes.getNamedItem('class4').nodeValue+'</td></tr>';
	    	}
		}
	    resultDivHtml+="</table>";
