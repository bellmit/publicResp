...
<head>
	...
	<title>Daily Treatment - Insta HMS</title>
	...
	<div class="pageHeader">${ifn:cleanHtml(param._method)=='add'?'Add':Edit} Daily Treatment</div>
	<insta:feedback-panel/>
	<insta:patientgeneraldetails  mrno="${param.mr_no}" addExtraFields="true"/>
	<form name="treatmentForm" method="post" action="${cpath}/IVF/IVFDailyTreatment.do" autocomplete="off">
	<input type="hidden" name="ivf_cycle_id" value="${ifn:cleanHtmlAttribute(param.ivf_cycle_id)}"/>
	<input type="hidden" name="ivf_cycle_daily_id" value="${dailyTreatDetails.ivf_cycle_daily_id}"/>
	...
	<input type="hidden" name="patient_id" id="patient_id" value="${ifn:cleanHtmlAttribute(param.patient_id)}" />
		...
		<fieldset class="fieldSetBorder"><legend class="fieldSetLabel">Day Treatment</legend>
			<table border="0"  width="100%" class="formtable">
				<tr>
					<td class="formlabel">Date :</td>
					<td>
						<fmt:formatDate value="${dailyTreatDetails.treatment_date}" pattern="dd-MM-yyyy" var="tdt" />
						<c:choose>
							<c:when test="${param._method=='add'}">
								<insta:datewidget id = "treatment_date" name="treatment_date" value="today"/>
							</c:when>
							<c:otherwise>
								<insta:datewidget id = "treatment_date" name="treatment_date" value="${tdt}"/>
							</c:otherwise>
						</c:choose>
					</td>
					<td class="formlabel">Day Of Treatment :</td>
					<td>
						<input type="text" name="treatment_days_from_start" id="treatment_days_from_start"
							value="${dailyTreatDetails.treatment_days_from_start}" class="number"
					...
					<td>
						<div id="autocomplete" style="padding-bottom:20px;width:200px;">
							<input type="text" name="doctor" id="doctor"
								value="${dailyTreatDetails.doctor_name}" ${readOnly}/>
							<div id="doctorContainer" style="width:300px;"></div>
						</div>
						<input type="hidden" name="doctor_id" id="doctor_id" value="${dailyTreatDetails.doctor}">
					</td>
				</tr>
			</table>
		</fieldset>
		<fieldSet class="fieldSetBorder"><legend class="fieldSetLabel">Follicles Details</legend>
			<table>
				<tr>
					<td>
						<fieldset class="fieldSetBorder" style="width:200px"><legend class="fieldSetLabel">Left Ovary</legend>
						<table class="dashboard" id="LTable" style="width:200px">
							<tr class="header">
								<td>Size</td>
								<td>Number</td>
								<td>&nbsp;</td>
							...
							<td colspan="2"></td>
							<td>
								<button type="button" name="addresults" Class="imgButton" Id="addresults" onclick="AddRowL(this)" >
					...
					<td>
						<fieldset class="fieldSetBorder" style="width:200px"><legend class="fieldSetLabel">Right Ovary</legend>
						<table class="dashboard" id="RTable" style="width:200px">
							<tr class="header">
								<td>Size</td>
								<td>Number</td>
								<td>&nbsp;</td>
							...
							<td colspan="2"></td>
							<td>
								<button type="button" name="addresults" Class="imgButton" Id="addresults" onclick="AddRowR(this)" >
									<img src="${cpath}/icons/Add.png" align="right"/>
								</button>
							</td>
							</tr>
		...
		<fieldset class="fieldSetBorder"><legend class="fieldSetLabel">Hormone Test Results</legend>
			<table class="formtable">
				<tr>
					<td class="formlabel">FSH:</td>
					<td><input type="text" name="fsh_value" id="fsh_value" value="${dailyTreatDetails.fsh_value}"
						onkeypress="return enterNumOnly(event);" onchange="return makeingDec(this.value,this);"/></td>
					<td class="formlabel">P4:</td>
					<td><input type="text" name="p4_value" id="p4_value" value="${dailyTreatDetails.p4_value}"
						onkeypress="return enterNumOnly(event);" onchange="return makeingDec(this.value,this);"/></td>
					<td class="formlabel">E2:</td>
					<td><input type="text" name="e2_value" id="e2_value" value="${dailyTreatDetails.e2_value}"
						onkeypress="return enterNumOnly(event);" onchange="return makeingDec(this.value,this);"/></td>
				</tr>
				<tr>
					<td class="formlabel">LH:</td>
					<td><input type="text" name="lh_value" id="lh_value" value="${dailyTreatDetails.lh_value}"
						onkeypress="return enterNumOnly(event);" onchange="return makeingDec(this.value,this);"/></td>
					<td class="formlabel">TSH:</td>
					<td><input type="text" name="tsh_value" id="tsh_value" value="${dailyTreatDetails.tsh_value}"
						onkeypress="return enterNumOnly(event);" onchange="return makeingDec(this.value,this);"/></td>
				</tr>
			</table>
		</fieldset>
		<fieldset class="filedsetBorder"><legend class="fieldsetLabel">Uterus Details</legend>
			<table class="formtable">
				<tr>
					<td class="formlabel">Endometrium:</td>
					<td>
						<input type="text" name="endometrium_thickness" id="endometrium_thickness"
							value="${dailyTreatDetails.endometrium_thickness}" onkeypress="return enterNumOnly(event);"
							onchange="return makeingDec(this.value,this);" class="number"/>  mm
					</td>
					<td class="formlabel">Remarks:</td>
					<td>
						<textarea name="uterus_remarks" style="width:500px">${dailyTreatDetails.uterus_remarks}</textarea>
					</td>
				</tr>
			</table>
		</fieldset>

		<fieldset class="fieldSetBorder"><legend class="fieldSetLabel">Medication</legend>
		...
		<fieldset class="fieldSetBorder">
			...
			<tr>
				...
				<td>
					<textarea name="final_remarks" style="width:700px">${dailyTreatDetails.final_remarks}</textarea>
				</td>
			</tr>
			</table>
		</fieldset>
		...
		&ivf_cycle_id=${ifn:cleanURL(param.ivf_cycle_id)}&start_date=${ifn:cleanURL(param.start_date)}">Daily Treatment List</a>
		| <insta:screenlink screenId="emr_screen" extraParam="?_method=list&mr_no=${param.mr_no}"
				target="_blank" label="EMR View" />
		</div>
...
<%-- 							<td class="formlabel">Presc. Date</td>
	...
	<script>
		var follicleDetails = ${follicleDetails};
	</script>
	</body>
</html>