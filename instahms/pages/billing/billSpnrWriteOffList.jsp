...
<html>
<head>
	<title><insta:ltext key="billing.billlist.list.sponsor.writeoff"/></title>
...
function ValidateWriteOffAppproval() {
	...
	document.BillWriteOffForm._method.value='approveSpnrWriteOff';
...
function showEditDialog(obj){
	...
	setNodeText("eSpnrAmt", document.getElementById("spnrDueAmt"+id).value);
	setNodeText("netSponsorDue", document.getElementById("netSponsorDueAmt"+id).value);
	setNodeText("sponsorCreditAmt", document.getElementById("sponsorCreditAmt"+id).value);
	editDialog.cfg.setProperty("context",[row.cells[12], "tr", "bl"], false);
	editDialog.show();
}
function setRemarks(){
	...
	setNodeText(row.cells[11], remarks, 20, remarks);
	editDialog.cancel();
	return true;
}
...
function clearSearch() {
     clearForm(document.forms.BillSearchWriteOffForm);
}
...
<c:set var="writeoffTypes">
<insta:ltext key="billing.billlist.list.notmarked.writeoff"/>,
...
<h1><insta:ltext key="billing.billlist.list.sponsor.writeoff"/></h1>

<insta:feedback-panel/>

<form name="BillSearchWriteOffForm" method="GET" >
	<input type="hidden" name="_method" value="getBillsSpnrWriteOffList">

	<input type="hidden" name="_searchMethod" value="getBillsSpnrWriteOffList"/>
		...
		<div id="optionalFilter" style="clear: both; display: ${hasResults ? 'none' : 'block'}" >
			<table class="searchFormTable">
				...
				<td>
					...
					<div class="sfField">
						<insta:checkgroup name="sponsor_writeoff" selValues="${paramValues.sponsor_writeoff}"
							opvalues="N,M,A" optexts="${writeoffTypes }"/>
					</div>
				</td>
				<td>
				...
				<td>
					<div class="sfLabel"><insta:ltext key="billing.writeoff.list.sponsorDue"/>&lt;= :</div>
					<div class="sfField">
						<input type="text" name="sponsor_due" name="sponsor_due0" value="${paramValues.sponsor_due[0]}">
						<input type="hidden" name="sponsor_due@op" value="le,ge"/>
						<input type="hidden" name="sponsor_due@type" value="numeric"/>
					</div>
					<div class="sfLabel"><insta:ltext key="billing.writeoff.list.sponsorDue"/>&gt;= :</div>
					<div class="sfField">
						<input type="text" name="sponsor_due" name="sponsor_due1" value="${paramValues.sponsor_due[1]}">
						<input type="hidden" name="sponsor_due@type" value="numeric"/>
					</div>
				</td>
				</tr>
...
<form name="BillWriteOffForm">
	<input type="hidden" name="_method" value="approveSpnrWriteOff">
	<div class="resultList">
		<table class="resultList dialog_displayColumns" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
			<tr onmouseover="hideToolBar();">
				...
				<th style="text-align: right;"><insta:ltext key="billing.billlist.list.spnrtax"/></th>
				<th style="text-align: right;"><insta:ltext key="billing.billlist.list.spnrpmt"/></th>
				<th style="text-align: right;"><insta:ltext key="billing.billlist.list.spncreditamt"/></th>
				<th style="text-align: right;"><insta:ltext key="billing.billlist.list.spnrdue"/></th>

				<th style="text-align: right;"><insta:ltext key="billing.writeoff.list.spnrWrittenOffAmt"/></th>
			...
			<c:forEach var="bill" items="${billWriteOffList}" varStatus="st">
				<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
					...
					<td>
						<c:set var="billDueAmt" value ="${bill.total_amount - bill.primary_total_claim - bill.secondary_total_claim - bill.total_receipts - bill.deposit_set_off - bill.points_redeemed_amt}"/>
						<c:set var="appChkDisable" value="${(bill.sponsor_writeoff == 'A') ? 'disabled' : ''}"/>
						<c:set var="flagReq" value="${bill.sponsor_writeoff == 'M'}"/>
						<input type="checkbox" name="approve" id="approve" ${appChkDisable} value="${bill.bill_no}"/>
						<c:set var="totPriSecSpoReceipt" value = "${bill.primary_total_sponsor_receipts + bill.secondary_total_sponsor_receipts}" />
						<c:set var="totClaimRecd" value = "${bill.claim_recd_amount}"/>
						<c:set var="totSpnReceipt" value = "${totPriSecSpoReceipt == 0 ? totClaimRecd : totPriSecSpoReceipt}"/>
						<c:set var="spnrDue" value="${bill.primary_total_claim + bill.secondary_total_claim + bill.total_claim_tax - totSpnReceipt}"/>
						<input type="hidden" name="writeOffRemarks" id="writeOffRemarks${st.index}" value="${bill.sponsor_writeoff_remarks}"/>
						...
						<input type="hidden" name="spnrDueAmt" id="spnrDueAmt${st.index}" value="${spnrDue}"/>
						<input type="hidden" name="sponsorCreditAmt" id="sponsorCreditAmt${st.index}" value="${bill.total_credit_sponsor}"/>
						<input type="hidden" name="netSponsorDueAmt" id="netSponsorDueAmt${st.index}" value="${bill.net_sponsor_due}"/>
					</td>
					<td style="text-align: right">${(pagedList.pageNumber-1) * pagedList.pageSize + st.index + 1 }</td>
					<td>
					<c:choose>
						<c:when test="${flagReq}">
							<img class="flag" src="${cpath}/images/blue_flag.gif"/>
						</c:when>
						<c:otherwise>
					...
					<td style="text-align: right;">${totSpnReceipt}</td>
					<td style="text-align: right;">${bill.total_credit_sponsor }</td>
					<td style="text-align: right;">${spnrDue}</td>
					<td style="text-align: right;">
						<c:choose>
							<c:when test="${bill.sponsor_writeoff == 'A'}">
								${spnrDue + bill.total_credit_sponsor }
							</c:when>
							<c:otherwise>
								0.00
							</c:otherwise>
					...
					<td><insta:truncLabel value="${bill.sponsor_writeoff_remarks}" length="20"/>
					</td>
					<td>
	...
	<div class="legend">
			...
			<div class="flagText"><insta:ltext key="billing.billlist.list.markedfor.writeoff" /></div>
 	</div>
</form>
<form name="editForm">
			...
			<fieldset class="fieldSetBorder">
				...
				<table class="formtable" width="100">
					<tr>
						...
						<td class="formlabel">Sponsor Due:</td><td class="forminfo"><label id="eSpnrAmt" style="float: left"></label></td>
					</tr>
					<tr>
						<td class="formlabel">&nbsp;</td>
						<td class="forminfo">&nbsp;</td>
						
						<td class="formlabel">Sponsor Credit Amount:</td><td class="forminfo"><label id="sponsorCreditAmt" style="float: left"></label></td>
						<td class="formlabel">Net Sponsor Due:</td><td class="forminfo"><label id="netSponsorDue" style="float: left"></label></td>
					</tr>
					<tr>
