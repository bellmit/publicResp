...
<html>
	...
	<insta:link type="script" file="widgets.js"/>
		<title>In Coming Test Details - Hospital wise</title>
...
<script type="text/javascript">
	...
	function onInit() {
		...
		selectInComingHospitals();
	}

	function getTestStatus(){
		if(!validateInComingHosptNames())
			return false;
	...
	function selectInComingHospitals(){
		var disable = document.forms[0].all.checked;
		var inComingHospLen = document.forms[0].hospIdArray.length;
		for (i=inComingHospLen-1;i>=0;i--){
			document.forms[0].hospIdArray[i].selected = disable;
		}
	}

	function validateInComingHosptNames(){
	...
	function onChangeReportType(){
		...
		if(type=="detailed"){
		document.inputform.report_name.value="InComingHospitalDetails";
		}else{
		document.inputform.report_name.value="InComingTestDetailsSummary";
		}
	 }
</script>
	</head>
	<body onload="onInit();">
		<div class="pageHeader">InComing Test Details - Hospital wise</div>

		<form name="inputform" target="_blank" method="GET">
		<input type="hidden" name="method" value="getReport">
		<input type="hidden" name="report_name" value="InComingHospitalDetails">
		<div class="tipText">
					This report shows in coming test details - hospital wise within given dates.
					The report displays Date,Patient Name,Test Name,Test Status,Amount.
		</div>
		   <table align="center"width="27%">
		      <tr>
		             <td>Select type of output:</td>
		       </tr>
		       <tr>
			       <td colspan="2">
						<input type="radio" id="item" name="reportType" onchange="onChangeReportType()" value="detailed"
							checked>
						<label for=detail>Detailed:one row per test. </label>
					</td>
				</tr>
				<tr>
					<td colspan="2">
						<input type="radio" id="hospital" name="reportType" onchange="onChangeReportType()" value="summary">
						<label for="summary">Summary:totals shown per patient.</label>
					</td>
				</tr>
			</table>
		<table align="center" >
				...
				<tr>
					<td>
						<table>
							<tr>
								<td valign="top">
								 <input type="checkbox" name="all" onclick="selectInComingHospitals();">All
								</td>
								<td>
									<insta:selectdb style="width: 300px;height: 200px;"  name="hospIdArray"  size = "5" onclick="deselectAll()"  multiple= "true"
									value="" table="incoming_hospitals" valuecol="hospital_id" displaycol="hospital_name"
									filtered="false"/>
								</td>
							</tr>
						</table>
						...
						<table>
							<tr>
								<td>
									...
									opvalues="N,P,NRN,C,CRN,V,S,RP,RC,RV,X,MA,TS,CC,CR,U"
									optexts="New (Results),In Progress,New (No Results),Conducted (Results),Conducted (No Results),Validated,Signed-off,Revision in Progress,Revision Completed,Revision Validated,Cancel,Patient Arrived,Scheduled for Transcriptionist,Conduction Completed,Change Required,Condn. Unnecessary" />
								</td>
							</tr>
						</table>
					</td>
