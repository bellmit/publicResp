...
<c:set var="searchUrl" value="${cpath}/message/MyNotifications.do?_method=list" />
<c:set var="detailsUrl" value="message/MyNotifications.do?_method=show" />
...
<div class="pageHeader">My Notifications</div>
...
<insta:search form="searchForm" optionsId="optionalFilter" closed="${hasResult}">
	<div class="searchBasicOpts">
		<div class="sboField">
		<div class="sboFieldLabel" style="display:none">Recipient Id: </div>
			<div id="riecipient_wrapper_r" class="autoComplete" style="display:none">
		...
		<div class="sboField">
			...
			<div>
				<insta:checkgroup name="message_status" selValue="${param.message_status}"
								opvalues="R,S" optexts="Read,Unread" />
			</div>
		</div>
				...
				<c:forEach var="messageLog" items="${messageLogList}" varStatus="status">
					<tr class="${status.index == 0 ?'firstRow': ''} ${status.index % 2 == 0? 'even':'odd' }"
						...
						onmouseover="hideToolBar(${status.index});" id="toolbarRow${status.index}">
							<td><c:if test="${messageLog.map.message_status eq 'S'}"><img src='${cpath}/images/red_flag.gif'></c:if>
						<c:if test="${messageLog.map.message_status eq 'R'}"><img src='${cpath}/images/empty_flag.gif'></c:if>
						${messageLog.map.message_sender_id} </td>
	...
	<table class="screenActions">
		<tr>
			<td>
				<a href="${cpath}/message/MyMessages.do?_method=list&sortOrder=last_sent_date&sortReverse=true&message_status=">
				Sent Notifications
				</a>
				&nbsp;|&nbsp;
			...
			<div class="legend" style="display: ${hasResult? 'block' : 'none'}" >
				<div class="flag"><img src='${cpath}/images/red_flag.gif'></div>
					<div class="flagText">Unread</div>
				<div class="flag"><img src='${cpath}/images/empty_flag.gif'></div>
					<div class="flagText">Read</div>
			</div>
		</tr>
	</table>
	</form>
