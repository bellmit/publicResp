						...
						<tr>
							<td>
								...
								<%-- <div class="sfField">
									...
									<div style="clear: both"/>
									<input type="text" name="patient_complaint" id="patient_complaint" value="${param.patient_complaint}"/>
								</div> --%>
								<div class="sfField">
		...
		<table class="resultList dialog_displayColumns" cellpadding="0" cellspacing="0" align="center" width="100%" id="resultTable" >
				...
				<c:forEach var="recipientData" items="${messageDataList}" varStatus="status">
					...
					<tr class="${status.index == 0 ?'firstRow': ''} ${status.index % 2 == 0? 'even':'odd' }">
						...
						<td>
						<c:if test="${messageLog.map.message_mode eq 'SMS' && empty recipientData.patient_phone}"><img src='${cpath}/images/red_flag.gif'></c:if>
						<c:if test="${messageLog.map.message_mode eq 'EMAIL' && empty recipientData.email_id}"><img src='${cpath}/images/red_flag.gif'></c:if>
						${recipientData.patient_name}
						</td>
						<td>${recipientData.patient_city}</td>
						<td>${recipientData.email_id}</td>
						<td>${recipientData.patient_phone}</td>
						<td>${recipientData.first_visit_reg_date}</td>
						<td>${recipientData.patient_state}</td>
						<td>${recipientData.country}</td>
					</tr>
				</c:forEach>
			</table>
		</div>
