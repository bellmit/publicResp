				...
				<c:choose>
	    			<c:when test="${fn:contains(msgActionName, ';')}">
	      				...
	      				<c:forTokens var="token" items="${msgActionName}" delims=";">
					      	  <a href = "${cpath}/message/notification/StdNotificationAction.do?_method=doAction&message_log_id=${messageId}&screen=my_msg&message_action_type=${action.map.message_action_type}&option=${token}">
									${token}
							  </a>&nbsp;|&nbsp;
						 </c:forTokens>
						 </c:if>
	   			 	...
	   			 	<c:otherwise>
	     			   <a href = "${cpath}/message/notification/StdNotificationAction.do?_method=doAction&message_log_id=${messageId}&screen=my_msg&message_action_type=${action.map.message_action_type}">
