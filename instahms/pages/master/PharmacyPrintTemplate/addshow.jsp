...
<head>
	...
	<title>Pharmacy Print Template - Insta HMS</title>
	...
	<script>
		...
		function doSave() {
			...
			document.pharmacyprint.action= "PharmacyPrintTemplate.do?method="+saveMethod;
			document.pharmacyprint.submit();
		}

		function doReset() {
			document.pharmacyprint.action= "PharmacyPrintTemplate.do?method=resetToDefault";
			document.pharmacyprint.submit();
		}
	</script>
</head>
		...
		<table class="screenActions">
			<tr>
				...
				<c:if test="${templateMode == 'H'}">
					<c:url var="changeEditor" value="PharmacyPrintTemplate.do">
				...
				<c:url var="dashboardUrl" value="PharmacyPrintTemplate.do">
						<c:param name="method" value="list"/>
				</c:url>
				<c:if test="${param.method=='show'}">
					<td>&nbsp;|&nbsp;</td>
					<td><a href="#" onclick="window.location.href='${cpath}/master/PharmacyPrintTemplate.do?method=getTemplateMode'">Add</a></td>
				</c:if>
				<td>&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="return gotoLocation('${dashboardUrl}');">Pharmacy Print Template List</a></td>
			</tr>
		</table>
	</form>
