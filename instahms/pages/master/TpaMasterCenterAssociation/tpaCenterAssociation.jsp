...
<html>
	<head>
		...
		<title>Add/Edit Tpa Center Association - Insta HMS</title>
		<insta:link type="script" file="master/TpaCenterAssociation/tpacenterassociation.js"/>
		<script>
	...
	<body onload="init();">
		<h1>Center Association - ${tpa_bean.map.tpa_name}</h1>
		<insta:feedback-panel/>
		<form action="TpaMasterCenterAssociation.do" name="tpa_center_association_form" method="POST">
			<input type="hidden" name="_method" value="update"/>
			<input type="hidden" name="tpa_id" value="${ifn:cleanHtmlAttribute(param.tpa_id)}"/>
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>
					...
					<div style="padding-top: 5px; float: left">TPA Association for all centers &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
					<div style="padding-top: 5px; float: left" id="enabledisableforclaimformat">Claim Format : ${tpa_bean.map.claim_format}</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">TPA Association only for following centers</div>
				</div>
				<div style="clear: both"></div>
				<table style="margin-top: 10px" class="detailList dialog_displayColumns" cellspacing="0" cellpadding="0"
					...
					<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="tpa_center_id" value="${app_center.tpa_center_id}"/>
								...
								<input type="hidden" name="claim_format" value="${app_center.claim_format}"/>
			...
			<table style="margin-top: 10px">
				<tr>
						...
						| <a href="${cpath}/master/TpaMaster.do?_method=list&amp;sortOrder=tpa_name&amp;sortReverse=false&amp;status=A">
							TPA / Sponsor List
						</a>
					</td>
				</tr>
						...
						<table id="avlbl_centers_table" class="detailList" style="margin-top: 10px">
							...
							<tr style="display: none">
								<td>
									...
									<input type="hidden" name="d_claim_format" value="${tpa_bean.map.claim_format}"/>
								</td>
								<td></td>
								<td></td>
								<td></td>
					...
					<fieldset class="fieldSetBorder">
						...
						<table class="formtable">
							...
							<tr>
								...
								<td class="forminfo">
									<select id="ed_claim_format" name="claim_format" class="dropdown" >
										<option value="XML" ${app_center.claim_format == 'XML' ? 'selected' : ''}>XML</option>
										<option value="XL" ${app_center.claim_format == 'XL' ? 'selected' : '' }>XL</option>
									</select>
								</td>
							</tr>
