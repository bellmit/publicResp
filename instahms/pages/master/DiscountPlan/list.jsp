		...
		var searchListData = {
				...
				toolbar : {
					Edit: {
							...
							href: 'master/DiscountPlan.do?_method=show',
							onclick: null,
...
<c:set var="addEditUrl" value="DiscountPlan"/>
...
<body onload="initSearchListPage(searchListData);">
	<insta:list-dashboard displayName="${entityName}" searchFormName="${searchFormName}" >
		...
		<insta:search-result keyField="${keyField}" columns="${listColumns}" sortableColumns="${sortableColumns}" dataList="${pagedList}"/>
		<insta:list-footer addEditUrl="${addEditUrl}" displayName="${entityName}"/>
