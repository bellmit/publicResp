...
<title>Edit Incoming Hospital - Insta HMS</title>
<insta:link type="script" file="hmsvalidation.js"/>
	<script>
      ...
      var backupName = '';
		...
		function keepBackUp(){
			backupName = document.InComingHospitalForm.hospital_name.value;
		}

		 Insta.masterData=${ifn:convertListToJson(hospitalsLists)};
...
<h1 style="float:left">Edit Incoming Hospital</h1>
<c:url var="searchUrl" value="/master/InComingHospitals/show.htm"/>
<insta:findbykey keys="hospital_name,hospital_id" fieldName="hospital_id" method="show" url="${searchUrl}"/>
     
<form action="update.htm" name="InComingHospitalForm" method="POST">
<input type="hidden" name="hospital_id" value="${bean.hospital_id}"/>
	...
	<table class="formtable">
		...
		<tr>
			...
			<td>
			<select name="default_rate_plan_id" class="dropdown">
				<option value="">--Select--</option>
					<c:forEach items="${ratePlanDetails}" var="ratePlans">
						<option value="${ratePlans.org_id}" ${bean.default_rate_plan_id == ratePlans.org_id ? 'selected':''}>${ratePlans.org_name}</option>
					</c:forEach>
			</select>
