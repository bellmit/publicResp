...
<title><insta:ltext key="generalmasters.regularexpression.show.title" /></title>
...
<script>
	...
	var backupName = '';
	function keepBackUp(){
		//if(document.regExp._method.value == 'update'){
				backupName = document.regExp.pattern_name.value;
		   //  }
		}
...
<form action="update.htm" method="POST" name="regExp">
	
	<input type="hidden" name="pattern_id" id="pattern_id" value="${bean.pattern_id}"/>

	<h1>Edit <insta:ltext key="generalmasters.regularexpression.list.pattern" /> </h1>
	...
	<fieldset class="fieldsetborder">
		...
		<table class="formtable">
			<tr>
				...
				<td colspan="2">
					<input type="text" name="pattern_name" id="pattern_name" value="<c:out value="${bean.pattern_name}"/>" maxlength="50" class="required" title="${labelmsg}"><span class="star">*</span>
				</td>
			</tr>
			<tr>
				<td class="formlabel">${patternexp }:</td>
				<td colspan="2">
					<input type="text" name="regexp_pattern" id="regexp_pattern" value="<c:out value="${bean.regexp_pattern}"/>" maxlength="100" class="required" title="${labelmsg}"><span class="star">*</span>
			...
			<tr>
				...
				<td colspan="2"><insta:selectoptions name="status" id="status" value="${bean.status}" opvalues="A,I" optexts="${status}" /></td>
			</tr>
			<tr>
				<td class="formlabel"><insta:ltext key="generalmasters.regularexpression.list.patterndescription" />:</td>
				<td colspan="2">
					<textarea name="pattern_desc" id="pattern_desc" cols="30" rows="5" title='<insta:ltext key="generalmasters.regularexpression.list.patterndescription"/>'><c:out value="${bean.pattern_desc}"/></textarea>
				</td>
			</tr>
		</table>
	</fieldset>
	...
	<div class="screenActions">
		...
		| <a href="${cpath}/${pagePath}/add.htm" ><insta:ltext key="generalmasters.regularexpression.show.add"/></a>
