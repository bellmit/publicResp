...
<insta:link type="script" file="hmsvalidation.js"/>
	<script>
		function doClose() {
			window.location.href = "${cpath}/master/dialyzerRating.do?_method=list&status=A&sortOrder=dialyzer_rating&sortReverse=false";
		}
...
<body>
	<form action="dialyzerRating.do" method="POST">
		...
		<h1>${param._method == 'add' ? addText : editText} <insta:ltext key="generalmasters.dialyzerratings.addoredit.dialyzerrating"/></h1>
		<insta:feedback-panel/>
		<c:if test="${param._method == 'show'}">
			<input type="hidden" name="dialyzer_rating_id" value="${bean.map.dialyzer_rating_id}"/>
		...
		<fieldset class="fieldsetborder">
			...
			<table class="formTable">
				<tr>
					...
					<td>
						 <input type="text" name="dialyzer_rating" value="${bean.map.dialyzer_rating}"
		...
		<table class="screenActions">
			<tr>
				...
				<c:if test="${param._method=='show'}">
				<td>&nbsp;|&nbsp;</td>
				<td><a href="#" onclick="window.location.href='${cpath}/master/dialyzerRating.do?_method=add'"><insta:ltext key="generalmasters.dialyzerratings.addoredit.add"/></a></td>
				</c:if>
				<td>&nbsp;|&nbsp;</td>
