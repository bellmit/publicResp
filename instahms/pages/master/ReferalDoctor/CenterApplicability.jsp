...
<html>
	<head>
		...
		<title>Add/Edit Referal Doctor Center Applicability - Insta HMS</title>
		<insta:link type="script" file="master/ReferalApplicability/referalcenterapplicability.js"/>
		<script>
	...
	<body onload="init();">
		<h1>Center Applicability - ${referal_bean.map.referal_name}</h1>
		<insta:feedback-panel/>
		<form action="ReferalDoctorApplicability.do" name="referal_center_association_form" method="POST">
			<input type="hidden" name="_method" value="update"/>
			<input type="hidden" name="referal_no" value="${ifn:cleanHtmlAttribute(param.referal_no)}"/>
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>
					...
					<div style="padding-top: 5px; float: left">Referral Doctor associated with all centers</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">Referral Doctor applicable only for following centers</div>
				</div>
				<div style="clear: both"></div>
					...
					<tr>
			 		...
			 		<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="referal_center_id" value="${app_center.referal_center_id}"/>
			...
			<table style="margin-top: 10px">
				<tr>
						<input type="button" name="Save" value="Save"
						 ...
						 | <a href="${cpath}/master/ReferalDoctor.do?_method=list&status=A&sortOrder=referal_name&sortReverse=false';">
