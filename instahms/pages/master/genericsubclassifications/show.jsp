...
<script>
    ...
    Insta.masterData=${ifn:convertListToJson(genericSubClassificationsLists)};
</script>
</head>
<body class="yui-skin-sam" >
	<h1 style="float:left">Edit Sub Classification Details</h1>
	<c:url var="searchUrl" value="${pagePath}/show.htm"/>
	<insta:findbykey keys="sub_classification_name,sub_classification_id" fieldName="sub_classification_id" method="show" url="${searchUrl}"/>
	<insta:feedback-panel/>
	<form method="POST" action="update.htm" name="genericsubclassForm">
		<input type="hidden" name="sub_classification_id" value="${bean.sub_classification_id}"/>
		<fieldset  class="fieldSetBorder">
			<legend class="fieldSetLabel">Sub Classification</legend>
			<table class="formtable">
			  <tr>
			    <td class="formlabel">Sub Classification Name:</td>
			  	<td><input type="text" name="sub_classification_name" id="sub_classification_name" value="${bean.sub_classification_name}"  maxlength="100"><span class="star">*</span></td>
			  	<td class="formlabel">Classification Name:</td>
			    <td><select name="classification_id" id="classification_id"  class="dropdown">
						<option value="">--select--</option>
						<c:forEach var="classi" items="${classificationdetails}">
							<option value="${classi.classification_id}"
								${bean.classification_id == classi.classification_id ? 'selected' : ''}>
								${classi.classification_name}</option>
						</c:forEach>
					</select>
				</td>
			  	<td>&nbsp;</td>
