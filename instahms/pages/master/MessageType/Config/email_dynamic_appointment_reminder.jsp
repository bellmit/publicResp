...
<form action="${cpath}/master/${urlPrefix}/${ifn:cleanURL(param.message_type_id)}/MessageConfig.do" method="POST">
	...
	<fieldset class="fieldsetborder">
		...
		<table class="formtable">
			...
			<tr>
				<td class="formlabel">Send Appointment Reminder:</td>
				<td  style="width:300px;" >
					<input type="hidden" name="message_configuration_id" value="${configMap['buffer_hours'].map.message_configuration_id}"/>
					<input style="width:45px;" type="text" id='param_value' name="param_value_${configMap['buffer_hours'].map.message_configuration_id}"
					value="${configMap['buffer_hours'].map.param_value}" />
					hours before appointment for both SMS/EMAIL &nbsp;&nbsp;&nbsp;<img class="imgHelpText" title="${configMap['buffer_hours'].map.param_description}" src="${cpath}/images/help.png"/>
				</td>
			</tr>
			<tr>
			  ...
			  <td colspan="2">
			    ...
			    <insta:selectoptions id="status" name="param_value_${configMap['status'].map.message_configuration_id}" value="${configMap['status'].map.param_value}" opvalues="Confirmed,Booked,Both" optexts="Confirmed,Booked,Both"/>
			    Appointment status &nbsp;&nbsp;&nbsp;<img class="imgHelpText" title="${configMap['status'].map.param_description}" src="${cpath}/images/help.png"/>
			  </td>
		  </tr>
		</table>
