...
<html>
	<head>
		...
		<title>Add/Edit Insurance Category Center Applicability - Insta HMS</title>
		<insta:link type="script" file="master/InsCatCenter/InsCatCenter.js"/>
		<script>
	...
	<body onload="init();">
		<h1>Center Applicability - ${category_bean.map.category_name}</h1>
		<insta:feedback-panel/>
		<form action="InsCatCenter.do" name="inscat_center_association_form" method="POST">
			<input type="hidden" name="_method" value="update"/>
			<input type="hidden" name="category_id" value="${ifn:cleanHtmlAttribute(param.category_id)}"/>
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>
					...
					<div style="padding-top: 5px; float: left"> Insurance Network/Plan Type applicability with all centers</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">Insurance Network/Plan Type applicability only for following centers</div>
				</div>
				<div style="clear: both"></div>
					...
					<tr>
			 		...
			 		<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="inscat_center_id" value="${app_center.inscat_center_id}"/>
			...
			<table style="margin-top: 10px">
				<tr>
						<input type="button" name="Save" value="Save"
						 ...
						 | <a href="${cpath}/master/InsuranceCatMaster.do?_method=list&sortOrder=category_name&sortReverse=false';">
