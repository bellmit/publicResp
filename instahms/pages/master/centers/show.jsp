...
<insta:ltext key="patient.resourcescheduler.doctornonavailability.saturday"/>
	...
	<title>Edit Center - Insta HMS</title>
	<style>
	 .centerMandatory {
	...
	//var regPref = <!--%= new JSONSerializer().serialize(RegistrationPreferencesDAO.getRegistrationPreferences())%-->
	//var regPref = null; //registrationCommon.js is using regPref of DTO object, but in centers master we are using baiscdynabean(spring code)
...
<body onload="init();" class="yui-skin-sam">
	...
	<form action="${cpath}${pagePath}/update.htm"  method="post">
		...
		<legend class="fieldSetLabel">Center Details</legend>
			<table class="formtable">
				<tr>
					...
					<td><input type="text" name="center_name" value="${bean.center_name}" class="required" title="Center Name is mandatory."></td>
					<td class="formlabel">Center Code: </td>
					<td><input type="text" name="center_code" value="${bean.center_code}" class="required" title="Center Code is mandatory."></td>
				...
				<tr>
					...
					<td>
						<input type="hidden" name="city_id" id="defaultcity" value="${bean.city_id}"/>
						<div id="city_state_country_wrapper" class="autoComplete">
							<input type="text" name="pat_city_name" id="pat_city_name"
								value="${bean.city_name}" class="required" title="City is mandatory."/>
					...
					<td>
						<input type="hidden" name="state_id" id="defaultstate" value="${bean.state_id}"/>
						<label id="statelbl" class="formlabel">${bean.state_name}</label>
					</td>
					<td class="formlabel">Country: </td>
					<td>
						<input type="hidden" name="country_id" id="defaultcountry" value="${bean.country_id}"/>
						<label id="countrylbl" class="formlabel">${bean.country_name}</label>
					</td>
				</tr>
				<tr>
					...
					<td>
						<textarea style="width:150px; height:50px; padding:0 0 2px 2px"
							...
							id="center_address" tabindex="90" onkeypress="return checkLength(this, 200, event);">${bean.center_address}</textarea>
					</td>
					<td class="formlabel">Center Contact No:</td>
					<td>
						<input type="text" maxlength="30" name="center_contact_phone" id="center_contact_phone" value="${bean.center_contact_phone}">
					</td>
					<td class="formlabel">Region:</td>
					<td><select name="region_id" id="region_id" class="dropdown">
							<option value="">-- Select --</option>
							<c:forEach items="${regionBean}" var="region">
								<option value="${region.region_id}"
									${bean.region_id == region.region_id ? 'selected':''}>${region.region_name}
								</option>
							</c:forEach>
					</select>
					</td>
				...
				<tr>
					...
					<td><input type="text" name="hospital_center_service_reg_no" id="hospital_center_service_reg_no"
							value="${bean.hospital_center_service_reg_no}"
				...
				<tr>
					...
					<td><input type="text" name="tin_number" id="tin_number" value="${bean.tin_number}"
		...
		<table style="margin-top: 10px">
				<tr>
					...
					<td> | <a href="${cpath}${pagePath}/list.htm?list&sortOrder=center_name&sortReverse=false">List</a>&nbsp;</td>
					<td>  <insta:screenlink screenId="mas_reports_cen_app" extraParam="?_method=getScreen&center_id=${param.center_id}" label="Edit Default Center Reports Delivery Time"  addPipe="true"/>&nbsp;</td>
					<td>  <insta:screenlink screenId="mas_reports_overrides" extraParam="?_method=getScreen&center_id=${param.center_id}" label="Edit Center Delivery Times Overrides"  addPipe="true"/></td>
				</tr>
			</table>
	</form>
</body>
