...
<script>
//var codesList = ${ifn:convertListToJson(codesList)};
...
function checkDuplicates() {
	...
	/* if ( validityend== null || validityend== ""){
		alert("Please enter validity end date");
	...
	/* for (var i = 0; i<codesList.length; i++) {
		var dbCode = codesList[i].subgroup_code.toLowerCase();
		var dbId = codesList[i].item_subgroup_id;
		if (id != dbId) {
			if (code == dbCode) {
				alert("tax  sub group code already exists, Please enter another code.");
				document.getElementById('subgroup_code').focus();
				return false;
			}
		}
	...
	<legend class="fieldSetLabel">Tax Sub Group</legend>
		<table class="formtable" >
			<tr>
				...
				<td>
					<insta:selectdb name="item_group_id" id = "item_group_id" table="item_groups" valuecol="item_group_id"
							...
							value=""  orderby="item_group_name" dummyvalue="-- Select --" dummyvalueId="" 
							filtercol ="item_group_type_id"  filtervalue ="TAX"/>
							<span class="star">*</span>
				</td>
				<td class="formlabel">Tax Sub Group:</td>
				<td>
					<input type="text" name="item_subgroup_name" id="item_subgroup_name" value=""  class="required validate-length"
					length="50" title="Max length of tax sub group can be 50"  />
					<span class="star">*</span>
				</td>
			...
			<tr>
				...
				<td>
					<input type="text" name="subgroup_code" id="subgroup_code" value=""  class="required validate-length"
					length="6" title="Max length of tax sub group code can be 6"  />
					<span class="star">*</span>
				</td> -->
				<td class="formlabel">Display Order:</td>
				<td>
					<input type="text" name="item_subgroup_display_order" id="item_subgroup_display_order" value=""  onkeypress="return enterNumOnly(event);"  />
				</td>
			</tr>
	...
	<legend class="fieldSetLabel">Tax Sub Group Details</legend>
		<table class="formtable">
			<tr>
				...
				<td>
					<input type="text" name="tax_rate" id="tax_rate" value=""  onkeypress="return numbersAndFloatsOnly(event);"  />
					<span class="star">*</span>
			...
			<c:if test="${roleId == 1 || roleId == 2}">
				 <tr>
					...
					<td colspan="4">
						<textarea rows="3" cols="100" name="tax_rate_expr" id="tax_rate_expr" maxlength="1000"></textarea>
					</td>
				</tr> 
			</c:if>
		</table>
