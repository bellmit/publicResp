...
<html>
<head>
	<title>Surgery/Procedure - Insta HMS</title>
...
<body onload="initOperationAc(); selectAllBedTypes(); init()"  class="yui-skin-sam">

<h1>Surgeries/Procedures</h1>
<insta:feedback-panel/>

<form action="OperationMaster.do" method="GET" name="searchform">
			...
			<table class="searchFormTable">
				<tr>
					...
					<td>
						...
						<div class="sfField" style="width:180px">
		...
		<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');" >
			<tr onmouseover="hideToolBar();">
				<th style="padding-top: 0px;padding-bottom: 0px;">
					<input type="checkbox" name="allPageOperations" disabled onclick="selectAllPageOperations()"/>
				</th>
				<insta:sortablecolumn name="operation_name" title="Surgery/Procedure Name"/>
			...
			<c:forEach var="operation" items="${operationList}" varStatus="st">
				...
				<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
					...
					{op_id: '${operation.op_id}', org_id: '${ifn:cleanJavaScript(org_id)}', operation_name: tb_names[${st.index}]}, '');" >
					<td>
						<input type="checkbox" name="selectOperation" disabled value="${operation.op_id}">
					</td>
					<td>
					...
					<c:forEach var="bed" items="${bedTypes}">
						<td class="number" align="right">${ifn:afmt(charges[operation.op_id][bed].map[chargeType])}</td>
						<!-- charges: for ${ifn:cleanHtml(chargeType)} ${charges[operation.op_id][bed].map.surg_asstance_charge} -->
					</c:forEach>
				</tr>
			</c:forEach>
		</table>
...
<div id="content">
<h1>
	Surgery/Procedure Creation Job status
</h1>
<div class="resultList" >
	<table class="resultList" id="schedulerTable">
	<tr onmouseover="hideToolBar();">
		<th>ID</th>
		<th>Surgery/Procedure ID</th>
		<th>Status</th>
		<th>Error</th>
		<th>Retry</th>
	</tr>
...
<table class="screenActions" width="100%">
	<tr>
		<td>
			<a href="${cpath}/master/OperationMaster.do?_method=add">Add New Surgery/Procedure</a>
			<c:if test="${urlRightsMap.operations_audit_log != 'N'}"> |
				<a href="${cpath}/operations/auditlog/AuditLogSearch.do?_method=getSearchScreen">Audit Log Search</a>
			</c:if>
		</td>
        <td align="right">
...
<div id="CollapsiblePanel1" class="CollapsiblePanel">
		...
		<table>
			<tr>
				<td valign="top">
					<form name="updateform" action="OperationMaster.do" method="POST">
						<input type="hidden" name="_method" value="groupUpdate">
						<input type="hidden" name="org_id" value="">
						<input type="hidden" name="chargeType" value="">

						<div style="display:none" id="operationListInnerHtml">
							<%-- this holds the hidden inputs for the list of selected operations --%>
						</div>

						<table class="search">
							<tr>
								<th>Select Surgeries/Procedures</th>
							...
							<tr>
								<td>
									<input type="radio" checked name="allOperations" onclick="onChangeAllOperations()" value="yes">
									All Surgeries/Procedures <br>
									<input type="radio" name="allOperations" onclick="onChangeAllOperations()" value="no">
									Selected Surgeries/Procedures
								</td>
				...
				<td valign="top">
					...
					<tr>
						<th>Export/Import Surgery/Procedure Details</th>
					</tr>
					<tr>
						<td>
							<table>
								<tr>
									<td>Export: </td>
									<td>
										<form name="exportoperationform" action="OperationMaster.do" method="GET"
												style="padding:0; margin:0">
											<div style="float: left">
												<input type="hidden" name="_method" value="exportOperationDetailsToXls">
								...
								<tr>
									<td>Import:</td>
									<td>
										<form name="uploadoperationform" action="OperationMasterUpload.do" method="POST"
												enctype="multipart/form-data" style="padding:0; margin:0">
											<input type="hidden" name="method" value="importOperationDetailsFromXls">
											<input type="file" name="xlsOperationFile" accept="<insta:ltext key="upload.accept.master"/>"/>
											<button type="button" accesskey="F" onclick="return doUpload('uploadoperationform')" >
							...
							<td>
								<table>
									<tr>
										<td>Export: </td>
										<td>
											<form name="exportform" action="OperationMaster.do" method="GET"
													style="padding:0; margin:0">
												<div style="float: left;">
									...
									<tr>
										<td>Import:</td>
										<td>
											<form name="uploadform" action="OperationMasterUpload.do" method="POST"
													enctype="multipart/form-data" style="padding:0; margin:0">
												<input type="hidden" name="method" value="importChargesFromXLS">
												<input type="hidden" name="org_id" value="">
												<input type="file" name="xlsOperationFile" id="xlsOperationFile" accept="<insta:ltext key="upload.accept.master"/>"/>
												<button type="button" accesskey="U"
												onclick="return validateImportChargesFile(this, 'xlsOperationFile', 'org_id');"><b><u>U</u></b>pload</button>
											</form>
										</td>
									</tr>
								</table>
