...
<head>
	...
	<title>Audit Control Sequence Preferences List - Insta HMS</title>
	...
	<script type="text/javascript">
		var toolbar = {
			Edit: {
				...
				onclick: null,
				description: "View and/or Audit Control Sequence Preference details"
				}
		};
		function init()
		{
...
<body onload="init();">
	...
	<h1><insta:ltext key="patient.sequences.auditcontolsequences.auditcontrolsequencepreference"/></h1>

	<insta:feedback-panel/>

	<form name="AuditControlSequencePrefSearchForm" method="GET">
		<input type="hidden" name="sortOrder" value="${ifn:cleanHtmlAttribute(param.sortOrder)}"/>
		<input type="hidden" name="sortReverse" value="${ifn:cleanHtmlAttribute(param.sortReverse)}"/>
		
		<insta:search-lessoptions form="AuditControlSequencePrefSearchForm" >
			<div class="searchBasicOpts">
				<div class="sboField">
					<div class="sboFieldLabel"><insta:ltext key="patient.sequences.auditcontolsequences.patternid"/>:</div>
		...
		<div class="resultList">
			<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
				<tr onmouseover="hideToolBar();">
					...
					<th><insta:ltext key="patient.sequences.auditcontolsequences.priority"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.visittype"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.billtype"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.restrictiontype"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.centerid"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.creditnote"/></th>
					<th><insta:ltext key="patient.sequences.auditcontolsequences.tpa"/></th>
				</tr>
				<c:forEach var="record" items="${pagedList.dtoList}" varStatus="st">
					<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
						onclick="showToolbar(${st.index}, event, 'resultTable',
						 {pattern_id: '${record.pattern_id}',bill_audit_number_seq_id: '${record.bill_audit_number_seq_id}'},'');" id="toolbarRow${st.index}">
		...
		<div class="screenActions" style="float:left"><a href="<c:out value='${url}' />"><insta:ltext key="patient.sequences.auditcontolsequences.addnewauditcontrolsequencepreference"/></a></div>
