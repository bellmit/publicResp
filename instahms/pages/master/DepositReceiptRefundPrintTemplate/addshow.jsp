...
<head>
	...
	<title>Deposit Receipt/Refund Print Template - Insta HMS</title>
	...
	<script>
		...
		var method = "${param._method == 'add' ? 'insert' : 'update'}";
		...
		function doSave() {
			...
			document.forms[0].action= "DepositReceiptRefundTemplate.do?_method=insertOrUpdate&method="+method;
			document.forms[0].submit();
		}

		function doReset() {
			document.forms[0].action= "DepositReceiptRefundTemplate.do?_method=resetToDefault";
			document.forms[0].submit();
		}

		function changeEditor(newMode) {
			window.location = '?_method=show&template_name=${ifn:cleanJavaScript(param.template_name)}&editorMode=' + newMode;
		}
...
<body>
<h1>Deposit Receipt/Refund Print Template</h1>
...
<insta:feedback-panel/>
	...
	<form method="POST">
		...
		<input type="hidden" id="template_mode" name="template_mode" value="${ifn:cleanHtmlAttribute(template_mode)}"/>
		<table class="formtable">
			<tr>
				<td class="formlabel">Template Name: </td>
				<td>
					<input type="text" id="template_name" name="template_name" ${ifn:cleanHtmlAttribute(param._method=='show' ? 'readonly' : '')} value="${ifn:cleanHtmlAttribute(template_name)}"/>
				</td>
			</tr>
			<tr>
	...
	<table class="screenActions">
		<tr>
			...
			<c:url var="dashboardUrl" value="DepositReceiptRefundTemplate.do">
						<c:param name="_method" value="list"/>
			</c:url>
			<c:if test="${templateMode == 'H'}">
				<c:url var="changeEditor" value="DepositReceiptRefundTemplate.do">
			...
			<td>&nbsp;|&nbsp;</td>
			<td><a href="#" onclick="window.location.href='${path}/master/DepositReceiptRefundTemplate.do?_method=selectMode'">Add</a></td>
			</c:if>
