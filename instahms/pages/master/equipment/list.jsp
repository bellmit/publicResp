...
<c:if test="${hasResults}">
	<c:set var="org_id" value="${equipmentCharges.get(0).get('org_id')}" />
	<c:set var="equipmentChargesMap" value='${ifn:listMapToMapMapBean(equipmentCharges, "equip_id","bed_type") }' />
	<c:set var="chargeType" value="${chargeType.get(0).get('chargeType')}"/>
</c:if>
<c:if test="${hasResults eq false}" >
	<c:set var="org_id" value="ORG0001" />
</c:if>
...
<form action="${cpath}/master/equipment.htm" method="GET" name="searchform">
				...
				<div class="sboFieldInput">
					<select name="org_id" id="org_id" class="dropdown">
						<c:forEach items="${orgMasterData}" var="orgDetails">
							<option value="${orgDetails.get('org_id')}" ${orgDetails.get('org_id') eq org_id ? 'selected' : ''}>${orgDetails.get('org_name')}</option>
						</c:forEach>
					</select>	
				</div>
			</div>
				...
				<tr>
					<td>
						<div class="sfLabel">Department</div>
							<div class="sfField">
								<select id="dept_id" name="dept_id" class="listbox" multiple="multiple" >
								<c:forEach items="${departments}" var="diagdept">
									<option value="${diagdept.get('dept_id')}" ${ifn:arrayFind(paramValues.dept_id, diagdept.get('dept_id')) != -1 ? 'selected' : ''}>${diagdept.get('dept_name')}</option>
								</c:forEach>
							</select>
						</div>
					</td>
		...
		<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');" >
			...
			<c:forEach var="equipment" items="${equipmentList}" varStatus="st">
				...
				<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}" id="toolbarRow${st.index}"
					...
					<c:forEach var="bed" items="${bedTypes}">
					     <td class="number" align="right">${equipmentChargesMap[equipment.eq_id][bed.get('bed_type')][chargeType]}
					    </td>  
					</c:forEach>
				</tr>
			</c:forEach>
...
<table class="screenActions">
	<tr>
		<td width="80%">
			<a href="${cpath}/master/equipment/add.htm?org_id=${org_id}">Add New Equipment</a>
		</td>
...
<script>
	var equipmentNames = ${ifn:convertListToJson(equipmentNames)};
