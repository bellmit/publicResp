...
<title>Service Consumables List - Insta HMS</title>
	...
	<script type="text/javascript">
		...
		var toolBar = {
			Edit : {
				...
				href : "/master/ServiceConsumableMaster.do?_method=show",
				onclick : null,
...
<h1>Service Consumables Master</h1>
			...
			<div class="searchBasicOpts" >
				<div class="sboField">
					...
					<div class="sboFieldInput">
						<input type="text" name="service_name" id="service_name" value="${ifn:cleanHtmlAttribute(param.service_name)}" style="width: 140px"/>
	...
	<div class="resultList" >
		...
		<c:forEach var="record" items="${pagedList.dtoList}" varStatus="st">
			<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}" id="toolbarRow${st.index}"
				onclick="showToolbar(${st.index}, event, 'resultTable', {service_id: '${record.service_id}',serviceName: '${record.service_name}'},'');">
		...
		<c:url var="Url" value="ServiceConsumableMaster.do">
			<c:param name="_method" value="add"/>
		</c:url>

		<div class="screenActions" style="float: left">
			<a href="${Url}">Add New Service Consumable</a>
		</div>
