...
<head>
	...
	<script>
		...
		var mapPlanCat = '${bean.map.category_id}';
		//var id = <%= request.getAttribute("tpa_id") %>;
		
		var tpaLists = ${tpaMasterLists};
		var insuNameTpaList = ${insuranceCompaniesTpaLists};
...
<c:choose>
    <c:when test="${param._method !='add'}">
			<form name="planNameForm" action="${cpath}/master/PlanMaster.do">
				<input type="hidden" name="_method" value="show">
				<table width="100%">
					<tr>
						<td width="100%"><h1>Edit Plan Details</h1></td>
						<td>Plan&nbsp;Name:&nbsp;</td>
						<td><input type="text" name="planName" id="planName" style="width: 80px"></td>
	...
	<form onsubmit="return validateForm();" name="policyMasterForm" action="PlanMaster.do" method="POST" >
	
		<input type="hidden" name="_method" value="${param._method == 'add' ? 'create' : 'update'}"/>
		<input type="hidden" name="plan_id" id="plan_id" value="${bean.map.plan_id}"/>
		<input type="hidden" name="updtdChargeHeadList" id="updtdChargeHeadList" value=""/>
		<input type="hidden" name="ip_applicable" id="ip_applicable" value="${bean.map.ip_applicable==null?'N':bean.map.ip_applicable}"/>
		<input type="hidden" name="op_applicable" id="op_applicable" value="${bean.map.op_applicable==null?'N':bean.map.op_applicable}"/>
		<input type="hidden" name="require_pbm_authorization" id="require_pbm_authorization" value="${bean.map.require_pbm_authorization==null?'N':bean.map.require_pbm_authorization}"/>
		<input type="hidden" name="limits_include_followup" id="limits_include_followup" value="${bean.map.limits_include_followup==null?'N':bean.map.limits_include_followup}"/>
			...
			<table class="formtable" cellpadding="0" cellspacing="0" width="100%" align="left">
				<tr>
					...
					<td class="forminfo">
						<c:choose>
    						<c:when test="${param._method !='add'}">
								<label>${bean.map.plan_name}</label>
								<input type="hidden" name="plan_name" id="plan_name"  value="${bean.map.plan_name}"/>
							</c:when>
							<c:otherwise>
								<input type="text" name="plan_name" id="plan_name" maxlength="100" value="${bean.map.plan_name}" />
							</c:otherwise>
						</c:choose>
					</td>
					...
					<td class="forminfo">
						<insta:selectdb name="insurance_co_id"  table="insurance_company_master" valuecol="insurance_co_id"
								displaycol="insurance_co_name"  filtered="false" 
									value="${bean.map.insurance_co_id}" orderby="insurance_co_name" onchange="populateInsuranceCategory();populateInsuranceTpaname();" onblur="populateInsuranceCategory();populateInsuranceTpaname();"/>
					</td>
				...
				<tr>
					...
					<td><insta:datewidget name="from_date" id="from_date" value="${from_date}"/></td>
					<td class="formlabel">Valid Upto :</td>
					<td><insta:datewidget name="to_date" id="to_date" value="${to_date}"/></td>
				</tr>
				<tr>
					<td class="formlabel">Plan Notes :</td>
					<td class="forminfo" colspan="5"><TEXTAREA COLS=95 ROWS=3 name="plan_notes"   id="plan_notes" >${bean.map.plan_notes}</TEXTAREA></td>
				</tr>
				<tr>
					<td class="formlabel">Plan Exclusions :</td>
					<td class="forminfo" colspan="5"><TEXTAREA COLS=95 ROWS=3 name="plan_exclusions"  id="plan_exclusions" >${bean.map.plan_exclusions}</TEXTAREA></td>
				</tr>
				<tr>
					...
					<td class= "forminfo">
						<input type="checkbox" name="requirePbmAuthz" id="requirePbmAuthz" value="Y" onclick="markApplicable(this);"
							<c:if test="${bean.map.require_pbm_authorization == 'Y' }"> checked </c:if> />
					</td>
					</c:if>
				
					
					 <td class="formlabel">Last Modified User :</td>
					<td class="forminfo"><b>${bean.map.username}</b><c:if test="${param._method =='add'}">--</c:if></td> 
					<td class="formlabel">Last Modified Date :</td>
					<td class="forminfo"><b><fmt:formatDate value="${bean.map.mod_time}" pattern="dd-MM-yyyy HH:mm"/></b><c:if test="${param._method =='add'}">--</c:if></td>
				</tr>
				<%-- <tr>
					<td class="formlabel">Over-All Treatment Limit :</td>
					<td class="forminfo"><input type="text" name="overall_treatment_limit" maxlength="8" class="numeric" onkeypress="return enterNumAndDotAndMinus(event);" value="${bean.map.overall_treatment_limit}"/></td>
				...
				<tr>
					...
					<td>
						<fmt:formatDate value="${bean.map.insurance_validity_start_date}" pattern="dd-MM-yyyy" var="start_date"/>
					...
					<td>
						<fmt:formatDate value="${bean.map.insurance_validity_end_date}" pattern="dd-MM-yyyy" var="end_date"/>
		...
		<table class="formtable" cellpadding="0" cellspacing="0" width="100%" align="left">
			<tr>
				<td class="formlabel"> Sponsor  : </td>
						<input type="hidden" name="tpaid" id="tpaid" value="${bean.map.sponsor_id}"/>
					
					<td class="forminfo"  <c:choose><c:when test="${param._method =='add'}">style="" </c:when><c:otherwise>style="valign:middle;"</c:otherwise> </c:choose>>  
				...
				<td class="formlabel">Plan Type :</td>
					<td class="forminfo" id="catStyle" <c:choose><c:when test="${param._method =='add'}">style="" </c:when><c:otherwise>style="valign:middle;"</c:otherwise> </c:choose>>
			...
			<tr>
				<td class="formlabel">Default Rate Plan :</td>
					...
					<insta:selectdb name="default_rate_plan" table="organization_details" valuecol="org_id"
							displaycol="org_name"
							value="${bean.map.default_rate_plan}"  orderby="org_name" dummyvalue="-- Select --" dummyvalueId="" />
					</td>
					...
					<select class= "dropdown"  name="discount_plan_id" >
					 	...
					 	<c:forEach var="defaultDiscountPlan" items="${defaultDiscountPlanList}"  varStatus="st" >
					 		 <option value='${defaultDiscountPlanList[st.index].discount_plan_id}'  
					 		 	${defaultDiscountPlanList[st.index].discount_plan_id == bean.map.discount_plan_id ? 'selected' : ''} >
				...
				<td class="formlabel" title="if unchecked, adds co-pay percent on the pre-discounted amount">
					...
					<td class= "forminfo" title="if unchecked, adds co-pay percent on the pre-discounted amount">
						<input type="hidden" name="is_copay_pc_on_post_discnt_amt" id="is_copay_pc_on_post_discnt_amt"
							value="${not empty bean.map.is_copay_pc_on_post_discnt_amt? bean.map.is_copay_pc_on_post_discnt_amt : 'N'}" />
						<input type="checkbox" name="is_copay_pc_on_post_discnt_amt_chk"
							...
							<c:if test="${ bean.map.is_copay_pc_on_post_discnt_amt eq 'Y' }"> checked </c:if> />
					</td>
				...
				<td class="formlabel"> IP-Applicable :</td>
					<td class= "forminfo">
						<input type="checkbox" name="ipApplicablez" id="ipApplicablez" value="Y" onclick="markApplicable(this);"
							<c:if test="${bean.map.ip_applicable eq 'Y' }"> checked </c:if> />
					</td>
				...
				<td class="formlabel"> OP-Applicable :</td>
					<td class= "forminfo">
						<input type="checkbox" name="opApplicablez" id="opApplicablez" value="Y" onclick="markApplicable(this);"
							<c:if test="${bean.map.op_applicable == 'Y' }"> checked </c:if> />
					</td>
			</tr>
		</table>
		</fieldset>
		...
		<fieldset class="fieldSetBorder" id="corporateInsu1"><legend class="fieldSetLabel"> DRG Payment Rules: </legend>
		<table class="formtable" cellpadding="0" cellspacing="0" width="100%" align="left">
			<tr>
				<td class="formlabel"> Base Rate: </td>
				<td class= "forminfo">
					<input type="text" name="base_rate" class="numeric"
						 	value="${bean.map.base_rate}"/>
...
It is a set amount that should be reviewed periodically in conjunction with updates to the base rate by the facility."
				...
				<td class= "forminfo">
					<input type="text" name="marginal_percent" maxlength="6" class="numeric"
						 	value="${bean.map.marginal_percent}"/>
						 <img class="imgHelpText"
...
there is no provision for an outlier payment."
			...
			<tr>
				...
				<td class= "forminfo">
					<input type="text" name="add_on_payment_factor" maxlength="6" class="numeric"
						 	value="${bean.map.add_on_payment_factor == null ? 75 : bean.map.add_on_payment_factor}"/>
			...
			<tr>
				...
				<td class= "forminfo">
					<input type="text" name="perdiem_copay_per" class="numeric"
						 	value="${bean.map.perdiem_copay_per}"/>
						 <img class="imgHelpText"
 title="Perdiem Co-pay(%) is the percentage of total perdiem amount to be collected as co-pay for a patient."
				...
				<td class= "forminfo">
					<input type="text" name="perdiem_copay_amount" class="numeric"
						 	value="${bean.map.perdiem_copay_amount}"/>
						 <img class="imgHelpText"
		...
		<table class="formtable" cellpadding="0" cellspacing="0" width="100%" >
			 <tr>
			    <td class="formlabel"><input type="checkbox" name="op_episode_visit" id="op_episode_visit" value="" onclick="updatevalues(this);markApplicable(this);"
			   							 <c:if test="${bean.map.limits_include_followup  == 'Y' }"> checked </c:if> />
			     </td>
			     <td> Limits Include Follow-up Visits</td>	
			     <td><input type="hidden" name="episode_visit_opvalue" id="episode_visit_opvalue" value="${bean.map.limits_include_followup  == 'Y' ? 'episode':'visit'}" />
			 ...
			 <tr>
				<td class="formlabel"  >Plan Limit : </td>
					<td >
						<input type="text" name="op_planlimit" id="op_planlimit" class="numeric"
						 	onkeypress="return enterNumOnly(event)" onchange="onPlanLimit(this,'op')"  value="${bean.map.op_plan_limit}"   maxlength="13"  />
					</td>
					 <td class="formlabel">Visit Sponsor Limit :
					</td>
				    <td class="forminfo" >
						<input type="text" name="episode_visit_op" id="episode_visit_op"  class="numeric" value="${bean.map.limits_include_followup == 'Y' ? bean.map.op_episode_limit:bean.map.op_visit_limit}"  
							onkeypress="return enterNumOnly(event)" onchange="onVisitSponsorLimit(this,'op')" maxlength="13" />
					</td>
					<td><input type="hidden" name="episode_visit_op1" id="episode_visit_op1" value="${bean.map.limits_include_followup == 'Y' ? bean.map.op_visit_limit:bean.map.op_episode_limit}" ></td>
			</tr> 
			<tr>
				<td class="formlabel" >Visit Deductible : </td>
					<td class="forminfo" >
						<input type="text" name="op_visit_deductible" id="op_visit_deductible" maxlength="13" value="${bean.map.op_visit_deductible}"
							onkeypress="return enterNumOnly(event)" onchange="onVisitdeductible(this,'op')" />
					</td>
				    <td class="formlabel" width="100px" >Visit Copay :</td>
					<td class="forminfo" >
						<input type="text" name="op_visit_copay" id="op_visit_copay" maxlength="13" value="${bean.map.op_copay_percent}"
					...
					<input type="text" name="op_visit_copay_limit"  id="op_visit_copay_limit" class="numeric" maxlength="13"
						 	onkeypress="return enterNumOnly(event)" onchange="onMaxCopay(this,'op')" value="${bean.map.op_visit_copay_limit}"/>
						 <img class="imgHelpText"
 title="OP Co-pay Limit is the amount of copay to be collected from a patient for a OP visit."
						...
						<table class="detailList"  id="chargesTable_o" cellpadding="0" cellspacing="0" width="100%">
							...
							<c:forEach var="record" items="${chargeBean}" varStatus="st">
								<c:if test="${record.patient_type eq 'o'}" >
									<tr <c:if test="${insuPayableMap[record.insurance_category_id] eq 'N'}"> title="This Insurance Item Category is not Insurance Payable."</c:if>>
										<c:choose>
										<c:when test="${insuPayableMap[record.insurance_category_id] eq 'Y'}">
											<td>
										...
										<td>
											...
											<input type="hidden" name="insuranceCategoryName" value="${record.insurance_category_name}" />
											<input type="hidden" name="_insurancePayable_o" value="${insuPayableMap[record.insurance_category_id] eq 'Y'}" />
										...
										<td align="right"  style="text-align:right;">
											...
											<input type="hidden" name="${record.insurance_category_id}_treatHidden_o"  id="${record.insurance_category_id}_treatHidden_o" value="${record.per_treatment_limit}"/>
										</td>
										</c:when>
										<c:otherwise>
											<td align="right" style="text-align:right;">
										...
										<c:when test="${insuPayableMap[record.insurance_category_id] eq 'Y'}">
											<td align="right" style="text-align:right;">
												...
												<input type="hidden" name="${record.insurance_category_id}_catAmtHidden_o"  id="${record.insurance_category_id}_catAmtHidden_o" value="${record.patient_amount_per_category}"/>
											</td>
										</c:when>
										<c:otherwise>
											<td align="right" style="text-align:right;">
										...
										<c:when test="${insuPayableMap[record.insurance_category_id] eq 'Y'}">
											<td align="right" style="text-align:right;">
												...
												<input type="hidden" name="${record.insurance_category_id}_amtHidden_o"  id="${record.insurance_category_id}_amtHidden_o" value="${record.patient_amount}"/>
											</td>
										</c:when>
										<c:otherwise>
											<td align="right" style="text-align:right;">
										...
										<td align="right"  style="text-align:right;">
											...
											<input type="hidden" name="${record.insurance_category_id}_perHidden_o"  id="${record.insurance_category_id}_perHidden_o" value="${record.patient_percent}"/>
										</td>
										</c:when>
										<c:otherwise>
											<td align="right" style="text-align:right;">
										...
										<td align="right"  style="text-align:right;">
											...
											<input type="hidden" name="${record.insurance_category_id}_capHidden_o"  id="${record.insurance_category_id}_capHidden_o" value="${record.patient_amount_cap}"/>
										</td>
										</c:when>
										<c:otherwise>
											<td align="right" style="text-align:right;">
						...
						<table  style="padding-top:0;border-top:0 solid #E0E0E0" class="detailList" align="right" cellpadding="0" cellspacing="0" width="100%" >
							<tr align="right">
		...
		<table class="formtable" cellpadding="0" cellspacing="0" width="100%" >
			 ...
			 <tr>
				<td class="formlabel"  >Plan Limit : </td>
					<td >
						<input type="text" name="ip_planlimit" id="ip_planlimit" class="numeric" maxlength="13"
						 	onkeypress="return enterNumOnly(event)" onchange="onPlanLimit(this,'ip')" value="${bean.map.ip_plan_limit}"   />
					</td>
					 <td class="formlabel">Visit Sponsor Limit :
					</td>
				    <td class="forminfo" >
						<input type="text" name="episode_visit_ip" id="episode_visit_ip"  class="numeric" value="${bean.map.ip_visit_limit}" 
				    ...
				    <td class="forminfo" >
						<input type="text" name="ip_per_day_limit" id="ip_per_day_limit"  class="numeric"
						 	onkeypress="return enterNumOnly(event)" onchange="onPerDayLimit(this)" maxlength="13" value="${bean.map.ip_per_day_limit}"   />
					</td>
			...
			<tr>
				<td class="formlabel" >Visit Deductible : </td>
					<td class="forminfo" >
						<input type="text" name="ip_visit_deductible" id="ip_visit_deductible" maxlength="13" value="${bean.map.ip_visit_deductible}"
						 onkeypress="return enterNumOnly(event)" onchange="onVisitdeductible(this,'ip')" />
					</td>
				    <td class="formlabel" width="100px" >Visit Copay :</td>
					<td class="forminfo" >
						<input type="text" name="ip_visit_copay" id="ip_visit_copay" maxlength="13" value="${bean.map.ip_copay_percent}" 
					...
					<input type="text" name="ip_visit_copay_limit"  id="ip_visit_copay_limit" class="numeric" maxlength="13"
						 	onkeypress="return enterNumOnly(event)" onchange="onMaxCopay(this,'ip')" value="${bean.map.ip_visit_copay_limit}"/>
						 <img class="imgHelpText"
					...
					<c:if test="${record.patient_type eq 'i'}" >
						<tr <c:if test="${insuPayableMap[record.insurance_category_id] eq 'N'}"> title="This Insurance Item Category is not Insurance Payable."</c:if>>
							...
							<td>
								...
								<input type="hidden" name="insuranceCategoryId" value="${record.insurance_category_id}" />
								<input type="hidden" name="insuranceCategoryName" value="${record.insurance_category_name}" />
								<input type="hidden" name="_insurancePayable_i" value="${insuPayableMap[record.insurance_category_id] eq 'Y'}" />
							...
							<td align="right"  style="text-align:right;">
								...
								<input type="hidden" name="${record.insurance_category_id}_treatHidden_i"  id="${record.insurance_category_id}_treatHidden_i" value="${record.per_treatment_limit}"/>
							</td>
							</c:when>
							<c:otherwise>
								<td align="right" style="text-align:right;">
							...
							<c:when test="${insuPayableMap[record.insurance_category_id] eq 'Y'}">
								<td align="right" style="text-align:right;">
									...
									<input type="hidden" name="${record.insurance_category_id}_catAmtHidden_i"  id="${record.insurance_category_id}_catAmtHidden_i" value="${record.patient_amount_per_category}"/>
								</td>
							</c:when>
							<c:otherwise>
								<td align="right" style="text-align:right;">
							...
							<c:when test="${insuPayableMap[record.insurance_category_id] eq 'Y'}">
								<td align="right" style="text-align:right;">
									...
									<input type="hidden" name="${record.insurance_category_id}_amtHidden_i"  id="${record.insurance_category_id}_amtHidden_i" value="${record.patient_amount}"/>
								</td>
							</c:when>
							<c:otherwise>
								<td align="right" style="text-align:right;">
							...
							<td align="right"  style="text-align:right;">
								...
								<input type="hidden" name="${record.insurance_category_id}_perHidden_i"  id="${record.insurance_category_id}_perHidden_i" value="${record.patient_percent}"/>
							</td>
							</c:when>
							<c:otherwise>
								<td align="right" style="text-align:right;">
							...
							<td align="right"  style="text-align:right;">
								...
								<input type="hidden" name="${record.insurance_category_id}_capHidden_i"  id="${record.insurance_category_id}_capHidden_i" value="${record.patient_amount_cap}"/>
							</td>
							</c:when>
							<c:otherwise>
								<td align="right" style="text-align:right;">
							...
							<table class="formTable" align="center">
								...
								<tr>
									<td class="formlabel">Pre Auth Required:</td>
							 	 	<td class="forminfo">
							 	 		<select name="_category_prior_auth_required_fld" class="dropdown" id="_category_prior_auth_required_fld">
	...
	<div id="myDialog1" style="display:block;visibility:hidden;">
							...
							<fieldset class="fieldSetBorder" style="width:460px;"><legend class="fieldSetLabel">Edit Item-Category Charge Details</legend>
									...
									<table class="formTable" align="center">
										...
										<tr>
											<td class="formlabel">Pre Auth Required:</td>
										 	 	<td class="forminfo">
										 	 		<select name="_category_prior_auth_required_fld1" class="dropdown" id="_category_prior_auth_required_fld1">
										 	 			<option value="">Not Defined</option>
										 	 			<option value="N">Never</option>
														<option value="S">Sometimes</option>
														<option value="A">Always</option>
		...
		<div class="screenActions">
			...
			|
			<c:if test="${param._method != 'add'}">
			<a href="${cpath}/master/PlanMaster.do?_method=add">Add</a>

			</c:if>
			|<a href="${cpath}/master/PlanMaster.do?_method=list">Plan List</a>
