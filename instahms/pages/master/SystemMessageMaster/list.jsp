...
<head>
	...
	var toolbar = {
		Edit: {
			...
			href: 'master/SystemMessageMaster.do?method=show',
			onclick: null,
		...
		DELETE: {
			...
			href: 'master/SystemMessageMaster.do?method=delete',
			onclick: null,
...
<body onload="init()">
	...
	<div class="resultList">
		<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
			...
			<c:forEach var="record" items="${beanList}" varStatus="st">
				<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
					onclick="showToolbar(${st.index}, event, 'resultTable',
					{message_id: '${record.map.message_id}'},'');" id="toolbarRow${st.index}">
					<td>${record.map.display_order}</td>
					<td>${record.map.messages}</td>
					<td>${severityDisplay[record.map.severity]}</td>
				</tr>
			</c:forEach>
		</table>
	</div>

	<c:url var="Murl" value="SystemMessageMaster.do">
