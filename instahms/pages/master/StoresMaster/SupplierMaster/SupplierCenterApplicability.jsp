...
<html>
	<head>
		...
		<title>Add/Edit Supplier Center Applicability - Insta HMS</title>
		<insta:link type="script" file="stores/suppliercenterapplicability.js"/>
		<script>
	...
	<body onload="init();">
		<h1>Center Applicability - ${supplier_bean.map.supplier_name}</h1>
		<insta:feedback-panel/>
		<form action="SupplierCenterApplicability.do" name="supplier_center_association_form" method="POST">
			<input type="hidden" name="_method" value="update"/>
			<input type="hidden" name="supplier_id" value="${ifn:cleanHtmlAttribute(param.supplier_id)}"/>
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>
					...
					<div style="padding-top: 5px; float: left">Suppliers associated with all centers</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">Supplier applicable only for following centers</div>
				</div>
				<div style="clear: both"></div>
					...
					<tr>
			 		...
			 		<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="supp_center_id" value="${app_center.supp_center_id}"/>
				...
				<tr>
					<td>
						<input type="button" name="Save" value="Save"
							...
							<c:url value="/pages/masters/insta/stores/suppdetails.do" var="supplierListUrl">
								<c:param name="_method" value="getSupplierDashBoard"/>
								<c:param name="status" value="A"/>
								<c:param name="sortOrder" value="supplier_name"/>
								<c:param name="sortReverse" value="false"/>
							</c:url>
						| <a href="${supplierListUrl}" title="Suppliers List">Suppliers List</a>
					 </td>
				</tr>
			</table>
