...
<head>
	...
	<title>Patient Visit Bills Template - Insta HMS</title>
	...
	<script>
		...
		function doSave() {
			var name = document.mainform.template_name.value;
			var reason = document.mainform.reason.value;
			if(name == "") {
				alert("Enter template name");
				document.mainform.template_name.focus();
				return false;
			}
			if (reason == ""){
				alert("Enter  reason for print customization");
				document.mainform.reason.focus();
				return false;
			}
			if (!checkDuplicates()) {
			...
			document.mainform.action= "PatientVisitBillsPrintTemplate.do?method="+saveMethod;
			document.mainform.submit();
		}

		function doReset() {
			document.mainform.action= "PatientVisitBillsPrintTemplate.do?method=resetToDefault";
			document.mainform.submit();
		}
		...
		function checkDuplicates() {
			...
			for(var i=0; i<templates.length; i++) {
				if(dbTemplateName.toUpperCase() != templates[i].template_name.toUpperCase()) {
					if (inputTemplateName.toUpperCase() == templates[i].template_name.toUpperCase()) {
		...
		<table class="screenActions">
			<tr>
				...
				<c:if test="${templateMode == 'H'}">
					<c:url var="changeEditor" value="PatientVisitBillsPrintTemplate.do">
				...
				<c:url var="dashboardUrl" value="PatientVisitBillsPrintTemplate.do">
						<c:param name="method" value="list"/>
				</c:url>
				<c:if test="${param.method=='show'}" >
					<td>&nbsp;|&nbsp;</td>
					<td><a href="#" onclick="window.location.href='${path}/master/PatientVisitBillsPrintTemplate.do?method=templateMode'">Add</a></td>
				</c:if>
				<td>&nbsp;|<a href="javascript:void(0)" onclick="return gotoLocation('${dashboardUrl}')">Patient Visit Bills Template List</a></td>
			</tr>
		</table>
	</form>
