...
<html>
	<head>
		...
		<script>
			function deleteSelected(e) {
				var deleteEl = document.getElementsByName("deletePrint");
			...
			var toolbar = {
				Edit: {
					...
					href: 'master/PatientVisitBillsPrintTemplate.do?method=show',
					onclick: null,
					description: "View and/or Edit Patient Visit Template"
					}
			};
			function init()
			{
	...
	<body onload="init();">
		<h1>Patient Visit Bills Template</h1>

		<insta:feedback-panel/>

		<form method="POST" action="PatientVisitBillsPrintTemplate.do">
				...
				<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
					...
					<c:forEach var="temp" items="${templateList}" varStatus="st">
						<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
							...
							<td><input type="checkbox" name="deletePrint" id="deletePrint"
			...
			<c:url var="url" value="PatientVisitBillsPrintTemplate.do">
				<c:param name="method" value="templateMode"/>
			</c:url>

			<div class="screenActions">
				<input type="submit" name="delete" value="Delete" onclick="deleteSelected(event)"/>&nbsp;|
				<a href="<c:out value='${url}'/>"> Add New Patient Visit Bills Template</a>
			</div>
