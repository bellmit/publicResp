...
<head>
	...
	<title>Add Consultation Print Template - Insta HMS</title>
	...
	<script>
		...
		if (templateMode == 'H' && (editorMode == 'tinyMCE')) {
			initEditor("prescription_template_content", '${cpath}','${pref.font_name}', '${pref.font_size}');
		}

		function doSave() {
			var templateName = document.prescriptionPrint.template_name.value;
			var reason = document.prescriptionPrint.reason.value
			if (templateName == '') {
				alert("Enter Template Name");
				document.prescriptionPrint.name.focus();
				return false;
			}
			...
			if (reason = '') {
				...
				document.prescriptionPrint.reason.focus();
				return false;
			}
			...
			document.prescriptionPrint.action = "PrescriptionsPrintTemplates.do?method="+saveMethod;
			document.prescriptionPrint.submit();
		}

		function doReset() {
			document.prescriptionPrint.action= "PrescriptionsPrintTemplates.do?method=resetToDefault";
			document.prescriptionPrint.submit();
		}
...
<body>
	...
	<form method="POST" name="prescriptionPrint">
		...
		<div style="margin-top:10;">
			<textarea id="prescription_template_content" name="prescription_template_content"
					style="width:800px; height:450px;font-family: Courier,fixed"><c:out value="${prescription_template_content}"/></textarea>
		</div>
		...
		<table class="screenActions">
			<tr>
				...
				<c:url var="dashboardUrl" value="PrescriptionsPrintTemplates.do">
						<c:param name="method" value="list"/>
				</c:url>
				<c:if test="${param.method=='show'}" >
					<td>&nbsp;|&nbsp;</td>
					<td><a href="#" onclick="window.location.href='${cpath}/master/PrescriptionsPrintTemplates.do?method=templateMode'">Add</a></td>
				</c:if>
				<td>&nbsp;|<a href="javascript:void(0)" onclick="return gotoLocation('${dashboardUrl}')"> Consultation Print Templates List</a></td>
			</tr>
		</table>
	</form>
