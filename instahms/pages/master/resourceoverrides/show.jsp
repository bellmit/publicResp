...
<body onload="init('edit')">
	...
	<h1>${editText} <insta:ltext key="patient.resourcescheduler.editresourceavailability.h1"/></h1>
	...
	<form name="ResourceAvailabilityForm" method="POST" action="update.htm" autocomplete="off">
			...
			<table class="formtable">
				<tr>
						...
						<label><b>
							<c:choose>
								<c:when test="${param.res_sch_type=='DOC'}">
									<insta:ltext key="patient.resourcescheduler.editresourceavailability.doctorconsultation"/>
								</c:when>
								<c:otherwise>
									<c:forEach var="resource" items="${categoryDescripton}">
										<c:if test="${param.res_sch_type == resource.map.res_sch_type}">
							...
							<input type="hidden" name="res_sch_type" id="res_sch_type" value="${bean.map.res_sch_type}">
						</label>
					</td>
					<td class="formlabel"><insta:ltext key="patient.resourcescheduler.editresourceavailability.resources"/></td>
						<td>
							<label>
								<b>${bean.map.resource_name}</b>
								<input type="hidden" name="_resource_name" id="_resource_name">
								<input type="hidden" name="res_sch_name" id="res_sch_name" value="${bean.map.res_sch_name}">
					...
					<td class="formlabel"><insta:ltext key="patient.resourcescheduler.editresourceavailability.availabilitydate"/></td>
						<td><label style="text-align:bottom"><b><fmt:formatDate pattern="dd-MM-yyyy" value="${bean.map.availability_date}"/></b></label></td>
						<input type="hidden" name="availability_date" id="availability_date" value="${bean.map.availability_date}" />
				</tr>
			</table>
		</fieldset>
		    			...
		    			<tr ${style} class="${records.map.availability_status == 'N' ? 'notAvailableRow': ''}">
		    				...
		    				<td>
								...
								<c:choose>
									...
									<c:when test="${records.map.availability_status == 'A' && (centerId == 0 || centerId == records.map.center_id)}">
										<c:if test="${param.res_sch_type == 'DOC'}">
											<a title="">
												<img src="${cpath}/icons/Delete.png" class="imgDelete button" onclick="deleteItem(this)" title='<insta:ltext key="patient.resourcescheduler.editresourceavailability.delete.resource"/>'/>
											</a>
