...
<head>
	...
	<script>
    	var existPriority = ${existingPriority};
		function doCancel(){
		...
		function validateForm(){
			...
			if(document.insuranceItemCatMasterForm.priority.value==""||document.insuranceItemCatMasterForm.priority.value==null)
			{
		...
		function checkPriorityEmpty(obj) {
			...
			if(document.getElementById("beanPriority").value == obj.value)
				return;
			for(var i =0 ; i<existPriority.length;i++){
				if(obj.value==existPriority[i]){
					alert("Priority "+existPriority[i]+" already exists..");
					document.insuranceItemCatMasterForm.priority.value = document.getElementById("beanPriority").value;
					break;
				}
			}
		}
...
<c:choose>
	...
	<form onsubmit="return validateForm();" name="insuranceItemCatMasterForm" action="${cpath}/Insurance/ItemInsuranceCategoryMaster.do" method="POST" >
		...
		<input type="hidden" id="beanPriority" value="${bean.map.priority}"/>
		...
		<fieldset class="fieldSetBorder" ><legend class="fieldSetLabel">Item Insurance Category Details</legend>
			...
			<table class="formtable">
				...
				<tr>
					...
					<td><input type="text" class="number" name="priority" onkeypress="return enterNumOnlyzeroToNine(event)" onblur="checkPriorityEmpty(this)" value="${bean.map.priority}"/></td>					
					<td class="formlabel">Display Order:</td>
					<td><input type="text" name="display_order" onkeypress="return enterNumOnlyzeroToNine(event)" value="${bean.map.display_order}"/></td>
				</tr>
			</table>
