...
<c:set target="${paymentTypeDisplay}" property="S" value="Sponsor Receipts"/>
<c:set target="${paymentTypeDisplay}" property="*" value="All"/>

<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
	<title>Receipt Number Sequence Preferences List - Insta HMS</title>
	...
	<script type="text/javascript">
		var toolbar = {
			Edit: {
				...
				onclick: null,
				description: "View and/or Receipt Number Sequence Preference details"
				}
		};
		function init()
		{
...
<body onload="init();">
	...
	<h1><insta:ltext key="patient.sequences.receiptnosequences.receiptnosequencepreference"/></h1>

	<insta:feedback-panel/>

	<form name="ReceiptNumberSequencePrefSearchForm" method="GET">
		<input type="hidden" name="sortOrder" value="${ifn:cleanHtmlAttribute(param.sortOrder)}"/>
		<input type="hidden" name="sortReverse" value="${ifn:cleanHtmlAttribute(param.sortReverse)}"/>
		
		<insta:search-lessoptions form="ReceiptNumberSequencePrefSearchForm" >
			<div class="searchBasicOpts">
				<div class="sboField">
					<div class="sboFieldLabel"><insta:ltext key="patient.sequences.receiptnosequences.patternid"/>:</div>
		...
		<div class="resultList">
			<table class="resultList" cellspacing="0" cellpadding="0" id="resultTable" onmouseover="hideToolBar('');">
				<tr onmouseover="hideToolBar();">
					...
					<th><insta:ltext key="patient.sequences.receiptnosequences.priority"/></th>
					<th><insta:ltext key="patient.sequences.receiptnosequences.visittype"/></th>
					<th><insta:ltext key="patient.sequences.receiptnosequences.billtype"/></th>
					<th><insta:ltext key="patient.sequences.receiptnosequences.restrictiontype"/></th>
					<th><insta:ltext key="patient.sequences.receiptnosequences.centerid"/></th>
					<th><insta:ltext key="patient.sequences.receiptnosequences.paymenttype"/></th>
				</tr>
				<c:forEach var="record" items="${pagedList.dtoList}" varStatus="st">
					<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}"
						onclick="showToolbar(${st.index}, event, 'resultTable',
						 {pattern_id: '${record.pattern_id}',receipt_number_seq_id: '${record.receipt_number_seq_id}'},'');" id="toolbarRow${st.index}">
						...
						<td>${paymentTypeDisplay[record.payment_type]}</td>
					</tr>
				</c:forEach>
			</table>
		...
		<div class="screenActions" style="float:left"><a href="<c:out value='${url}' />"><insta:ltext key="patient.sequences.receiptnosequences.addnewreceiptnosequencepreference"/></a></div>
