...
<head>
	...
	function doClose() {
		window.location.href = "${cpath}/resourcemanagement/contractsType.do?_method=list&sortOrder=contract_type&sortReverse=false&status=A";
	}
</script>
...
<form action="contractsType.do" method="POST">
	<input type="hidden" name="_method" value="${param._method == 'add' ? 'create' : 'update'}">
	<c:if test="${param._method == 'show'}">
		<input type="hidden" name="contract_type_id" value="${bean.contract_type_id}"/>
	</c:if>

	<div class="pageHeader">${param._method == 'add' ? 'Add' : 'Edit'} Contract Type</div>
	...
	<table class="screenActions">
		<tr>
			...
			<c:if test="${param._method=='show'}">
			<td>&nbsp;|&nbsp;</td>
			<td><a href="#" onclick="window.location.href='${cpath}/resourcemanagement/contractsType.do?_method=add'">Add</a></td>
			</c:if>
			<td>&nbsp;|&nbsp;</td>
			<td><a href="javascript:void(0)" onclick="doClose();">Contract Type</a></td>
		</tr>
	</table>
