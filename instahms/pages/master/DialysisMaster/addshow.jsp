...
<insta:link type="script" file="hmsvalidation.js"/>
	<script>
		function doClose() {
			window.location.href = "${cpath}/master/dialyzerType.do?_method=list&status=A&sortOrder=dialyzer_type_name&sortReverse=false";
		}
...
<body>
	<form action="dialyzerType.do" method="POST">
		...
		<h1>${param._method == 'add' ? addText : editText} <insta:ltext key="generalmasters.dialyzertypes.addoredit.dialyzertype"/></h1>
...
<c:set var="status">
 ...
 <insta:ltext key="generalmasters.dialyzertypes.list.inactive"/>
		...
		<c:if test="${param._method == 'show'}">
			<input type="hidden" name="dialyzer_type_id" value="${bean.map.dialyzer_type_id}"/>
		...
		<fieldset class="fieldsetborder">
			...
			<table class="formtable">
				<tr>
					...
					<td>
						 <input type="text" name="dialyzer_type_name" value="${bean.map.dialyzer_type_name}"
		...
		<tr>
			...
			<c:if test="${param._method=='show'}">
			<td>&nbsp;|&nbsp;</td>
			<td><a href="#" onclick="window.location.href='${cpath}/master/dialyzerType.do?_method=add'"><insta:ltext key="generalmasters.dialyzertypes.addoredit.add"/></a></td>
			</c:if>
			<td>&nbsp;|&nbsp;</td>
