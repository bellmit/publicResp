...
<c:set var="pagePath" value="<%=URLRoute.STORE_PATH %>"/>
	<script>
		...
		function enableAllowRaiseBill() {
			...
			if (isSalesStore == 'N' ) allowRaiseBillObj.checked = false;
		}
		function enableSellingPriceForBatch() {
			var storeTariffValue = document.getElementById("store_rate_plan_id").value;
			var sellingPriceForBatchObj = document.getElementById("batch_selling_price_id");
		...
		function validate() {
			...
			var autoPoGeneration = document.getElementById('allow_auto_po_generation_tmp').value;
			
			if(autoPoGeneration == 'Y'){
					valid = valid && validateRequired(document.storemasterform.auto_po_generation_frequency_in_days,"Enter Frequency Days");
					
					if ( !valid ){
						document.storemasterform.auto_po_generation_frequency_in_days.focus();
						return false;
					}
			}else{
			...
			if(autoPoCancel == 'Y'){
					valid = valid && validateRequired(document.storemasterform.auto_cancel_po_frequency_in_days,"Enter Frequency Days");
					
					if ( !valid ){
						document.storemasterform.auto_cancel_po_frequency_in_days.focus();
						return false;
					}
			}else{
		...
		function chooseAccountGroup() {
			...
			if (counterId != '') {
				...
				req.open("GET", "${cpath}/master/StoreMaster/list.do?_method=getAccountGrpId&counterId="+counterId, true);
		...
		Insta.masterData=${ifn:convertListToJson(storesLists)};
    	   ...
    	   document.storemasterform.auto_po_generation_frequency_in_days.readOnly = obj.value == 'N' || document.storemasterform.allow_auto_cancel_po.value == 'N';
    	   
    	   document.storemasterform.allow_auto_cancel_po_tmp.disabled = obj.value == 'N';
    	   document.storemasterform.auto_cancel_po_frequency_in_days.readOnly = obj.value == 'N' || document.storemasterform.allow_auto_cancel_po.value == 'N';
       }
...
<h1 style="float:left">Edit Store</h1>
<c:url var="searchUrl" value="/master/StoreMaster.do"/>
<insta:findbykey keys="dept_name,dept_id" fieldName="dept_id" method="show" url="${searchUrl}"/>
        

<c:set var="actionUrl" value="${cpath}${pagePath}/update.htm?dept_id=${bean.dept_id}"/>
<form action="${actionUrl}"  name="storemasterform" method="POST">
	...
	<table class="formtable" >
		<tr>
			...
			<td>
				<input type="text" name="dept_name" value="${bean.dept_name}" onblur="capWords(dept_name)"
			...
			<td>
				<select name="counter_id" id="counter_id" class="dropdown" onchange="chooseAccountGroup();">
		...
		<tr>
			...
			<td><input type="text" name="pharmacy_tin_no" value="${bean.pharmacy_tin_no}"/></td>
			<td class="formlabel">Drug License Number:</td>
			<td>
				<input type="text" name="pharmacy_drug_license_no"
				value="${bean.pharmacy_drug_license_no}"/>
			</td>
		</tr>
		<tr>
			<td class="formlabel">Auto fill prescriptions:</td>
			<td>
				<insta:selectoptions name="auto_fill_prescriptions" value="${bean.auto_fill_prescriptions}" opvalues="false,true"
				 optexts= "No,Yes"/>
			</td>
			<td class="formlabel">Account Group:</td>
			<td>
				<insta:selectdb name="account_group" id="account_group" value="${bean.account_group}" table="account_group_master"
			...
			<td><input type="text" name="sales_store_vat_account_prefix" id="sales_store_vat_account_prefix"
		...
		<td class="formlabel">Purchase Account Prefix(GST):</td>
			...
			<td><input type="text" name="purchases_store_vat_account_prefix" id="purchases_store_vat_account_prefix"
				value="${bean.purchases_store_vat_account_prefix}"/></td>
			<td class="formlabel">Purchase Account Prefix(CST):</td>
			<td><input type="text" name="purchases_store_cst_account_prefix" id="purchases_store_cst_account_prefix"
				value="${bean.purchases_store_cst_account_prefix}"/></td>
			<td class="formlabel">Store Type:</td>
			<td>
				<insta:selectdb name="store_type_id" value="${bean.store_type_id}" table="store_type_master"
						valuecol="store_type_id" displaycol="store_type_name" filtered="false" dummyvalue="..select.."/>
			</td>
		</tr>
		<tr>
			<td class="formlabel">Is Super Store:</td>
			<td>
				<insta:selectoptions name="is_super_store" value="${bean.is_super_store}" opvalues="Y,N" optexts="Yes,No" onchange="checkAutoPOFields(this)" />
			</td>
			<td class="formlabel">Sale Units:</td>
			<td>
				<insta:selectoptions name="sale_unit" value="${bean.sale_unit}" opvalues="I,P" optexts="Issue,Package"/>
			</td>
			<td class="formlabel">Allowed to raise Bill:</td>
            <td>
            	<insta:selectoptions name="allowed_raise_bill" value="${bean.allowed_raise_bill}" opvalues="N,Y" optexts="No,Yes" />
            </td>
		</tr>
		<tr>
			<td class="formlabel">Prescription Print Template:</td>
			<td >
				<select name="presc_template_name" id="presc_template_name" class="dropdown">
			...
			<td >
				<select name="presc_lbl_template_name" id="presc_lbl_template_name" class="dropdown">
					<option value="BUILTIN_HTML">Built-in Default Template</option>
					<option value="BUILTIN_TEXT">Built-in Text Template</option>
			...
			<td>
				<select name="template_name" id="template_name" class="dropdown">
					<option value="BUILTIN_HTML">Built-in Default Template</option>
					<option value="BUILTIN_TEXT">Built-in Text Template</option>
		...
		<tr>
            ...
            <td>
                  <insta:selectoptions name="is_sales_store" id="is_sales_store" value="${bean.is_sales_store}" opvalues="Y,N"
                  optexts= "Yes,No" onchange="enableAllowRaiseBill()"/>
             </td>
             <td class="formlabel">Auto Fill Indents:</td>
            <td>
                  <insta:selectoptions name="auto_fill_indents" value="${bean.auto_fill_indents}" opvalues="false,true"
					 optexts= "No,Yes"/>
             </td>
             <td class="formlabel">Is Sterile Store:</td>
             <td>

             	<insta:selectoptions name="is_sterile_store" value="${bean.is_sterile_store}" opvalues="N,Y"  dummyvalue="--None--" dummyvalueId=""
					 optexts= "No,Yes"/>
             </td>
		</tr>
		<tr>
			...
			<td class="forminfo">
				<insta:selectdb name="store_rate_plan_id" id = "store_rate_plan_id" table="store_rate_plans" valuecol="store_rate_plan_id"
					displaycol="store_rate_plan_name" orderby="store_rate_plan_name" value="${bean.store_rate_plan_id}"
					dummyvalue="Default" dummyvalueId="" onchange="enableSellingPriceForBatch()"/>
			</td>
			<td class="formlabel">Use Selling Prices from Item Batch:</td>
			<td class="forminfo">
				<insta:selectoptions name="use_batch_mrp" value="${bean.use_batch_mrp}" opvalues="N,Y" optexts= "No,Yes"/> <img class="imgHelpText"
		...
		<tr>
			...
			<td class="forminfo">
				<insta:selectoptions name="allow_auto_po_generation_tmp" id="allow_auto_po_generation_tmp" value="${bean.allow_auto_po_generation}" opvalues="N,Y"
                  optexts= "No,Yes" disabled="${bean.is_super_store == 'N' }"  onchange="setAutoPoValue(this,'allow_auto_po_generation','auto_po_generation_frequency_in_days');checkFrequencyFields(this,'allow_auto_po_generation','auto_po_generation_frequency_in_days')"/>
                  <input type="hidden" name="allow_auto_po_generation" id="allow_auto_po_generation" value="${bean.allow_auto_po_generation}"/>
                  
			</td>
			<td class="formlabel">PO generation Frequency(days):</td>
			<td class="forminfo">
				<input type="text" id="auto_po_generation_frequency_in_days" name="auto_po_generation_frequency_in_days" onkeypress="return enterNumOnlyzeroToNine(event)"
					value="${bean.auto_po_generation_frequency_in_days}"  ${bean.is_super_store == 'N' || bean.allow_auto_po_generation == 'N' ? 'readonly' : ''} autocomplete="off"
					<c:if test="${bean.allow_auto_po_generation != 'Y' }"> disabled </c:if>  />
			</td>
			<td class="formlabel">Auto Cancel PO :</td>
			<td class="forminfo">
				<insta:selectoptions name="allow_auto_cancel_po_tmp" id="allow_auto_cancel_po_tmp" value="${bean.allow_auto_cancel_po}" opvalues="N,Y"
                  optexts= "No,Yes" disabled="${bean.is_super_store == 'N' }"  onchange="setAutoPoValue(this,'allow_auto_cancel_po','auto_cancel_po_frequency_in_days');checkFrequencyFields(this,'allow_auto_cancel_po','auto_cancel_po_frequency_in_days')"/>
                  <input type="hidden" name="allow_auto_cancel_po" id="allow_auto_cancel_po" value="${bean.allow_auto_cancel_po}"/>
			</td>
		</tr>
		<tr>
			<td class="formlabel">Auto Cancel PO Frequency(days):</td>
			<td class="forminfo">
				<input type="text" id="auto_cancel_po_frequency_in_days" name="auto_cancel_po_frequency_in_days" onkeypress="return enterNumOnlyzeroToNine(event)"
					value="${bean.auto_cancel_po_frequency_in_days}"  ${bean.is_super_store == 'N' || bean.allow_auto_cancel_po == 'N' ? 'readonly' : ''} autocomplete="off"
