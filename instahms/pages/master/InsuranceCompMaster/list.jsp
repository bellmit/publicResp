...
<head>
	...
	<title>Insurance Company Master - Insta HMS</title>
	...
	<script type="text/javascript">
		var toolBar = {
			Edit : {
				...
				href : "/master/InsuranceCompMaster.do?_method=show",
				onclick : null,
				description : "View and/or Edit the contents Insurance Company"
				}
		};
...
<body onload="init()">
	...
	<form name="InsuranceCompForm" method="GET">
		...
		<insta:search-lessoptions form="InsuranceCompForm" >
			...
			<table class="resultList" cellspacing="" cellpadding="" id="resultTable" onmouseover="hideToolBar();">
				...
				<c:forEach var="record" items="${pagedList.dtoList}" varStatus="st">
					<tr class="${st.index == 0 ? 'firstRow' : ''} ${st.index % 2 == 0 ? 'even' : 'odd'}" id="toolbarRow${st.index}"
						onclick="showToolbar(${st.index}, event, 'resultTable', {insurance_co_id: '${record.insurance_co_id}'},'');">
						...
						<td>${record.insurance_co_email}</td>
						<td>${record.insurance_co_phone}</td>
					</tr>
				</c:forEach>
			</table>
		...
		<c:url var="Url" value="InsuranceCompMaster.do">
			<c:param name="_method" value="add"/>
		</c:url>

		<div class="screenActions" style="float: left">
			<a href="${Url}">Add New Insurance Company</a>
		</div>
