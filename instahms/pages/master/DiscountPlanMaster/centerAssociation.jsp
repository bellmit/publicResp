...
<html>
	<head>
		...
		<insta:link type="script" file="master/DiscountPlanAvailability/discount_plan_applicabilty.js"/>
	...
	<body onload="init();">
		<h1>Center Applicability - ${bean.entity_name}</h1>
		<insta:feedback-panel/>
		<form action="DiscountPlanCenterAssociation.do" name="center_association_form" method="POST">
			<input type="hidden" name="_method" value="updateAssociation"/>
			<input type="hidden" name="entity_id_column_name" value="discount_plan_id"/>
			<input type="hidden" name="entity_id" value="${bean.discount_plan_id }"/>
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>  
					...
					<div style="padding-top: 5px; float: left">Discount Plan Available For All Centers</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">Discount Plan Available For Following Centers</div>
				</div>
				<div style="clear: both"></div>
					...
					<tr>
			 		...
			 		<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="discount_plan_center_id" value="${app_center.discount_plan_center_id}"/>
				...
				<tr>
					<td>
						<input type="button" name="Save" value="Save"
							onclick="return saveForm('center_association_form');">
							<c:url value="/master/discountplans/list.htm" var="discountPlanListUrl">
								<c:param name="sortOrder" value="discount_plan_name"/>
								<c:param name="sortReverse" value="false"/>
							</c:url>
						| <a href="${discountPlanListUrl}" title="Discount Plan List">Discount Plan List</a>
					 </td>
				</tr>
			</table>
