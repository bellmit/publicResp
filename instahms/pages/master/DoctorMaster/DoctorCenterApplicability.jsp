...
<html>
	<head>
		...
		<title>Add/Edit Doctor Center Applicability - Insta HMS</title>
		<insta:link type="script" file="master/DoctorApplicability/doctorcenterapplicability.js"/>
		<script>
	...
	<body onload="init();">
		<h1>Center Applicability - ${doctor_bean.map.doctor_name}</h1>
		<insta:feedback-panel/>
		<form action="DoctorCenterApplicability.do" name="doctor_center_association_form" method="POST">
			...
			<fieldset class="fieldSetBorder" style="display: ${max_centers > 1 ? 'block' : 'none'}">
				...
				<div>
					...
					<div style="padding-top: 5px; float: left">Doctor associated with all centers</div>
				</div>
				...
				<div style="margin-top: 10px">
					...
					<div style="padding-top: 5px; float: left">Doctor applicable only for following centers</div>
				</div>
				<div style="clear: both"></div>
					...
					<tr>
			 		...
			 		<c:forEach begin="1" end="${app_for_all_centers ? 1 : app_centers_count+1}" var="i" varStatus="loop">
						...
						<tr style="display: ${empty app_center ? 'none' : 'table-row'}">
							<td>
								<input type="hidden" name="doc_center_id" value="${app_center.doc_center_id}"/>
