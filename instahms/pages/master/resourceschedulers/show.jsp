...
<c:set var="actionUrl" value="update.htm"/>
...
<input type="hidden" name="method" value="update">
	...
	<input type="hidden" name="res_sch_id" value="${bean.map.res_sch_id}"/>
	<input type="hidden" name="res_sch_type" id="res_sch_type" value="${ifn:cleanHtmlAttribute(param.res_sch_type)}"/>
...
<c:set var="editText">
	<insta:ltext key="patient.resourcescheduler.common.edit"/>
</c:set>
	<h1>${editText} <insta:ltext key="patient.resourcescheduler.doctortimingdashboard.h1"/></h1>
				...
				<c:choose>
						...
						<c:otherwise>
							<c:forEach var="resource" items="${categoryDescripton}">
								<c:if test="${param.res_sch_type eq resource.map.res_sch_type}">
									${resource.map.resource_description}
								</c:if>
							</c:forEach>
						</c:otherwise>
					</c:choose>
	...
	<table class="screenActions">
		...
		<tr>
			...
			<insta:accessbutton buttonkey="patient.resourcescheduler.doctortimingdashboard.save" type="submit" onclick="return innerResourcesTable();" id="savebtn"/>
				...
				<td>&nbsp;|&nbsp;</td>
				<td><a href="javascript:void(0)" onclick="window.location.href='${cpath}/master/resourceschedulers/add.htm?method=add&category_default_duration=&category=&resource_type=primary'"><insta:ltext key="patient.resourcescheduler.doctortimingdashboard.add"/></a></td>
			<td>&nbsp;|&nbsp;</td>
			...
			<c:if test="${(not empty param.res_sch_type) && (param.res_sch_name != '*')}">
				<td>&nbsp;|&nbsp;</td>
				<td><a href="<c:out value='${url}'/>"><insta:ltext key="patient.resourcescheduler.doctortimingdashboard.overrides"/></a></td>
			</c:if>
		</tr>
	</table>
...
function onchangeDuration() {
	...
	window.location.href = "../../master/resourceschedulers/show.htm?method=show&res_sch_type="+category+"&duration="+duration+"&category_default_duration="+duration+"&dept="+dept+"&schedulerName="+schedulerName+"&resource_type=primary";
}
