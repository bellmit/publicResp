...
<form action="StoresUserIndentApproval.do" method="POST">
	...
	<h1> <insta:ltext key="storemgmt.indentapprovallist.addshow.user.approve.rejectindent"/> </h1>
	   ...
	   <legend class="fieldSetLabel">${indentType[indentdetails.map.indent_type] }</legend>
		 <table   class="formtable" >
			...
			<tr>
				...
				<c:choose>
					<c:when test="${indentdetails.map.status eq 'A' || indentdetails.map.status eq 'R'}">
						...
						<td class="forminfo">
							...
							<c:if test="${indentdetails.map.status eq 'R'}">${indentdetails.map.closure_reasons}</c:if>
						</td>
					</c:when>
					<c:otherwise>
						<td class="formlabel"><insta:ltext key="storemgmt.indentapprovallist.addshow.expecteddate"/>:</td>
			...
			<c:forEach var="indent" items="${indentlist}" varStatus="status">
				...
				<tr id="row${i}">
					...
					<c:choose>
						...
						<c:otherwise>
							<td>
								${indentStatus[indent.map.status]}
							</td>
						</c:otherwise>
					</c:choose>
				</tr>
	...
	<div class="screenActions">
		...
		<a href="${cpath }/stores/StoresUserIndentApproval.do?_method=list&status=O&sortOrder=indent_no&sortReverse=true"><insta:ltext key="storemgmt.indentapprovallist.addshow.backtodashboard"/></a>
	</div>
