...
<head>
<title>Purchase Summary - Insta HMS</title>
	<script>
	...
	var jStores = <%= StoresDBTablesUtil.getTableDataInJSON(StoresDBTablesUtil.GET_STORES_MASTER) %>;
		function csvReport() {
			...
			document.inputform.method.value = 'purchaseSummaryExportCSV';
			document.inputform.submit();
		}
		function pdfReport() {
			document.inputform.action="${cpath}/stores/PurchaseSummaryReport.do";
...
<html>
	<body onload="setDateRangeYesterday(fromDate, toDate);">
		<div class="pageHeader">Store Purchases Summary Report</div>
		<form name="inputform" method="GET" target="_blank"	action="PurchaseSummaryReport.do">
			<input type="hidden" name="report" value="PharmacyPurchaseSummary">
			...
			<div class="tipText">
				...
				dates. The report does not contain details of every item purchased, instead,
				only the invoice total amount, tax and discounts in each invoice. The report is
				grouped by suppliers, with sub-totals.<br/>
				<b>Note:</b> The CSV output does not include Invoice Level amounts (Invoice Discounts
				and Round Offs).
			</div>
			...
			<table align="center">
				...
				<tr>
					...
					<td>
						<select name="supplier_id" class="dropdown" >
